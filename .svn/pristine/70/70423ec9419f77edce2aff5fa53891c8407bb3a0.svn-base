...
for i=1:numel(ha)
    ...
    if i<= numel(data)
        ...
        if ScalarFieldSelection<7
            ...
            xlim(ax(i),[field_width.xmin,field_width.xmax]); ylim(ax(i),[field_width.ymin,field_width.ymax]);
        end
...
for k=1:max_points
    for i=1:numel(data)
        ...
        if any(strcmp(behavior,'Ridge Follow'))
            ...
            for ri = 1:length(xs)
                %Vri = SwarmSimWeightedAttractNew(RobotParams, ri, SensorRange,@MinGradient,[SensorRange]);
                Vri = SwarmSimWeightedAttractNew(RobotParams, ri, SensorRange,@HigherSVThanRefWeights, ss(ri)); 
                us(ri) = Vri(1);
                vs(ri) = Vri(2);
            end
            if length(h_line(i,:))< (j+1) || (~isgraphics(h_line(i,j+1)))
                h_line(i,2) = quiver3(ax(i),xs,ys,ss,us,vs,zeros(size(xs)));
            else
                set( h_line(i,j+1),'XData',xs,'YData',ys,'ZData',ss+50,'UData',us,'VData',vs,'WData',zeros(size(xs)));
            end
            j=2;
            while sum(~isnan(Sensor_Value)) >2
                ...
                Vrf = SwarmSimWeightedAttractNew(RobotParams, max_robot_i, SensorRange,@MinGradient,[SensorRange]);
