function [VCommand] = SwarmSimSum(Attract, Disperse, Avoidance,GoTo,Orbit, Formation,FindMin, FindMax, FindContour, FollowRidge, FollowTrench,FrontId, TrajFollow, Boundary)
...
%Total= 1*Avoidance +0.2*Orbit+0.5*Attract+0.3*u;
%Total= Avoidance +0.5*Attract+0.25*Orbit+ 0.25*FindContour;
%Total= 1*Avoidance +0.2*Orbit+0.3*Formation+0.5*u;
%Total= Avoidance +Attract;
%Total= Avoidance+0.1*u + 0.70*Disperse+0.1*GoTo + 0.10*Attract;
Total= Attract + Disperse + 1000*Avoidance +GoTo+Orbit+Formation+ FindMin + FindMax + FindContour + FollowRidge + FollowTrench+FrontId+TrajFollow+10000*Boundary;
VCommand(1)= Total(1);
VCommand(2)= Total(2);
VCommand(3)=0;
