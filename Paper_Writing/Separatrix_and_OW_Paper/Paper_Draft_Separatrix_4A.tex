...
along the stretching eigenvector: core-seek descends $\hat{s}_1$ along
the curve toward the TRAP core,
$\dot{\mathbf{p}}_c^{\,\parallel} =
\mathrm{sat}(-g_\perp\,\nabla\hat{s}_1^\top\mathbf{e}_2)\,
\mathbf{e}_2$, while ride cruises at constant speed,
$\dot{\mathbf{p}}_c^{\,\parallel} = v_{\max}\tanh(1)\,\mathbf{t}$
with $\mathbf{t} = \pm\mathbf{e}_2$ oriented by continuity with the
previous step's tangent $\mathbf{t}_{\text{prev}}$; the threshold
...
(Section~\ref{sec:strain_field}). The complete control law, with cases
evaluated in order once per cycle, is
\begin{equation}
    \dot{\mathbf{p}}_c =
    \begin{cases}
        -\mathrm{sat}(g_\perp\nabla\hat{s}_1), &
            r < \varepsilon_{\text{degen}}\ \text{or pre-band
            (ACQUIRE)}, \\[1pt]
        \mathbf{0}, & \hat{s}_1 > -s_{\text{trim}} \ \text{(TRIM)},
            \\[1pt]
        -\mathrm{sat}(g_\perp\nabla\hat{s}_1), &
...
\end{equation}
where the saturation in the ACQUIRE and PARK cases acts componentwise
on the gradient, the flag banded latches once $\hat{s}_1 <
-s_{\text{trim}}$ first holds, and PARK is active only in core-seek
mode, where $s_{\text{capture}}$ is set. The cases carry the terminal
behaviors inline: ACQUIRE descends $\hat{s}_1$ toward stronger
attraction before first structure contact or wherever the eigenframe is
unreliable; TRIM holds position where attraction no longer dominates,
the structure's Serra-Haller endpoint; PARK settles at the $s_1$
minimum inside the core well.

% Implementation: oecs_trap_step in pentagon_primitives.py.
The TRIM test plays the role that the capture test plays in the $D$
tracker, and $s_{\text{capture}}$ is the exact analogue of
...
tracker sits on its 35--38\% random floor out to 0.8, while the $D$
...
$\sigma_{uv}$ \textbackslash\ $\sigma_p$ & 0.000 & 0.005 & 0.010 & 0.020 & 0.050 \\
\hline
0.000 & 100.0 & 98.1 & 94.1 & 87.9 & 79.4 \\
0.001 & 100.0 & 98.1 & 94.5 & 87.8 & 79.8 \\
0.005 &  97.8 & 96.3 & 93.1 & 87.1 & 78.6 \\
0.010 &  93.6 & 92.8 & 90.6 & 85.6 & 78.8 \\
0.020 &  86.6 & 86.0 & 85.0 & 82.0 & 76.1 \\
0.050 &  69.7 & 70.1 & 69.5 & 69.3 & 68.3 \\
0.100 &  51.3 & 49.8 & 50.6 & 51.6 & 53.0 \\
0.200 &  39.1 & 39.5 & 39.7 & 40.9 & 41.0 \\
\hline
\end{tabular}
...
% experiments/outputs/mc_*/ and mc_analysis_10000.txt. 1000-trial
% baseline + 10,000-trial final archived read-only at
% experiments/outputs/mc_final_1000_ARCHIVE/ (README.md has provenance,
% including the 2026-07-09 5-trial overwrite of mc_separatrix and its
% restore from git commit 4de5e97).
\end{table*}
...
% experiments/outputs/mc_*/trials_fixed.csv. 10,000 trials/cell as of
% 2026-07-16 (commit 148df0a); see mc_final_1000_ARCHIVE/README.md.
...
99.6\% at $\sigma_{uv} = 0.001$, then falling through the 50\% level between
$\sigma_{uv} = 0.005$ and $0.01$, landing at 43.7\% at
...
Section~\ref{sec:noise_model}: $\sigma_p = 0.01$ alone gives 44.5\%,
close to $\sigma_{uv} = 0.01$ alone at 43.7\%. Beyond the cliff, success
does not fall to zero but levels off near 12--16\% through
...
within the sampling noise of the sweep (quartile success 75.9--77.1\%
...
% Sweep: experiments/mc_sweep_oecs.py, 10,000 trials/cell, fixed start
% (0, 0.35), core-seek; table from outputs/mc_oecs/summary_fixed.csv,
% git 148df0a. Auto-extension ran to sigma_uv = 0.8; the 0.4 and 0.8
% rows sit on the random floor (35-38%) and are summarized in prose.
% 1000-trial baseline + 10,000-trial final archived read-only at
% outputs/mc_final_1000_ARCHIVE/ (see README.md there for provenance).
...
traverse, core arrival still succeeds in 93.6\% of trials; the 50\%
level is not reached until $\sigma_{uv} \approx 0.1$, and beyond it
success settles onto a floor of 35--38\% (measured out to $\sigma_{uv}
...
between $\sigma_{uv} = 0.01$ and $0.02$ (0.712 at 0.01, 0.497 at
...
alone, which leaves the $D$ tracker at 19.0\%, leaves the $s_1$
tracker at 79.4\%. Part of the margin is the shorter task, inherent to
...
uniform-random-start trials, roughly half (47.1\%, $n = 10{,}000$) of
