...
\cite{9} uses for its own Santa Barbara Channel experiment, the same
...
in the words of \cite{9} on this same record, ``extremely challenging
...
straddle to stop, the failure count of \cite{9}, used here as an
...
% =====================================================================
% TEMPORARY STAGING. Prose moved out of Section VI (2026-07-30) so that
% Results reports only observations answering the Test Plan. Nothing here
% is new text; each block is verbatim from the Results passage named in
% its comment, except where a fragment needed a subject to stand alone.
% To be redistributed into the Discussion subsections above, then this
% subsection and these comments deleted.
% =====================================================================
\subsection{Moved from Results (unsorted)}
\label{sec:moved_from_results}

% from VI-A, six clean D-tracker runs. Candidate home: Failure Modes,
% which already treats the floor e on the H_D eigenvalues.
This traces to the floor $e$ on $\hat{\mathbf{H}}_D$
(Section~\mbox{III-D}). Near the well the estimated eigenvalues are
indefinite and more than an order of magnitude too small, so the
capture test $\lambda_1\lambda_2 \geq 0$ does not fire even without
noise.

% from VI-A, estimator accuracy (Fig. 4b). Candidate home: Failure
% Modes, which already argues the frame degrades before the gradient.
Fig.~\ref{fig:est_accuracy}(b) shows that the gradient is not the only
input failing there. The floor $e$ does leave the eigendirections
intact when nothing else perturbs them. Both quantities the $D$
tracker consumes, the gradient and the curvature eigenframe, become
unusable at the same noise level, and the eigenframe is the one that
fails first.

% from VI-A, noise-ceiling paragraph. Also asserted in the Conclusion.
The estimator, not the control law, therefore sets the noise ceiling.

% from VI-A, acquisition sweep.
Acquisition itself is not the noise-limited phase. Arrival at the one
designated terminal saddle is a different quantity, set by which branch
of the network the descent reaches first. At zero noise the 40\%
arrival rate reflects outcome isotropy, not estimator isotropy. The
initial heading alone decides which branch the descent reaches first,
and heading is not uniform across the two outcomes it produces here,
while the estimator-error isotropy of (\ref{eq:gain_ladder}) holds and
is confirmed independently.

% from VI-B-1, noise-free s1 runs.
The largest path gaps occur from a start equidistant enough that the
seeded tangent and the $D$ tracker's own flow-tested tangential term do
not agree; where they commit to the same saddle the two paths are
visually coincident until the $s_1$ tracker settles and the $D$ tracker
continues onto the crossing wall trench, the divergence the floor $e$ of
Section~\mbox{III-D} already predicts for the $D$ tracker's own command.
Both trackers traverse the same network from the same objective starting
condition, but only the $s_1$ tracker's terminal test is built to fire
reliably at what it finds.

% from VI-B-2, objectivity. Candidate home: Objectivity Tradeoffs.
The rotating observer's $D'$ landscape (\ref{eq:D_not_objective}) and
swirl-contaminated flow measurement are what carry the $D$ tracker off
the separatrix. Under a rotating observer the $D$ tracker follows a
frame artifact; the $\mathbf{S}$-based tracker does not, past the single
seeding instant Proposition~\ref{prop:equivariance} identifies, which
confines the frame dependence to that one instant rather than carrying
it for the life of the run.

% from VI-B-3, tracking-error ordering. Candidate home: Objectivity
% Tradeoffs, which already reads the same conclusion off Table V.
The ultimate bound (\ref{eq:iss_bound}) predicts the opposite ranking of
zero-noise tracking error, since $a_{\perp,s_1} > a_{\perp,D}$ at this
operating point. The ordering instead matches the discrete stability
condition of Table~\ref{tab:bandwidth}, which the $s_1$ tracker crosses
over the outer half of the structure. Under noise the two trackers do
not degrade the same way, and the difference is not one of degree.

% from VI-B-3, panel (b) against panel (c).
What separates the two trackers is visible only by comparing
Table~\ref{tab:mc_success}(b) with panel (c). The team has not lost the
...
inversion, not a decay toward chance, and it is what makes the
far-saddle column the honest score. A near-saddle settle is a
directional failure of the ride, indistinguishable in every other
recorded quantity from a success.

% from VI-B-3, the seed mechanism. Candidate home: Limitations, which
% already states the seed's flow dependence.
The mechanism is the one-time flow read that seeds
$\mathbf{t}_{\text{ref}}$ on the first tracking step. At this start the
differential flow signal across the six sample points is small (span
$\approx 0.12$), comparable to $\sigma_{uv}$ in the 0.001--0.003 range,
so the sign it resolves is decided by noise well before the transverse
channel's own tolerance is approached. Against peak flow, $\pi A \approx
0.314$, $\sigma_{uv} = 0.002$ is a modest 0.6\%; against this 0.12
differential span, the denominator the tangent decision actually
consumes, the same noise is 1.7\%, which is the reading that explains
the seed's fragility. Because the decision is made once and then
...
opposite branch. The noise is comparable to that 1.7\%-scale signal, so
above the threshold nearly every draw crosses it in the same direction
rather than half reversing and half not. The flip is near-total, not an
even split.

% from VI-B-3, the sigma_p axis.
Position noise enters through the same effective-measurement-error
coupling $\mathbf{J}(\mathbf{p}_i)\boldsymbol{\xi}_i$ of
(\ref{eq:pos_noise}), so at this test point, where the $D$ tracker's own
noise grid gave $\|\mathbf{J}\| \approx 1$, a given $\sigma_p$ degrades
the seed by roughly the same amount as an equal $\sigma_{uv}$ would. The
two panels of Fig.~\ref{fig:flip_resolution} are one finding measured on
two axes, not two failure modes. Whichever noise channel is active, it
is the one-time flow read that fails first, well before the transverse
channel's own regulation would predict trouble.

% from VI-B-3, straddle retention.
Straddle retention staying below success is consistent with straddle
being the stricter condition once both are scored against the same
target. The $D$ tracker's own retention does not depend on which saddle
is reached, since its target never moves.

% from VI-B-3, tangent selection. Candidate home: Failure Modes, which
% already names tangent selection as the defense once a ride is underway.
The tangent-selection mechanism itself (\ref{eq:tangent_select}) is not
the source of this fragility. It uses only the already-computed gradient
projections, and the tangent direction survives noise past the point
every gradient-based quantity approaches chance, which still applies to
whichever eigenvector it currently selects. The fragility is specific to
the sign convention at the seed, a single comparison against the ambient
flow.

% from VI-C, FTLE anchor times.
The ocean experiment is genuinely time-varying, and each controller sees
only the instantaneous samples at its own formation position.

% from VI-C, s1 tracker on the ocean field. The ground-truth sentence
% here describes how ground truth was computed and may belong in the
% Ocean HFR Setup of Section V-C rather than in the Discussion.
Reaching no two-dimensional minimum of $s_1$ within the budget is an
outcome of the same law, not a failure of it. Absent a stationary point
of $s_1$ close enough to the ride to satisfy (\ref{eq:capture_test}),
the $s_1$ tracker behaves as a traverser, reading the same ridge
structure the $D$ tracker does from the objective strain term alone.
Ground truth on this field, computed offline from the full radar grid in
the style of \cite{17,36} (TRAP cores as local minima of $s_1$ at each
hourly frame), is generic, unlike the double gyre. It has isolated
...
where neither the eigenframe nor the trench network is axis-aligned. The
method runs unmodified on measured data for 28~h and produces a path
that follows the dominant FTLE ridge, the same demonstration the $D$
tracker gives from the same start.

% ===== end TEMPORARY STAGING =====
