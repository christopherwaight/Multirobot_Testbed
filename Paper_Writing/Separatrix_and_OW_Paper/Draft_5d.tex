...
is Galilean invariant while the latter is objective, a distinction that
matters whenever a team cannot certify its own orientation against a
trusted external reference, as when a vehicle dead-reckons on a drifting
compass heading between GPS fixes or operates without GPS at all
\cite{22,23}.
...
conditioning of $\boldsymbol{\Phi}$ (Section~\mbox{VII-E}).
...
removes by a separate mechanism (Section~\mbox{VII-B}). All
determinant-derived quantities are additionally frame dependent at a
relative $O(\mathrm{Ro})$ (Section~\mbox{VII-C}).
...
positions back into cluster variables for feedback \cite{37,38}. The
...
The command is well defined despite the sign ambiguity of eigenvectors.
The transverse term is quadratic in $\mathbf{w}_2$ and so does not
depend on its sign, and the tangential term fixes the sign of
$\mathbf{w}_1$ explicitly. One guard falls outside (\ref{eq:d_law}).
...
dropped. Each proof follows its statement.
...
of (\ref{eq:tau}) since neither transverse command contains $\tau$, the
exponential rate following from concavity of $\tanh$ on $|n| \leq
c_{\max}$. For (ii), both branches return $\tau \geq 0$ with
$\mathbf{w}_1$ signed so that $\hat{\mathbf{v}}_0^\top\mathbf{w}_1 \geq
0$, so the tangential command never opposes the flow. On the band $\tau
= \hat{\mathbf{v}}_0^\top\mathbf{w}_1 \geq f_{\min}$ outside the $r_0$
balls; off it $\tau = |h'(\ell)|/|\lambda_1| \geq
m_h/\bar{\lambda}_\parallel$, with $\bar{\lambda}_\parallel$ bounding
$|\lambda_1|$ on the tube. Since $\tanh$ is increasing, the realized
tangential speed is at least
\begin{equation}
    m = c_{\max}\min\bigl\{\tanh(f_{\min}/c_{\max}),\;
    \tanh(m_h/\bar{\lambda}_\parallel c_{\max})\bigr\},
    \label{eq:progress_rate}
\end{equation}
and since (i) holds in both branches the band test cannot reverse
$\ell$, so a trench of length $L_\Gamma$ takes at most $L_\Gamma/m$.
\end{IEEEproof}
...
\begin{IEEEproof}
By Section~\mbox{II-C} the transport term in $\mathbf{J}_Q$ is
antisymmetric and lands in the spin part, so $\mathbf{S}_Q = \mathbf{Q}
\mathbf{S}\mathbf{Q}^\top$, leaving $s_1$ and $r$ invariant and the
eigenvectors and $\nabla s_1$ co-rotating. Every test setting $\beta$ is
a scalar inequality in those invariants, so $\beta$ agrees at
corresponding states. Rotation leaves (\ref{eq:tangent_select})
unchanged, since $\lvert(\mathbf{Q}\nabla s_1)^\top(\mathbf{Q}
\mathbf{e}_k)\rvert = \lvert\nabla s_1^\top\mathbf{e}_k\rvert$; and if
$\mathbf{t}_{\mathrm{ref},Q} = \mathbf{Q}\mathbf{t}_{\mathrm{prev}}$ at
some step, the sign test is likewise unchanged and $\mathbf{t}_Q =
\mathbf{Q}\mathbf{t}$, which supplies the reference for the next, so by
induction the tangent is equivariant from the second tracking step
onward. Then $\mathbf{P}_Q = \mathbf{Q}\mathbf{P}\mathbf{Q}^\top$, and
the saturation scales an invariant magnitude while preserving a
co-rotating direction, so $\boldsymbol{\delta}_Q = \mathbf{Q}
\boldsymbol{\delta}$ and integrating (\ref{eq:kinematic_model}) gives
the stated relation. The exception is the degenerate guard, which reads
the measured flow. That carries $\dot{\mathbf{Q}}\mathbf{Q}^\top
\mathbf{p}_Q$ with no inertial counterpart, and is reachable only on the
first tracking step, where the same fallback seeds the sign. At that one
instant the resolved sign need not agree between frames; from the second
step onward orientation is carried by continuity alone, which is
covariant.
\end{IEEEproof}
...
(Section~\mbox{V-C}). Ocean values are in world units per simulated
...
and outside the scope.'' The noise model of Section~\mbox{V-D} is
...
directional failure, and those runs are counted separately in
Table~\ref{tab:mc_success}(c). Each trial also records straddle
...
$|y| \leq 0.60$ against $0.52$, for the few-centimetre terminal
...
sweep of Section~\mbox{V-A} draws $10^4$ realizations per condition at a
...
the degenerate-frame guard is needed at the isotropic point. And
...
the terminal saddle rather than stopping there. The capture test
$\lambda_1\lambda_2 \geq 0$ never fires, even without noise.
...
5\%. In Fig.~\ref{fig:est_accuracy}(b), truncation acting alone costs the
$\hat{\mathbf{H}}_D$ eigendirections $2.2^\circ$, while noise
costs them $24^\circ$ by $\sigma_{uv} = 0.003$ and $39^\circ$ at the cliff,
against $45^\circ$ for a uniformly random axis.
...
Straddle retention, the failure metric of \cite{17}, decays cleanly with
...
$\sigma_{uv} = 0.05$. From the start whose ride can reach either
saddle, arrival at the one designated terminal saddle holds near 40\%
from zero noise through $\sigma_{uv} = 0.01$, one grid step later than
the straddling start's own cliff in Table~\ref{tab:mc_success}(a).
...
0.322 across the six (Fig.~\ref{fig:traverse_vs_logic_c}). The gap is
largest where the two trackers commit to different saddles.
...
saddle. The $D$ tracker's two runs are not. Its rotating-frame run
leaves the separatrix entirely, reaching a mean trench-network distance
of 0.052 (ten times its inertial value of 0.005) and a final gap of
1.219 from its inertial twin, ending in the gyre interior where no
structure exists.
...
0.0016. (The widened $0.60$ exit box recovers
...
Comparing Table~\ref{tab:mc_success}(b) with panel (c), where
far-saddle success is 0.0\% at $\sigma_{uv} = 0.005$, 90.9\% of those
same runs still settle cleanly at a saddle. The fraction of runs whose
ride commits toward $\mathbf{p}^*_1$ is 100.0\% at zero noise, 91.9\%
at $\sigma_{uv} = 0.001$, and 0.1\% at 0.005.
...
1\%). The $D$ tracker's own straddle retention decays more gradually
over the same interval (0.589 at 0.005, 0.389 at 0.01).
...
Open-loop checks were run at the nominal
radius. The fitted shear strain at three test points is
...
offline over the same window in the style of \cite{25}; both paths
...
ridge persists while the finer structure downstream evolves.
...
A genuine two-dimensional minimum of $s_1$ is not reached within
the 28-h budget from this start, so the terminal test
(\ref{eq:capture_test}) never fires and the $s_1$ tracker's mean
$\hat{s}_1$ along the ride is the tracked value rather than a certified
settle.

\section{Discussion}
\label{sec:discussion}

Each of the four experiment families of Section~\mbox{V-E} puts the two
trackers on the same field under one changed condition, so each returns
a verdict. The four verdicts differ, and taken together they say when
each surrogate should be used.

\subsection{Clean Runs: One Structure Read Two Ways}
\label{sec:disc_clean}

On the noise-free double gyre the two trackers behave the same. From
the same six starts, spanning both gyre halves, the origin, and points
off the structure entirely, both acquire the trench network, ride the
same segments in the same direction, and cross the isotropic point at
the origin. Theorem~\ref{thm:separatrix} does not distinguish them: the
normal form (\ref{eq:normal_form}) is written for either surrogate and
both laws command the same transverse dynamics (\ref{eq:n_dynamics}).
The runs agree with that. Where the two commit to the same saddle the
paths are visually coincident, and the gaps of 0.020 to 0.322 open only
where they commit to different ones, from a start equidistant enough
that the seeded tangent and the $D$ tracker's own flow-tested
tangential term do not agree. Neither needs dedicated machinery at the
crossing. The $s_1$ tracker rides through the origin on ordinary
tangent selection (\ref{eq:tangent_select}), and over the sweep of
$r_{\text{band}}$ from 0.05 to 0 the degenerate-frame guard never fires
once.
...
strain-dominated so that $s_1 < 0$, the two transverse derivatives
vanish together and the two trenches are the same curve. The double
gyre's entire trench network is such a set. With the field
(\ref{eq:dg_u}), $\omega = -2\pi^2 A \sin(\pi x_f)\sin(\pi y_f)$, which
is identically zero on the separatrix $x_f = 1$ and on all four walls.
The lone exception is the isotropic point at the origin, where
$\mathbf{J}$ vanishes entirely and the step from
(\ref{eq:trench_coincide}) is unavailable.
The six clean runs therefore agree pointwise for a reason specific to
this field, not because the two surrogates are interchangeable in
general. Section~\mbox{VII-D} removes the coincidence.

The trackers part only at the end. The $s_1$ tracker settles, within
0.011 to 0.022 of a saddle. The $D$ tracker rides onto the crossing
wall trench and continues, because the capture test $\lambda_1\lambda_2
\geq 0$ does not fire even without noise. That traces to the floor $e$
on $\hat{\mathbf{H}}_D$ (Section~\mbox{III-D}). Near the well the
estimated eigenvalues are indefinite and more than an order of
magnitude too small, so the definiteness the test asks about is not a
quantity the fit returns.

The terminal split is a fact about the model class rather than about
tuning, and it is forced in both directions. Under a quadratic velocity
fit $\hat{\mathbf{H}}_{s_1}$ is negative semidefinite for every field
and every formation (Remark~\ref{rem:s1hess}), while
$\hat{\mathbf{H}}_D$ carries a floor no formation radius or sensor
improvement reduces. Neither surrogate admits a definiteness
...
The $D$ tracker's traversal and the $s_1$ tracker's capture are one
consequence of representability read twice.

The same clean-field descent also fixes the reachable set, and it is
not the Okubo-Weiss partition. No start settles at a gyre core. Descent
of $\hat{D}$ from the rotation-dominated interiors reaches the trench
network within the 500-step horizon in all 20\,000 runs. The basin map
is instead the watershed of the descent composited with the flow
direction on the acquired branch, a central diamond of direct
traverses (26.9\%), an upper region that rides the top wall to
$\mathbf{p}^*_2$ and continues to $\mathbf{p}^*_1$ (20.5\%),
interleaved with north-branch losses at the $\mathbf{p}^*_2$ junction
(5.6\%), and lower and outer watersheds whose boundary-trench rides
leave the domain without reaching $\mathbf{p}^*_1$
($47.1\%_{\text{grid loss}}$), since the walls are also trenches of $D$
and the bottom wall carries flow away from $\mathbf{p}^*_1$. The grid's
own success rate, direct plus top-wall arrivals, is $26.9\% + 20.5\% =
47.4\%$, agreeing within sampling noise with a separate
$47.1\%_{\text{random}}$ from $n = 10{,}000$ uniformly random starts
and headings. The near-match of that $47.1\%_{\text{random}}$ to the
unrelated grid-loss figure $47.1\%_{\text{grid loss}}$ above is a
coincidence of digits between two different quantities, not a second
confirmation. Prior adaptive-navigation primitives \cite{3}, \cite{4}
report local convergence without basin estimates.

\subsection{Under Noise: Why the $D$ Tracker Holds Longer}
\label{sec:disc_noise}

Noise separates the two, and it favors the $D$ tracker. Its 50\%
crossing sits a full grid step later, and the two do not fail the same
way.

For the $D$ tracker the cliff and an estimator threshold nearly
coincide. Traverse success crosses 50\% at $\sigma_{uv} \approx 0.0079$
and $\nabla\hat{D}$ stays informative to $\approx 0.0075$, within 5\%.
Read alone, that agreement suggests the gradient is what runs out.
Fig.~\ref{fig:est_accuracy}(b) shows it is not the only input that has.
At the cliff the curvature eigenframe is already $39^\circ$ from truth
against $45^\circ$ for a uniformly random axis, and it passed
$24^\circ$ by $\sigma_{uv} = 0.003$, well before. Both terms of
(\ref{eq:d_law}) are built on that frame and normalized by its
eigenvalues, so the command inherits the degradation in full. The
mechanism is separate from the floor $e$, which falls on the
eigenvalues and leaves the eigendirections nearly intact. The
directions are resolved instead by the separation of the eigenvalues
against the entry noise (\ref{eq:gain_ladder}) places at
$\nu/\tilde{\rho}^{\,2}$, and once that noise reaches the separation
the recovered frame is uniformly distributed. So the coincidence is a
weaker diagnosis than it looks. By the cliff both inputs the law
consumes have failed, and the eigenframe failed first. What the
coincidence does establish is the ceiling's owner, since neither
threshold is a property of (\ref{eq:d_law}). The estimator, not the
control law, sets the noise ceiling.

Two further readings sit alongside it. Beyond the cliff success does
not fall to zero but levels off in the low tens of percent through
$\sigma_{uv} = 0.2$, an unbiased-random-walk floor that decays only at
the extreme extension $\sigma_{uv} = 0.8$, while straddle retention
decays cleanly with no such floor. And acquisition is not the
noise-limited phase: the team reached the trench network in every one
of the 20\,000 acquisition trials through $\sigma_{uv} = 0.02$. Arrival
at the one designated terminal saddle is a different quantity, set by
which branch of the network the descent reaches first, so the 40\%
arrival rate at zero noise reflects outcome isotropy rather than
estimator isotropy. The estimator-error isotropy of
(\ref{eq:gain_ladder}) holds and is confirmed independently, by
initial-heading quartile.

The $s_1$ tracker fails earlier, for a reason that has nothing to do
with estimator accuracy. Its success falls from 91.8\% at $\sigma_{uv}
= 0.001$ to 0.0\% by 0.005, where the $D$ tracker's corresponding
column falls only from 62.4\% to 43.7\%. Table~\ref{tab:mc_success}(c)
says what the failed runs are doing. At $\sigma_{uv} = 0.005$, where
far-saddle success is 0.0\%, 90.9\% of those same runs still settle
cleanly at a saddle. The team has not lost the structure, become
erratic, or collapsed. It rides the separatrix competently and settles,
in the wrong direction. This is a sign inversion, not a decay toward
chance, and it is what makes the far-saddle column the honest score. A
...
Against peak flow, $\pi A \approx 0.314$, $\sigma_{uv} = 0.002$ is a
modest 0.6\%. Against that 0.12 span, the denominator the sign decision
actually consumes, the same noise is 1.7\%, and the second reading is
the one that explains the fragility. It explains its shape as well.
Because the noise is comparable to the signal the decision reads, draws
above the threshold cross it in the same direction rather than half
reversing and half not, so the flip is near-total rather than an even
split. Position noise enters the same comparison through the coupling
$\mathbf{J}(\mathbf{p}_i)\boldsymbol{\xi}_i$ of (\ref{eq:pos_noise}),
and at this test point $\|\mathbf{J}\| \approx 1$, so an equal
$\sigma_p$ degrades the seed by roughly the same amount. The two panels
of Fig.~\ref{fig:flip_resolution} are one finding measured on two axes,
not two failure modes. Tangent selection itself is not the source. It
uses only the already-computed gradient projections, and the tangent
direction survives past the noise level at which every gradient-based
quantity approaches chance. The fragility is specific to the sign
convention at the seed, a single comparison against the ambient flow.

One prediction of Section~\mbox{IV-D} comes out backwards here, and the
discrepancy locates itself. The ultimate bound (\ref{eq:iss_bound}) is
$\delta/a_\perp$, and $a_{\perp,s_1} > a_{\perp,D}$ at this operating
point, so it predicts the smaller zero-noise tracking error for the
$s_1$ tracker. The measurement is the reverse, 0.0075 against 0.0016.
The ordering matches the discrete stability condition of
Table~\ref{tab:bandwidth} instead, which the $s_1$ tracker crosses over
the outer half of the structure. The bound is a continuous-time
statement and the limit that binds at this step size is a discrete one,
so a larger $a_\perp$ tightens the first while violating the second.
...
(Section~\mbox{VII-C}), and it is still the less robust tracker here,
because one of its decisions is made once and never revisited. A
perturbation large enough to flip that decision does not degrade the
run, it commits the whole ride to the opposite branch. That is why the
transition is continuous on both noise axes while the outcome it
selects between is not (Fig.~\ref{fig:flip_resolution}). Robustness is
set less by the per-cycle accuracy of what a law reads than by how many
of its commitments are unrevisable.

\subsection{Under a Rotating Observer: Why the $D$ Tracker Fails}
\label{sec:objectivity_tradeoff}

The rotating-frame experiment reverses the ranking, and not by a
margin. The $s_1$ tracker's two runs are the same material path, final
gap 0.025 and mean gap 0.014, with mean distance to the true trench
network of 0.000 inertial against 0.005 rotating. The $D$ tracker's
rotating-frame run does not degrade, it leaves the structure. Final gap
1.219 from its inertial twin, mean trench-network distance ten times
its inertial value, ending in the gyre interior where no structure
exists.
...
(\ref{eq:tau}) reads, so both terms of (\ref{eq:d_law}) are steered by
a frame artifact rather than by the field. No gain recovers this,
because the artifact is in the surrogate. The $s_1$ tracker inherits
the protection of Proposition~\ref{prop:equivariance} instead. That
proposition is not a claim of exact frame independence, since it
exempts one instant and holds only up to the frame-transport term, so a
small nonzero gap is what it predicts rather than a violation of it.
The experiment bounds that residue at 0.025 over a full run without
separating its sources.

Objectivity is not free, and the double gyre is the wrong field on
which to price it. Objectivity buys no extra reach there, since the two
trenches coincide for the reason given in Section~\mbox{VII-A}, and the
identically vanishing shear keeps $r$ away from zero, so the $1/r$
exposure a generic field loads is barely tested. What the double gyre
does expose is rate. The $D$ tracker normalizes by a curvature and
holds $a_\perp = 1$ along the whole structure. The $s_1$ tracker has no
curvature to normalize by (Remark~\ref{rem:s1hess}), so it takes
$a_\perp = g_\perp\kappa_\perp$ from the field, and at the operating
point of Table~\ref{tab:params} that puts the outer half of the
structure past the discrete stability threshold
(Table~\ref{tab:bandwidth}). The five-fold zero-noise tracking-error
gap of Section~\mbox{VII-B} is that cost, measured.

The accounting is clean. The ladder (\ref{eq:gain_ladder}) places both
surrogates on the same rung, so the choice between them is not one of
accuracy. It is which channels carry constants. Everything derived from
$\hat{D}$ carries the observer term $O(\mathrm{Ro})$, and
$\hat{\mathbf{H}}_D$ carries the unmodelled-derivative floor $e$ as
well. $\hat{s}_1$ carries neither and is exposed instead at $1/r$,
which belongs to the strain field rather than to the sensing. The
rotating-frame gaps are the first of those constants made visible, and
the gap contains no $\nu$, no $\tilde{\rho}$, and no formation
geometry, so no estimator improvement removes it. Objectivity is
therefore free at the estimator and paid at the controller, in the
Newton normalization Remark~\ref{rem:s1hess} forbids. One derivative
order runs the other way and partly offsets it. The tangent the $s_1$
tracker rides comes from the \emph{first-order} fitted coefficients,
one order better in noise gain than the $\hat{\mathbf{H}}_D$
eigenvectors the $D$ tracker uses for the same purpose and free of the
floor those carry. At $\sigma_{uv} = 0.01$ the median direction error
of $\mathbf{e}_2$ is $4.5^\circ$, against $41^\circ$ for the
$\hat{\mathbf{H}}_D$ eigenvector and $44^\circ$ for $\nabla\hat{s}_1$,
the $s_1$ tracker's own second-order quantity. The ladder's exponent is
visible inside a single tracker at a single noise level.

\subsection{On a Measured Field: Different Paths, One Corridor}
\label{sec:disc_ocean}

The ocean record removes the coincidence Section~\mbox{VII-A} rests on.
A measured field has no reason to carry a vorticity-free trench
network, so (\ref{eq:trench_coincide}) no longer forces the two
surrogates onto a common curve and the two trackers need not produce
the same path. They do not, and it matters less than the exact
agreement on the double gyre would suggest. Over the 28-h trial both descend through the
channel entrance and track the same ridge through the bifurcation,
their paths overlapping for nearly the entire record. They separate in
the final stretch, where the $D$ tracker continues to landfall on the
middle island's north coast while the $s_1$ tracker is still closing on
the same target when the budget ends.

What survives the loss of pointwise agreement is the corridor. Both
paths follow the dominant FTLE ridge from the channel entrance to the
bifurcation, computed offline over the same window, without that ridge
being available to either controller at any point. Only the local
estimates built from the current samples at the formation enter the
commands. The $D$ tracker's outcome is not knife-edge sensitive to the
start either, with 84 of 100 jittered starts landing on the same
middle-island branch. Two controllers reading different halves of the
...
Instantaneous criteria of this kind are not in general reliable
indicators of the material transport skeleton \cite{24}. The double
gyre is a favorable special case, and this is a finding on one record
rather than a guarantee that carries to arbitrary flows. The record is
also genuinely time-varying, with each controller seeing only the
instantaneous samples at its own formation position and no analytic
guarantee offered for that case. Forward FTLE recomputed at four anchor
times establishes only that the dominant ridge persists while finer
structure downstream evolves.

The field is generic in a second way that matters for the $s_1$
tracker. It has isolated degeneracies and cores that move between
frames, so the experiment exercises tangent selection
(\ref{eq:tangent_select}) where neither the eigenframe nor the trench
network is axis-aligned, which the double gyre cannot do. No genuine
two-dimensional minimum of $s_1$ is reached within the 28-h budget from
this start, so (\ref{eq:capture_test}) never fires.

That non-firing is an outcome of the law rather than a failure of it,
and it closes the terminal argument of Section~\mbox{VII-A} from the
other side. Absent a stationary point of $s_1$ close enough to the ride
to satisfy (\ref{eq:capture_test}), the same unmodified controller
behaves as a traverser, reading the ridge structure from the objective
strain term alone. Which of the two behaviors appears, capture on the
double gyre and traverse on the ocean, is a property of the field and
not of a setting. The terminal test is unconditional by design and has
no mode to select.
...
set. A level set is fixed by first-order information, while a trench
...
assumptions of Section~\mbox{IV-D} are the price. Three further
conditions sit outside that analysis. The fit is ill-conditioned
whenever the six sampling points drift toward a common conic
(Proposition~\ref{prop:conic}); monitoring $\kappa(\boldsymbol{\Phi})$
and adding a shape-maintenance term is the direct mitigation, not
implemented here. Noise can chatter the $D$ tracker across the band
boundary (\ref{eq:flow_band}), which costs only along-trench speed,
since both branches of (\ref{eq:tau}) share the transverse dynamics
(\ref{eq:n_dynamics}). And the strain eigenframe is fragile where
...
rather than eigenvector identity, and the residual risk of a wrong
branch at a crossing under heavy noise is damped but not excluded by
the latch on (\ref{eq:capture_test}).

The $s_1$ tracker's tangent seed is the sharpest of the design
limitations, and Section~\mbox{VII-B} gives its mechanism. Three
unimplemented alternatives trade it differently. A start-independent
...
already accepts. A confidence check on the seed converts a wrong
commitment into a longer settling phase at the cost of a noise-level
estimate the controller does not otherwise need. Re-fitting the sign
over several early steps trades the single-instant risk for a short
transient of ordinary tracking-error exposure, closer in spirit to how
time-to-band already absorbs acquisition. None was implemented, since
the finding was made late enough in the noise-sweep validation that a
design change here would need its own noise sweep to verify.

No controlled head-to-head against the straddle family
\cite{17,18,19} or the onboard-FTLE strategy of \cite{21} was run, and
two grounds keep the comparison of Section~\mbox{I-A} structural rather
than empirical. First, initialization is not a detail of those methods
but the premise of their guarantees. \cite{17}'s straddling formation
control is stated for robots already on opposite sides of the boundary,
and \cite{21} assumes the center robot starts on $B_u$. Running either
from an off-structure start is outside its own stated problem. Second,
the two plants differ. \cite{17,18,19,20} and \cite{21} both write the
robot's velocity as the commanded term plus the ambient flow (their
eq.~1a in \cite{17}, eq.~5a in \cite{21}), so their robots are
advected, while this paper's are not
(Section~\ref{sec:problem_statement}). One simulator cannot host both
without altering one of them.
...
\cite{40}. On the ocean field it is an extrapolation. The commanded
...
The cluster controller of Section~\mbox{IV-A} acts on a twelve-element
cluster pose, three variables locating and orienting the formation and
nine describing its shape. The formation is parameterized as a cluster
of clusters. Robots are grouped into three pairs, $\{1,2\}$, $\{3,4\}$,
and $\{5,6\}$, with pair midpoints
\begin{equation}
    ...
    \label{eq:pair_midpoints}
\end{equation}
where $\mathbf{p}_i = (x_i, y_i)$ is the position of robot $i$ and
$\mathbf{m}_\bullet = (m_{\bullet x}, m_{\bullet y})$. The three
midpoints form a triangle carrying the coarse geometry of the
formation, described in the side-angle-side form standard in cluster
space work \cite{37,38}, and each pair carries a length and an
orientation about its own midpoint. Writing $c$ for the cluster frame,
the forward position kinematics are
\begin{align}
    ...
    \label{eq:ck_th4}
\end{align}
Equations (\ref{eq:ck_xc}) and (\ref{eq:ck_yc}) place the cluster
frame at the centroid of the three midpoints, which for equal pair
weighting is the centroid of the six robots, so $\mathbf{p}_c =
(x_c, y_c)$ throughout the paper. Equation (\ref{eq:ck_thetac}) fixes
the heading along the $\mathbf{m}_B \to \mathbf{m}_A$ leg;
(\ref{eq:ck_p})--(\ref{eq:ck_beta}) give the side-angle-side triangle
on the midpoints, with $\beta_1$ the interior angle at
$\mathbf{m}_B$ recovered by the law of cosines; and
(\ref{eq:ck_L2})--(\ref{eq:ck_th4}) give each pair's separation and
bearing. The last nine are the shape variables the formation
controller regulates to their nominal values, while $x_c$, $y_c$, and
$\theta_c$ take their commands from the navigation layer.

The symbols in (\ref{eq:pair_midpoints})--(\ref{eq:ck_th4}) belong to
the cluster controller layer alone, and several of them are spelled the
same as symbols used elsewhere in the paper for unrelated quantities:
$p$ and $q$ appear as expansion orders in the error budget
(\ref{eq:error_budget}), $\beta$ as the ride flag in
(\ref{eq:oecs_law}), and the Jacobian of this appendix is not the flow
velocity gradient $\mathbf{J}$ of (\ref{eq:jacobian}). The layered
architecture of Section~\mbox{IV-A} is what keeps these apart. Each
layer presents a fixed interface to the one above and holds its own
state, so a cluster variable never enters an estimator expression and
an estimator quantity never enters a kinematic one. The only symbols
crossing the boundary are the centroid $\mathbf{p}_c$ and the command
$\dot{\mathbf{p}}_c$. The reuse is therefore harmless, but the reader
should take the definitions above as local to this appendix and to the
cluster controller layer.

The inverse kinematics follow geometrically rather than by algebraic
inversion. The triangle is built in the cluster frame from $p_1$,
$\beta_1$, and $q_1$ by placing $\mathbf{m}_B$ at the origin and
$\mathbf{m}_A$ along the frame's $x$-axis; the construction is
translated so its own centroid sits at the origin, rotated by
$\theta_c$, and translated to $(x_c, y_c)$; each pair is then expanded
symmetrically about its recovered midpoint by $\pm\tfrac{1}{2}L_k$
along $\theta_k$. With the forward and inverse position kinematics in
hand, the forward Jacobian and its inverse are obtained in the usual
way \cite{37}, by differentiating (\ref{eq:ck_xc})--(\ref{eq:ck_th4})
and the inverse relations respectively, giving the $12 \times 12$
matrices that map robot velocities into cluster velocities for feedback
and cluster velocities into robot velocities for command.

The nominal geometry used in every experiment is the
pentagon-plus-center configuration of
Corollary~\ref{cor:pentagon}: robot~1 at the cluster centroid and
robots~2 through~6 on a ring of radius $\rho$ at $72^\circ$ spacing.
In the variables above this is $L_2 = \rho$ for the center-to-ring
pair and $L_3 = L_4 = 2\rho\sin 36^\circ$ for the two ring-to-ring
pairs, with the remaining shape variables set by the ring geometry.
