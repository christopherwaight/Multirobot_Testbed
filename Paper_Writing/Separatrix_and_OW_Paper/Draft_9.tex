...
being informative. A rotating-frame trial reverses the ranking: the
...
corridor for nearly the full record, separating only near landfall.
Together these results yield an operational rule for selecting between
...
\IEEEPARstart{M}{ultirobot} systems offer redundancy, increased
...
Navigating an unknown field is one such behavior. In adaptive
navigation (AN), a vehicle modifies its direction and path in real time
from measurements taken while moving \cite{2}. Rather than covering a region with a prescribed survey trajectory, a
cluster collects spatially distributed readings at one instant, fits a
local model of the landscape, and steers on its features. The
sensing argument favors the cluster. A single vehicle must translate to sense a
...
spatial resolution and noise suppression against breadth of exploration
\cite{4}.
...
center recovering both in closed form \cite{4}. McDonald, Kitts, and
Neumann developed and experimentally verified the ridge, trench, and
saddle primitives \cite{2,3,6}.

A vector field carries its own catalog of features. Critical points are
the isolated members, sinks and sources that mark convergence zones and
subsurface leaks \cite{7}, and in \cite{8} a three-robot cluster
estimated the location and type of one from a linear fit of
simultaneous local velocity measurements. The scalar primitives above
read a single signal channel. A coupled two-component field doubles the
estimation problem to twelve unknowns.

The extended features of a coastal flow govern how material moves
through it. Floating material does not spread evenly, but collects along or separates across
...
coastal sensor array is best deployed \cite{9,10,11}. Its saddle points
are the stagnation points where two barriers cross, deciding the
corridor a drifter follows. An attracting objective Eulerian
coherent structure (OECS) is an accumulator, the curve on which debris,
larvae, and search objects collect \cite{12}. Coordinated multi-vehicle sampling of dynamic ocean features is an
active area of marine robotics \cite{13,14,15}. Knowing where these curves lie is the
...
dependence \cite{9}. It is computed as a ridge of the finite-time
Lyapunov exponent field \cite{11}. Objectivity, the Lagrangian/Eulerian tradeoff, sparse-trajectory gradient recovery, and the strain-spin decomposition are adjacent active directions \cite{16,17,18,19,20}. The
...
censuses \cite{25}. Both routes characterize the structure offline from a reconstructed
field, depend on an integration horizon that delays estimates, and
assume the structure's identity before tracking begins.
...
the structure near a saddle in both flow-tank and ocean trials. Kularatne
and Hsieh instead compute the local FTLE field on board, recovering
type at the cost of a nonzero integration horizon \cite{30}. In none of
it does a formation acquire an extended structure it does not already
occupy.
...
synchronous sample; b) two AN primitives, the $D$ tracker and the $s_1$
tracker, that read the two terms of one splitting identity, $D =
\omega^2/4 - s_1^2$, off the same fit; c) frame-equivariant commands
for the $s_1$ tracker, with path divergence bounded under a stated rate
condition; and d) a tradeoff between objectivity and noise tolerance
located in the estimator rather than either control law. That tradeoff
yields a rule for selecting between the primitives by observing
platform and mission.
For the estimator, six robots are proved minimal and the failing
formations are exactly those lying on a common conic. Its noise gains
are exact and isotropic in formation heading, one scaling rule per
derivative order, though the truncation bias is not isotropic. Both
...
The critical-point estimator of \cite{8} fits an affine model of each
component and solves for the point where $\mathbf{v} = \mathbf{0}$. The
surrogates tracked here are built not from $\mathbf{v}$ but from its
spatial derivatives through second order, which a planar fit, with
identically zero second derivatives, cannot supply.

The cluster works from the second-order Taylor expansion about the
centroid $\mathbf{p}_c$. Coordinates $\mathbf{p} = (x, y)$ are measured
from the centroid throughout this section, so $\mathbf{p} = \mathbf{0}$.
\begin{align}
...
\end{align}
The twelve coefficients are constants. Each coefficient is a derivative of the field at
the centroid, the one-half factors making the match direct,
...
discarded, and marks estimates with a hat.
...
and $v_i$ being the field components sampled by robot $i$.  If the field is exactly quadratic
...
$\boldsymbol{\Phi}$. Neither holds exactly in practice.
...
Any method returning a single answer from five supplies a structural
prior, and this estimator declines the obvious one: horizontal surface
...
Six robots suffice exactly when $\boldsymbol{\Phi}$ is nonsingular.
Nonsingularity is poisedness of the six nodes for bivariate quadratic interpolation, which fails exactly when they lie on a common conic. A conic through all six nodes is a nonzero quadratic vanishing at every sample, so it lies in the null space of $\boldsymbol{\Phi}$. A ring
alone is degenerate at any size, being a conic by construction. The
pentagon-plus-center formation used throughout is not, and it is the
configuration \cite{4} derived for scalar Hessian estimation.

\subsection{Estimator Sensitivity}
\label{sec:sensitivity}

Everything the estimator does is fixed by the formation matrix
$\boldsymbol{\Phi}$ of (\ref{eq:solve}), built from the robot positions
alone, not the field, the noise, or the mission. Rank decides whether the fit has an answer, conditioning its cost. Condition numbers are
reported on
the radius-normalized $\boldsymbol{\Phi}$, its columns scaled by
$\rho^{\,q}$ for a basis element of order $q$, since the raw monomial
basis carries a $\rho^{-2}$ column scaling that would report a healthy
formation as a degenerate one. The nominal shape gives
$\kappa(\boldsymbol{\Phi}) = 8.26$ at every radius.

Conic drift degrades the estimate continuously, with no threshold.
Moving the center robot out to $0.99\rho$ puts all
six robots within one percent of a common circle, the degeneracy of
Section~\ref{sec:minimality}, and raises $\kappa(\boldsymbol{\Phi})$ to
$564$. The inflation is not uniform, and the pattern matters more than the number: the pure second-order gains rise by about $56$ and the
constant term by $70$, while the first-order and mixed second-order
gains are untouched to three digits. Conic drift attacks the curvature
channels the $D$ tracker reads and leaves the first-order channels the
$s_1$ tracker rides alone. Formation \emph{shape} error enters the
conditioning only here; position error takes the separate route below.

The fit is linear in the measurements, so the noise gains are the row
norms of $\boldsymbol{\Phi}^{-1}$, geometry again. A coefficient of
derivative order $q$ has standard deviation $\gamma_q
\sigma_{\mathrm{eff}} / \rho^{\,q}$, so curvature pays $\rho^{-2}$
where the gradient pays $\rho^{-1}$. Table~\ref{tab:gain_ladder} gives
the ladder for the pentagon-plus-center formation. The constants are
formation specific and the exponent $-q$ is not, and within second
order the pure and mixed coefficients carry different constants, a
factor of two apart. One scaling rule holds per derivative order, not
one gain per order.

\begin{table}[!t]
\caption{Noise gains for the pentagon-plus-center formation. The
constants $\gamma$ are exact and independent of ring radius; the radius
enters only through $\rho^{-q}$.}
\label{tab:gain_ladder}
\centering
\begin{tabular}{llll}
\hline
Coefficient & Order $q$ & $\gamma$ & Std.\ dev. \\
\hline
$a_1$ & 0 & $1$ & $\sigma_{\mathrm{eff}}$ \\
$a_2$, $a_3$ & 1 & $2/\sqrt{10} \approx 0.632$ &
    $\gamma\,\sigma_{\mathrm{eff}}/\rho$ \\
$a_4$ (mixed) & 2 & $4/\sqrt{10} \approx 1.265$ &
    $\gamma\,\sigma_{\mathrm{eff}}/\rho^2$ \\
$a_5$, $a_6$ (pure) & 2 & $8/\sqrt{10} \approx 2.530$ &
    $\gamma\,\sigma_{\mathrm{eff}}/\rho^2$ \\
\hline
\end{tabular}
\end{table}
...
The channels differ in what they bias, not only in what they cost.
Under measurement noise each determinant term pairs one $\hat{a}$ with
one $\hat{b}$ from independent components, so the cross terms vanish in
expectation and $\hat{D}$, $\nabla\hat{D}$, and $\hat{\mathbf{H}}_D$
are unbiased at all noise levels. The correlation above breaks that
independence, leaving $E[\hat{a}_2\hat{b}_3] = a_2 b_3 +
\operatorname{cov}(\hat{a}_2, \hat{b}_3)$ and a determinant bias that
scales with the local Jacobian. Unbiasedness belongs to the measurement
channel alone.

The strain eigenvalue is biased under either channel, subtracting a
convex norm. For a noisy two-vector $E\lVert(\hat{s}_n,
\hat{s}_s)\rVert > \lVert(s_n, s_s)\rVert$, giving
\begin{equation}
    E[\hat{s}_1] - s_1 \approx -\frac{\sigma_r^2}{2r}
    \label{eq:s1_bias}
\end{equation}
at moderate signal-to-noise, with $\sigma_r$ the strain-component
standard deviation from the ladder. So $\hat{s}_1$ reads a trench
deeper than it is, worst where the strain degenerates. That same $1/r$
sets the resolution floor and returns in Section~\ref{sec:disc_noise}
as the $s_1$ tracker's exposure.

One error does not fall with noise. Fitting a quadratic to a field with
nonzero third derivatives leaves the truncation residual
$e_{\mathbf{H}} = \mathcal{O}(\lVert\mathbf{p}\rVert^3)$, which reaches
$\hat{\mathbf{H}}_D$ at order $\rho$ after the $\rho^{-2}$ gain. It
floors the curvature estimate at every noise level, and it is why
radius cannot be chosen for noise alone: shrinking $\rho$ suppresses
$e_{\mathbf{H}}$ and amplifies noise.

The formation is a spatial filter whose passband the operator sets
before deployment. Radius fixes the scale at which the field is
read, robot count buys precision at the orders the geometry already
constrains, and radial diversity buys the higher orders outright.
Robots added to a single ring drive the first-order gain down as $\sqrt{2/N}$ but leave the second-order gain near a floor of $2$, since $x^2 + y^2 = \rho^2$ identically on a ring and the quadratic basis is rank deficient there for any $N$. The ring-plus-center configurations that avoid this are design guidance, not exercised here. Each is a
specifiable mission attribute selected against a stated trade, not a
gain tuned once the cluster is in the water.
...
coefficients in closed form, without numerical differencing. Each Jacobian entry is affine in position, so $\hat{D}(\mathbf{p})$ is exactly quadratic over the footprint. Its gradient and Hessian at the centroid are polynomials in the coefficients, the Hessian being
\begin{equation}
    \hat{\mathbf{H}}_{D,0} =
    \begin{bmatrix}
...
does not sharpen as the cluster contracts. The strain side reads from the same fit through the mean, deviatoric
normal, and shear strain, $\mu =
...
derivatives, outside a quadratic fit's span. Both rule out a Newton step. The gradient is unaffected, its transverse
...
The two primitives share the estimation step, the saturation idiom, and
the control architecture, and differ in which term of the splitting
identity each reads. The $D$ tracker reads
...
Both run inside the layered architecture of prior cluster space studies
\cite{8,31}, shown in Fig.~\ref{fig:control_architecture} from the
bottom up. The robot control layer consumes individual robot velocity
commands and emits actuation. The cluster space layer turns a
cluster-level command into those robot velocities through the inverse
kinematic Jacobian, and maps measured positions back into cluster
variables for feedback \cite{31,32}. The adaptive
...
mode. It sequences acquisition, traversal, and terminal capture. In the
$s_1$ tracker it also applies the depth latch of
(\ref{eq:capture_test}), which keeps a noise-driven flicker from
recycling a mode; the $D$ tracker's band test carries no such latch,
and Section~\ref{sec:disc_limits} reports what that costs. Each layer
presents a fixed interface upward, so swapping trackers changes only
the adaptive navigation layer, the one that reads the field.
...
floor. The Newton and gradient terms are displacements, so each carries an implicit per-step time unit absorbed into $k$. That gain therefore has units of inverse time, the step being $600$~s at the ocean operating point. Table~\ref{tab:params}
...
along it, through the crest separating one saddle from the next. Both
motions come from the local quadratic surface the estimator gives.

A trench is taken here in the height-ridge sense: a curve on which the
gradient is orthogonal to a Hessian eigenvector whose eigenvalue is
positive, so the surface is convex across the curve \cite{36}. The
...
The law below assumes a trench convex across and concave along,
...
point at once. It is a displacement, so commanding it as a velocity
closes the whole remaining distance in one unit of time and diverges as
$\hat{\mathbf{H}}_{D,0}$ approaches singularity. The stationary point it
seeks is also the crest, not a point on the separatrix. The first
objection is met with the hyperbolic saturation
...
one it vanishes, so the flow supplies the speed instead. The test is a shallow-determinant band $\mathcal{B}$, entered when
\begin{equation}
    ...
    \frac{|\hat{D}_0|}{\|\hat{\mathbf{H}}_{D,0}\|_F}
        < \varepsilon_{\dim},
\end{equation}
a raw threshold on absolute well depth, or a curvature-normalized one measuring depth against the local curvature. The latter carries units of length squared, so the band scales with the landscape rather than the absolute size of $D$. Note that $\mathcal{B}$ is a band around
$\{D = 0\}$, not around the separatrix: on the benchmark the separatrix
carries $D = -\pi^4 A^2$ along almost all of its length and enters the
band only at the crest, the case the branch is written for. The set
also contains the Okubo-Weiss boundary, which the cluster can cross
during acquisition, but firing there is harmless, since the
along-trench speed hands off to a flow that off-structure advects the
cluster while the transverse channel keeps acting. The
...
This law decouples travel from trench holding through the eigenbasis.
...
\begin{equation}
    ...
    \label{eq:d_capture}
\end{equation}
when the fitted surface stops presenting a trench frame at all. A saddle of $D$ where two trench segments cross is such a point, the terminal analogue of the vanishing-gradient test the $s_1$ tracker uses. It is weaker, since a quadratic fit admits no definiteness certificate for either surrogate (Section~\ref{sec:surrogates}). Section~\ref{sec:disc_clean} reports
what that costs on the benchmark, where the test cannot fire.

Convergence follows from the trench geometry. In arclength coordinates $(\ell, n)$ along and across the trench, $D = h(\ell) + \tfrac{1}{2}\kappa_\perp n^2$ with $\kappa_\perp = 2\pi^6 A^2 > 0$ along a separatrix segment of the benchmark. The transverse channel of the $D$ law then reduces to $\dot{n} = -k\,\mathrm{sat}(a_\perp n)$, with $a_\perp = \kappa_\perp/\hat{\kappa}_\perp$ the ratio of true transverse curvature to fitted. Taking $V = \tfrac{1}{2}n^2$
...
cannot reverse. Both branches also vanish at a stagnation point, where
the flow and the along-trench gradient go to zero together, so the
speed is bounded below only away from one. On a segment of length
...
Away from a stationary point $\nabla\hat{s}_1$ therefore points almost entirely along the trench. The tangent is whichever eigenvector the gradient is less orthogonal to.
\begin{equation}
...
This law decouples descent from travel through the projector.
...
threshold is a hysteresis ratio, not a physical relation. The condition arms at four times the first-contact depth and releases when $\hat{s}_1$ rises back above $-s_{\mathrm{trim}}$. A noise-driven flicker of the gradient test therefore does not send the cluster back through the tangent comparison at the well, while a genuine departure still does.
...
at the isotropic point at $y = 0$. The restoring force disappears there, but so does the transverse gradient, so nothing steers the cluster off the structure. The ride carries it across a ball of radius $\epsilon$ in time $2\epsilon/(k\,c_{\max}\tanh 1)$ while $|n|$ grows by at most $O(\epsilon)$, after which exponential convergence resumes. Traversal is immediate, since the tangential channel runs at constant speed with direction seeded once by the measured flow. That direction is guaranteed only under exact estimates, and Section~\ref{sec:disc_noise} reports the inversions that follow when it is seeded from a noisy fit.
...
measured ocean record, each described with its results below.
...
floor drops to $0.002$, having no vessel-scale analogue. No noise level is estimated for this field, since the exactly-determined six-robot fit leaves no residual to estimate $\sigma_{uv}$ from. Quantifying the uncertainty in ocean data and its effect on tracking is, as \cite{26} concludes for this same record, a harder problem left outside its scope, so the noise model here is validated on the double gyre only.
...
held. Coefficient standard deviations matched
Table~\ref{tab:gain_ladder} within 2.1\% and the reading error induced
...
noise level, bottoming at $0.94$ and held there by $e_{\mathbf{H}}$. In Fig.~\ref{fig:est_accuracy}(b),
...
\begin{figure}[!tb]
    \centering
    \includegraphics[width=0.38\textwidth]{figures/traverse_vs_logic_c.png}
...
These runs establish that both primitives work, little more. The transverse derivatives of $D$ and $s_1$ vanish together wherever the vorticity vanishes and the flow is strain-dominated (\ref{eq:trench_coincide}). The double gyre's entire trench network is such a set, its vorticity identically zero on the separatrix and on all four walls, excepting only the isotropic point at the origin.

Both ran noise-free from six matched starts
(Fig.~\ref{fig:traverse_vs_logic_c}), spanning both gyre halves, the origin, and points up to $2.7
ho$ off the structure.
Both acquire the network from all six, in 0 to 35 steps for the $D$ tracker and 0 to 46 for the $s_1$ tracker. Both then ride the same segments in the same direction and cross the isotropic point at the origin. A run
...
model class, not tuning. The $s_1$ tracker
...
$\pm|\hat{H}_{xx}|$: at the cluster's offset from the well the estimated
pair is $[-0.51, 0.45]$ against a true $[18.3, 19.2]$, the induced trace
being a finite-radius truncation artifact. The same underestimate also
...
true $18.3$ inflates it by a factor of order thirty. The primitive
crosses the stagnation region rather than stalling in it for the same
reason it cannot certify a capture.

The limit binds in both directions, since $\hat{\mathbf{H}}_{s_1}$
is negative semidefinite for every field and formation
(Section~\ref{sec:surrogates}), so neither surrogate admits a definiteness
certificate. The one terminal test a quadratic fit supports is a vanishing gradient, and it succeeds for a different reason than the fitted curvature suggests. The fitted \emph{gradient} tracks the true gradient even where the fitted \emph{curvature} carries the wrong sign, and a flow saddle is a genuine two-dimensional minimum of the true $s_1$. The test is sufficient exactly where the $s_1$ tracker needs one
and unavailable where the $D$ tracker would need one. Traversal and
capture are the same representability limit read twice.
...
A spot check on the time-varying member of the same family \cite{11}, its separatrix oscillating at $\varepsilon = 0.1$, holds the $D$ tracker on the moving trench to a mean transverse offset of 0.006. The primitive carries no time-derivative term, and no analytic guarantee covers that case.
...
being informative (Section~\ref{sec:results_est}). The gradient is not the only input exhausted by then. Fig.~\ref{fig:est_accuracy}(b) puts the curvature eigenframe $39^\circ$ from truth at the cliff, against $45^\circ$ for a random axis, and both terms of the $D$ law are built on that frame. Neither threshold belongs to the control law, so the estimator sets the
noise ceiling and the failure decays toward chance.
...
estimator accuracy. On the separatrix $s_s \equiv 0$ and $\mu \equiv 0$, so the gradient collapses to $\nabla\hat{s}_1 = \pm(\hat{a}_5, \hat{a}_4)$ and tangent selection reduces to the comparison $|\hat{a}_5| > |\hat{a}_4|$. The true $a_5 = u_{xx}$ is identically zero there and $a_4$ collapses at both saddles, so the comparison decides at a signal-to-noise ratio of $1.4$ at $\sigma_{uv} = 0.002$, against the seed's $71$. Supplying one channel at a time confirms it: a clean flow to the seed
moves the correct-saddle rate at $\sigma_{uv} = 0.002$ from 44.6\% to
45.8\%, a clean gradient and eigenframe to the argmax to 100\%. The cluster rides the separatrix competently and settles in the wrong
direction, a sign inversion rather than a decay. Holding the tangent whenever the argmax margin falls below the $\gamma_2\sigma_{\mathrm{eff}}/\rho^2$ the gain ladder predicts refuses the decision rather than taking it at chance. That derived threshold raises far-saddle success from 44.6\% to 84.1\% at that level.

Sweeping the formation radius closes the design trade of
Section~\ref{sec:sensitivity} in the loop, over $\rho \in \{0.0375,
0.075, 0.105, 0.15\}$ at 400 trials per cell. The zero-noise tracking
error grows as $\rho^{0.98}$ for the $D$ tracker and $\rho^{0.94}$ for
the $s_1$ tracker, against the $\mathcal{O}(\rho)$ the truncation floor
predicts. The noise level at 50\% success grows as $\rho^{2.02}$ and
$\rho^{1.95}$, against the $\rho^2$ of the second-order rung. Both halves of the ladder hold in closed loop, and the exponents identify the binding channel. A first-order-limited law would tolerate noise as $\rho^1$, so both primitives are limited by their second-order channels, even though the $s_1$ tracker rides a first-order tangent. Radius buys robustness and costs accuracy, with no interior optimum
over this range.

Two readings generalize past the sweep. The zero-noise tracking errors, 0.0075 for the $s_1$ tracker against 0.0016 for the $D$ tracker, have a separate cause: both primitives park where their own transverse command vanishes, where the \emph{fitted} trench lies and not the field's, the gap being the orientation-dependent truncation bias. The second is that robustness is set less by the per-cycle accuracy of what a law reads than by how far a decision is carried before it is checked against the world. The $D$ tracker re-signs $\mathbf{w}_1$ against $\hat{\mathbf{v}}_0$ every cycle; the $s_1$ tracker carries the sign in state through $\mathbf{t}_{\text{ref}}$. That reliance is forced rather than chosen,
...
\begin{figure}[!tb]
    \centering
    \includegraphics[width=0.76\columnwidth]{figures/objectivity_traverser.png}
...
The added solid-body vorticity moves the whole $D'$ landscape, and the same rotation contaminates the flow measurement the on-band branch of the along-trench speed reads. Both terms of the $D$ law are therefore steered by a frame artifact, not by the field. No gain recovers this: the artifact is in the surrogate, and
against $D$ itself it is unbounded at the crest the primitive must
cross. The $s_1$ tracker inherits the protection of
...
channels carry constants. Everything derived from $\hat{D}$ carries the additive observer term, and $\hat{\mathbf{H}}_D$ the truncation floor $e_{\mathbf{H}}$ as well. The strain eigenvalue carries neither, and is exposed instead at $1/r$, a property of the strain field rather than of the sensing. Objectivity
...
second-order coefficients and is not better conditioned, where
...
longer vanishes. That departure, $2|\mu|/r$, normalizes the divergence against the deviatoric strain. It has median $0.8$--$1.1$ and 90th percentile $3.0$--$5.9$ along the two primitives' paths, so the $s_2 = -s_1$ substitution is not tightly bounded here and the splitting-identity reading of their agreement is qualified accordingly.
Both were run unmodified from a shared start north of the channel
entrance, $(34.41^{\circ}\mathrm{N}, -120.39^{\circ}\mathrm{W})$, at
the operating point of Table~\ref{tab:params}. The $s_1$ tracker's
depth condition latched at the first step and stayed latched for the
entire 168-step run, so the record reports the ride, not gradient descent on an unlatched field. Both spend the record near the edge
...
95th-percentile ridge point is 1.8~km for the $D$ tracker and 2.4~km
for the $s_1$ tracker, against a 2~km grid and a 5.4~km formation
radius, with that ridge available to neither. Forward
FTLE recomputed at four anchor times confirms that the dominant ridge
persists while the finer structure downstream evolves, and the $D$
tracker's branch outcome agrees across 84 of 100 jittered starts.
...
The framework's scope should be stated plainly. Cluster
...
regime the six-robot estimator occupies by construction. Computation is
not the binding constraint: one $6 \times 6$ solve per cycle against a
$600$~s ocean step.
...
common conic. No field tested here drives the formation toward one, so the estimator
is untested in the regime its own conditioning limit describes, and the
mitigation, monitoring $\kappa(\boldsymbol{\Phi})$ with a
shape-maintenance term, is not implemented.
Second, the band test carries no hysteresis, so noise can chatter the
$D$ tracker across its boundary. This costs speed alone, since both branches share the same transverse
dynamics.
Third, the strain eigenframe is fragile where $\mathbf{S}$ approaches
isotropy, the $1/r$ exposure of (\ref{eq:s1_bias}). Tangent selection is the defense once the ride is underway, and the
residual risk of a wrong branch at a crossing under heavy noise is
damped but not excluded by the capture test latch.

Fourth is the $s_1$ tracker's tangent rule. Its exposure
is the eigenvector comparison, so remedies aimed at
the seed do not reach it. The margin-hold rule of Section~\ref{sec:disc_noise} recovers most of the loss without removing the exposure. Two further changes remain unimplemented: re-running the sign decision against the flow whenever the selected axis changes, and resolving the sign against a world axis every step where heading is trusted.

Fifth, the $D$ tracker's traversal past a saddle is untested off the
benchmark, and on it runs outside the premise of
...
leave (\ref{eq:hess_det}) traceless. The fitted frame stays indefinite by a symmetry of the field, not a
property of the estimator.

No controlled head-to-head against the straddle family \cite{26,27,28} or the onboard-FTLE strategy of \cite{30} was run. Running those methods from an off-structure start is outside their stated problems, and their robots are advected while this paper's are not, so one simulator cannot host both unaltered.
...
both frames, final gap 0.025. On Santa Barbara Channel currents both followed the same transport corridor for 28 hours, within 1.8 and 2.4~km of an FTLE ridge neither could see. That is the bound this sensing carries: instantaneous criteria resolve the corridor reliably, the trajectory within it unreliably. The operator's choice
therefore depends on the observing platform, not the flow. A cluster
that can certify its orientation should carry the $D$ tracker, which
tolerates a noisier sensor; one that cannot should carry the $s_1$
tracker, which alone returns the same material curve in any frame.

The primitives are admittedly simple, reactive, and minimal in robot
count; even so, they demonstrate the full suite on a measured record.
...
bias; c) verifying both primitives on the Decabot testbed and then with
surface vessels; d) climbing the sensing ladder that relates a feature
class to the cluster that can resolve it; and e) broadening the catalog
to transient attracting profiles and eddy cores. That ladder runs three
robots for a critical point \cite{8} and six for these structures, ten
the next rung.
...
\bibitem{35} NOAA Nat. Centers Environ. Inf., ``Near-real-time surface
ocean velocities derived from HF-radar stations,'' NCEI Accession
gov.noaa.nodc:IOOS-HFRadarRTVector.

\bibitem{36} D. Eberly, R. Gardner, B. Morse, S. Pizer, and C.
