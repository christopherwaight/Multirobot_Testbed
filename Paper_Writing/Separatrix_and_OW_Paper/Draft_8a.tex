...
fails only when they lie on a common conic \cite{4}. A ring alone is
degenerate at any size; the pentagon-plus-center formation used
throughout is not, and it is the configuration \cite{6} derived for
...
\det(\mathbf{J}(\mathbf{p}))$ throughout. Both surrogates this paper
tracks are read off $\mathbf{J}$, the first from its determinant and the
...
Both surrogates and their derivatives follow from the fitted
coefficients in closed form, with no numerical differencing at any
stage. The estimated Jacobian at the centroid is assembled from the
linear coefficients,
\begin{equation}
    \hat{\mathbf{J}}_0 =
    \begin{bmatrix}
