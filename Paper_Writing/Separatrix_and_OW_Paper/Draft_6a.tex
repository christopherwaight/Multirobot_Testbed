...
instantaneous coherent structures, which within hours divide it into
rotation-dominated cores that retain drifting material and
strain-dominated boundaries across which it separates.
Existing robotic trackers either commit to a presumed
structure in advance or require time-horizon integration before an
estimate exists. This paper presents a real-time multirobot method for
finding and following these structures using only instantaneous local
flow velocity measurements. A cooperative estimator recovers the local
quadratic model of the field from one synchronous measurement set, with
six robots proved minimal for the unconstrained quadratic model.
The fit supplies two trackable surrogates: the Jacobian determinant and
the smaller rate-of-strain eigenvalue. The determinant traces the flow's
separatrices but shifts under a rotating observer, creating a liability
for teams lacking a global heading reference. The strain eigenvalue is
mathematically objective, recovering the identical material curve in any
observer frame.
We develop adaptive navigation controllers for both surrogates and
demonstrate a fundamental tradeoff between tracking objectivity and
noise tolerance. In a rotating-frame experiment, the strain tracker
maintains path invariance, while the determinant tracker completely
diverges from the structure. However, Monte Carlo sweeps reveal that the
determinant tracker withstands significantly higher measurement noise,
failing at the noise level where the estimator's gradient stops being
informative; the strain tracker fails at roughly one fifth that noise
level. Finally, deployed on Santa Barbara Channel radar currents, both
controllers follow the same dominant ridge for 28 hours.
\end{abstract}
...
the unconstrained quadratic model and with exact, heading-isotropic
noise gains. Second, the $D$ tracker, a hybrid controller on the
Jacobian determinant of the field, $D$, a Galilean-invariant surrogate for the
attracting and repelling manifold without committing to a
manifold identity in advance. Third, the $s_1$ tracker, built from the
strain eigenframe on the smaller rate-of-strain eigenvalue $s_1$, which
unlike $D$ is objective and so stays valid when a team cannot certify
its own orientation against a trusted external reference, as when a
vehicle dead-reckons on a drifting compass heading between GPS fixes or
operates without GPS at all \cite{10,22}. Fourth, validation of both trackers under a
fully specified noise model, in a closed-loop rotating-frame
objectivity experiment, and on real Santa Barbara Channel radar
currents.
...
Its robots start on the manifold and are advected.
...
positions and so stays exact even as the formation deforms.
...
\begin{equation}
    ...
    \label{eq:splitting}
\end{equation}
the middle equality holding generally, and the last specializing to incompressible flow ($s_2 = -s_1$). Three
...
attracting and repelling OECS meet, and certify a terminal
...
measured field of Section~\ref{sec:ocean_sim}. (As we verify in Section~\ref{sec:ocean_sim}, this measured divergence is small enough that the approximation in (\ref{eq:splitting}) holds safely.) A five-robot estimator
that imposed $\operatorname{tr}(\mathbf{J}) = 0$ would bias every
recovered coefficient there, not only the three it eliminates. The sixth
...
fit, so they contribute a floor $e$ that no radius, sensor, or geometry
...
radius governs $\hat{D}$ and $\nabla\hat{D}$, the floor $e$ governs
...
returns $\hat{\mathbf{H}}_{D,0}$ traceless with zero off-diagonal entry. At finite radius, truncation induces a small trace, but the eigenvalues stay closely approximated by $\pm|\hat{H}_{xx}|$ everywhere and the degenerate
...
where the floor $e$ of Section~\ref{sec:sensitivity} vanishes, which on
...
and a trench of length $L_\Gamma$ takes at most $L_\Gamma/m$. On the
$s_1$ law the tangential channel of (\ref{eq:oecs_law}) is
$\beta c_{\max}\tanh(1)\mathbf{t}$ with $\beta = 1$ and
$\lVert\mathbf{t}\rVert = 1$ throughout the ride, so the realized
...
real and positive below $\beta = (1 - \sqrt{\alpha_{\text{mom}}})^2$ and
unstable above $\beta = (1 + \sqrt{\alpha_{\text{mom}}})^2$. The lag the
...
Section~\ref{sec:stability}, at the double-gyre operating point of
Table~\ref{tab:params} ($\Delta t = 0.1$, $k = 3.0$,
...
(1-\alpha_{\text{mom}}) > (1+\sqrt{\alpha_{\text{mom}}})^2$, which at
this operating point is $\Delta t\,k\,a_\perp > 11.3$.}
\label{tab:bandwidth}
\footnotesize
...
$0.300^{\circ}$ of latitude and $0.363^{\circ}$ of longitude, so one world
...
axes carry the same physical scale and one world unit is $L = 51.4$~km in
each. The map is then a similarity, a uniform dilation of the local
tangent plane, and with $\mathbf{J}_{\mathrm{world}} = L
\mathbf{J}_{\mathrm{phys}}$ the determinant scales by $L^2$ and the
symmetric part by $L$, both positive constants. Sampled vector
...
internal metric only) and tracking error (mean of
$|x_c|$). Where failures occur, the sweeps supply noise-free inputs to individual channels to isolate the mechanism. The $s_1$ sweep widens the exit box to
...
noise level, held there by $e$. In Fig.~\ref{fig:est_accuracy}(b),
...
(\ref{eq:grad_s1}) at $(0.03, 0.25)$ recovers a restoring slope of $6.6$
per unit offset against the analytic transverse curvature $6.888$, a
$4.7$\% truncation deficit at the nominal radius. That single number is
not representative, and the spread is the orientation dependence of
...
between $4.0$ and $9.7$ about a mean of $6.83$, and halving $\rho$ halves
...
Differentiating it across the structure gives
\begin{equation}
...
eigenvalues approximate $\pm|\hat{H}_{xx}|$ everywhere on the network, the nonzero trace being a finite-radius truncation artifact. At the well
the estimated pair is $[-0.72, 0.43]$ against a true $[18.3, 19.2]$,
which is the floor $e$ at $|y| = 0.45$, evaluated in closed form in
Section~\ref{sec:disc_noise}. The same limit binds in
...
Okubo-Weiss partition. Over 20\,000 gridded starts no run settles at a
...
separate from the floor $e$ of Section~\ref{sec:sensitivity}, which on
...
signal-to-noise ratio here is $1.4$ at $\sigma_{uv} = 0.002$, fifty times
...
$\sigma_{uv} = 0.002$ from 42.7\% to 45.0\%. Giving the argmax its clean
gradient and eigenframe moves it to 100\%, and to 95.7\% at $\sigma_{uv}
= 0.005$ where the unaided tracker is at zero. The sign is therefore
...
run at 1.1\% of tracking steps at $\sigma_{uv} = 0.001$ and 16.5\% at
...
shifts the 50\% crossing from between 0.0015 and 0.002 to between 0.0025
and 0.003 with nothing else changed. The exposure therefore sits at the
...
trial-mean $|x_c|$ of 0.0019 and 0.0079 against those measurements. It
vanishes at the two ring phases that make the pentagon mirror symmetric
about $x = 0$ and peaks $18^\circ$ away, and the measured error carries
that $36^\circ$ period, running from 0.0006 to 0.0024 for the $D$
...
cycle, so a corrupted frame costs it one cycle. The $s_1$ tracker
...
carries the unmodelled-derivative floor $e$ as well, while $\hat{s}_1$
...
onto a common curve. While the measured field carries a nonzero horizontal divergence, its magnitude in the tracking corridor was empirically small compared to the strain, bounding the $s_2 = -s_1$ substitution error of (\ref{eq:splitting}). Both trackers were run unmodified from a single shared
...
Read together, the four conditions give a selection rule.
...
points drift toward a common conic (Remark~\ref{rem:conic}), for
...
exposure is the eigenvector comparison, not the seed, so the remedies
that address the seed alone do not reach it. Two unimplemented changes
do. Holding the tangent whenever the argmax margin
...
the decision instead of taking it at chance, and re-running the sign
decision against the flow whenever the selected axis changes re-anchors
the sign rather than propagating it. Neither costs objectivity, the first
because it reads only fitted coefficients, the second because it is the
same one-time flow read the law already permits, taken again. A team with
a trusted heading can resolve the sign against a world axis at every step
instead, which removes the propagation chain entirely. None was
implemented, since the finding was made late enough in the noise-sweep
validation that a design change here would need its own noise sweep to
verify.

No controlled head-to-head against the straddle family \cite{17,18,19} or
the onboard-FTLE strategy of \cite{21} was run. Running those methods from an off-structure start is
outside their own stated problems, and their robots are advected (eq.~1a
in \cite{17}, eq.~5a in \cite{21}) while this paper's are not
(Section~\ref{sec:problem_statement}), so one simulator cannot host both
without altering one of them.

All results are from simulation, and hardware validation on the Decabot
testbed is planned as a follow-on study. The no-advection premise of
Section~\ref{sec:problem_statement} is exact rather than approximate for
that testbed, since the robots are not immersed in anything and the field
is encoded and read rather than physically experienced \cite{39}. On the ocean field it is an extrapolation. The
...
tracking error, which no gain reaches. Hardware deployment on the
Decabot testbed would test the no-advection assumption directly.
...
accuracy. On
real Santa Barbara Channel currents both trackers were run unmodified
from a single shared start and followed the same dominant ridge for 28
hours, the $D$ tracker to a landfall reproduced by 84 of 100 jittered
starts and the $s_1$ tracker, reading the objective strain term alone,
still closing on that same target when the record ended. That field
offers no genuine two-dimensional minimum of $s_1$ within reach of the
ride, so the terminal test (\ref{eq:capture_test}) never fired, and the
unconditional law behaved as a traverser. The non-firing is a property
of the field rather than a failure of the controller. The choice between the two trackers is set by the observing
