...
Consider six robots at positions $\mathbf{p}_i$, each measuring the
...
relative $O(\mathrm{Ro})$ (Section~\ref{sec:objectivity_tradeoff}).
...
directly will not work, because which of the two is tangent swaps at the
degeneracy lines. The gradient is used instead. On the trench, the transverse
...
sign. The direction itself does not depend on it.
...
branches return $\tau \geq 0$ with $\mathbf{w}_1$ signed so that
...
$O(\mathrm{Ro})$ term of Section~\ref{sec:sensitivity} into everything its law
reads.
...
\subsection{Double-Gyre Results, $D$ Tracker}
...
\hline
& & (a) $D$ & (b) $s_1$ \\
$\sigma_{uv}$ & $\sigma_p$ & traverse & traverse \\
\hline
0.000 & 0.000 & 100.0 & 100.0 \\
      & 0.005 &  65.7 &   1.3 \\
      & 0.010 &  44.5 &   0.0 \\
      & 0.020 &  33.3 &   0.0 \\
      & 0.050 &  19.0 &   0.0 \\[2pt]
0.001 & 0.000 &  99.6 &  91.8 \\
      & 0.005 &  63.2 &   1.1 \\
      & 0.010 &  45.6 &   0.0 \\
      & 0.020 &  32.9 &   0.0 \\
      & 0.050 &  19.1 &   0.1 \\[2pt]
0.005 & 0.000 &  62.4 &   0.0 \\
      & 0.005 &  52.1 &   0.0 \\
      & 0.010 &  43.5 &   0.0 \\
      & 0.020 &  31.2 &   0.0 \\
      & 0.050 &  19.4 &   0.0 \\[2pt]
0.010 & 0.000 &  43.7 &   0.0 \\
      & 0.005 &  40.7 &   0.0 \\
      & 0.010 &  37.0 &   0.0 \\
      & 0.020 &  28.8 &   0.0 \\
      & 0.050 &  18.7 &   0.0 \\[2pt]
0.020 & 0.000 &  25.5 &   0.0 \\
      & 0.005 &  25.7 &   0.0 \\
      & 0.010 &  24.3 &   0.0 \\
      & 0.020 &  22.6 &   0.0 \\
      & 0.050 &  17.0 &   0.1 \\[2pt]
0.050 & 0.000 &  16.0 &   0.2 \\
      & 0.005 &  16.2 &   0.2 \\
      & 0.010 &  15.8 &   0.1 \\
      & 0.020 &  16.1 &   0.1 \\
      & 0.050 &  14.9 &   0.1 \\[2pt]
0.100 & 0.000 &  14.1 &   0.5 \\
      & 0.005 &  14.1 &   0.4 \\
      & 0.010 &  14.3 &   0.4 \\
      & 0.020 &  14.6 &   0.4 \\
      & 0.050 &  13.6 &   0.3 \\[2pt]
0.200 & 0.000 &  13.5 &   0.5 \\
      & 0.005 &  12.4 &   0.4 \\
      & 0.010 &  12.8 &   0.6 \\
      & 0.020 &  12.8 &   0.4 \\
      & 0.050 &  12.4 &   0.4 \\[2pt]
0.400 & 0.000 &  12.1 &   0.5 \\
      & 0.005 &  12.2 &   0.5 \\
      & 0.010 &  12.0 &   0.5 \\
      & 0.020 &  12.3 &   0.6 \\
      & 0.050 &  11.9 &   0.5 \\[2pt]
...
Table~\ref{tab:mc_success}(a) reports traverse success for the $D$
tracker over the noise grid of Section~\ref{sec:test_plan}, scored
against $\mathbf{p}^*_1$. Success holds at 100\% with no noise and
crosses 50\% at $\sigma_{uv} \approx 0.0079$.
Position noise scales the estimator-accuracy curve above by
$\|\mathbf{J}\|$, per (\ref{eq:sigma_eff})
...
\subsection{Double-Gyre Results, $s_1$ Tracker}
...
Table~\ref{tab:bandwidth}. At $\sigma_{uv} =
...
\label{sec:results_objectivity}

The objectivity demonstration follows the rotating-frame protocol of
Section~\ref{sec:test_plan}, which runs both trackers from the same start in two
observer frames. Fig.~\ref{fig:oecs_objectivity} shows the
...
the 28-h budget from this start. The record offers no such core within
reach of the ride, so the reachability condition of Problem 2 does not
engage. The terminal test (\ref{eq:capture_test}) never fires, and the
$s_1$ tracker's mean $\hat{s}_1$ along the ride is the tracked value
rather than a certified settle.

\section{Discussion}
\label{sec:discussion}

Each of the four experiment families of Section~\ref{sec:test_plan} puts the two
...
On the noise-free double gyre the two trackers behave the same until the
terminal step. From
...
general. Section~\ref{sec:disc_ocean} removes the coincidence.
...
on $\hat{\mathbf{H}}_D$ (Section~\ref{sec:sensitivity}). Near the well the
...
decays cleanly with no such floor. Acquisition is also not the
...
column falls only from 62.4\% to 43.7\%. Runs on either side of the
transition settle at a saddle with comparable reliability and differ in
which one (Fig.~\ref{fig:flip_resolution}). The team has not lost the
structure, become erratic, or collapsed. It rides the separatrix
competently and settles, in the wrong direction. This is a sign
inversion, not a decay toward
chance, and it is what makes the far-saddle column the score that
separates the two outcomes. A
...
One prediction of Section~\ref{sec:stability} comes out backwards here, and the
...
(Section~\ref{sec:objectivity_tradeoff}), and it is still the less robust tracker here,
...
trenches coincide for the reason given in Section~\ref{sec:disc_clean}, and the
...
gap of Section~\ref{sec:disc_noise} is that cost, measured.
...
The ocean record removes the coincidence Section~\ref{sec:disc_clean} rests on.
...
and it closes the terminal argument of Section~\ref{sec:disc_clean} from the
...
assumptions of Section~\ref{sec:stability} are the price. Three further
...
Section~\ref{sec:sensitivity}. Tangent selection (\ref{eq:tangent_select}) is
...
limitations, and Section~\ref{sec:disc_noise} gives its mechanism. Three
...
already accepts. That convention also makes the law memoryless. A team
with a trusted heading resolves the sign against the world axis at
every step rather than carrying $\mathbf{t}_{\mathrm{ref}}$ forward
from the first, at no cost in objectivity, since $s_1$ and its
eigenframe are objective whether or not the sign is anchored
externally. A confidence check on the seed converts a wrong
...
two grounds keep the comparison of Section~\ref{sec:related_robots} structural rather
...
estimator. Finally, targets other than critical points, transient attracting
