...
instantaneous coherent structures, where drifting material collects
within hours. Existing robotic trackers either commit to a presumed
...
six robots proven to be the minimum necessary team.
...
failing only at the estimator's theoretical limit; the strain tracker
fails at roughly one fifth that noise level. Finally, deployed on Santa
Barbara Channel radar currents, both controllers successfully track a
coherent ridge for 28 hours to a consistent landfall.
\end{abstract}
...
Jacobian determinant $D$, a Galilean-invariant surrogate for the
transport skeleton read directly from the local velocity gradient,
whose modified Newton step snaps onto a trench of $D$, slides along it,
and traverses the network of saddle crossings without committing to a
...
operates without GPS at all \cite{22,23}. Its design follows from the
structural result that no quadratic velocity fit can certify
$s_1$-trench curvature. Fourth, validation of both trackers under a
...
states, every cell sourced from the cited papers' own text. The
initialization and advection rows are premises of those methods'
guarantees rather than details of their implementations.
...
\cite{34} or obstacle avoidance \cite{35}.
...
\cite{25}. This is the objective counterpart of the $D$ trench. Serra
...
collect onto them within hours \cite{36}.
...
estimator fits the unconstrained model deliberately, and the reason is
physical rather than methodological. Horizontal surface currents are not
...
measured field of Section~\ref{sec:ocean_sim}. A five-robot estimator
...
Incompressibility is assumed elsewhere in this paper, and the division is
worth stating. It is used wherever the surrogates are interpreted, by
(\ref{eq:eig_detj}) for the eigenvalue pair, by the last equality of
(\ref{eq:splitting}), by $Q = -D$ in Section~\ref{sec:separatrix}, and by
$\mu = 0$ below. It is used nowhere in the estimator, which fits and
reports $\mu$ whatever its value. On a field with $\mu \neq 0$ the
recovered coefficients stay exact and the interpretations weaken to
$Q = \mu^2 - D$ and $s_{1,2} = \mu \mp r$.
...
reduces. On the benchmark that floor is available in closed form.
Substituting (\ref{eq:dg_u}) into (\ref{eq:hess_det}) gives, on the
separatrix,
\begin{equation}
...
eigenvalues of $\hat{\mathbf{H}}_D$, not on its eigendirections, which
noise removes separately (Section~\ref{sec:disc_noise}).
...
cluster variables for feedback \cite{37,38}. A robot controller layer
...
Corollary~\ref{cor:pentagon}, its centroid and heading following the
...
how well $\lambda_2$ matches the field's transverse curvature, which
Section~\ref{sec:sensitivity} shows is the one quantity the fit cannot
supply. Section~\ref{sec:stability} therefore proves an exponential rate,
not a superlinear one. Since $v_\parallel \geq
...
$\hat{b}_5 = \hat{b}_6$ in the Taylor limit and (\ref{eq:hess_det})
returns $\hat{\mathbf{H}}_{D,0}$ traceless with zero off-diagonal entry.
Its eigenvalues are $\pm|\hat{H}_{xx}|$ everywhere and the degenerate
...
where $e = 0$ in (\ref{eq:floor_closed}), which on the benchmark is the
crest alone. The $s_1$ tracker has no usable curvature to normalize by
...
\begin{equation}
    ...
    \label{eq:iss_bound}
\end{equation}
for $\delta \ll k\,c_{\max}$, the disturbance divided by the local slope
of the restoring law. Two disturbances enter here and they behave
...
$(34.2^{\circ}\mathrm{N}, -120.4^{\circ}\mathrm{W})$, and one simulated
step represents $600$~s of field time. The $168$-step trial spans the
full record.

The map assigns the same degree scale to both axes, so it is not a
similarity. At $34.2^{\circ}$N a degree of longitude is $92.1$~km against
$111.3$~km of latitude, so one world unit is $51.4$~km north but
$42.5$~km east, and $\rho = 0.105$ spans $5.4$~km north-south against
$4.5$~km east-west. The two surrogates are not equally exposed. Writing
$\mathbf{J}_{\mathrm{world}} = \mathbf{J}_{\mathrm{phys}}
\operatorname{diag}(L_x, L_y)$, the determinant scales by the positive
constant $L_x L_y$, so every zero set, trench, and extremum of $D$ is
preserved exactly, confirmed on the record to $10^{-12}$ relative. The
symmetric part is not, and a purely rotational physical Jacobian acquires
a spurious world strain of $(1 - L_x/L_y)/2 = 0.087$ per unit vorticity.
Over the corridor the trackers use, at four times across a $7 \times 7$
grid of placements, the angle between the world $\mathbf{e}_2$ and the
physical one has median $1.8^{\circ}$ and maximum $25.4^{\circ}$. The
$s_1$ results here are therefore statements about a structure defined in
world coordinates, offset from the physical strain geometry by that
amount. Rescaling one axis of the map is the direct fix, not applied
here.
...
eigenvalues are $\pm|\hat{H}_{xx}|$ everywhere on the network. At the well
the estimated pair is $[-0.72, 0.43]$ against a true $[18.3, 19.2]$,
which is (\ref{eq:floor_closed}) at $|y| = 0.45$. The same limit binds in
...
separate from the floor $e$, which falls on the eigenvalues and leaves
the eigendirections nearly intact. The directions are resolved instead by
...
rate. Neither tracker holds a constant transverse rate along the structure
(Table~\ref{tab:bandwidth}), and neither holds the Newton value. The $D$
tracker normalizes by a curvature the fit does not recover away from the
crest, so its rate is $\csc^2 \pi y_f$ rather than unity and it is the
tracker that reaches the instability threshold first, over the outer
10.4\% of each segment. The $s_1$ tracker has no curvature to normalize
by at all (Remark~\ref{rem:s1hess}), so it takes
$a_\perp = g_\perp\kappa_\perp$ from the field and spends 77.7\% of the
segment past the monotone bound while never reaching the instability
one. The ordering the draft previously drew from these two rows does not
survive as a categorical split, and the five-fold zero-noise
tracking-error gap is not this cost. That gap is set by where each fitted
trench sits relative to the field's, which
Section~\ref{sec:disc_noise} measures and which the Newton normalization
cannot move, since a denominator cannot displace the zero of a numerator.
...
the onboard-FTLE strategy of \cite{21} was run, and the initialization
and advection rows of Table~\ref{tab:assumptions} are why the comparison
stays structural. Running those methods from an off-structure start is
