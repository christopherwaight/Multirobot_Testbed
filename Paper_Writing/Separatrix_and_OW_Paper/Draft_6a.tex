...
structure or pay an integration latency. This paper instead uses only
instantaneous local flow measurements. A cooperative estimator recovers
the local quadratic field model from one synchronous sample, with six
robots proved minimal. The fit supplies two surrogates: the Jacobian
...
and recovers the identical material curve in any frame. We develop
navigation controllers for both and find a tradeoff between objectivity
and noise tolerance. In a rotating-frame experiment the strain tracker
maintains path invariance while the determinant tracker diverges; Monte
Carlo sweeps show the determinant tracker withstands about four to five
...
currents, both controllers follow the same dominant ridge for nearly
the full record, separating only near landfall. Four experiment
families give a rule for choosing between the two surrogates by field
and mission.
\end{abstract}
...
Some environments are better modeled by vector fields, which assign
both a magnitude and a direction to each point. The critical points of
such a field often mark the features a mission cares about. In ocean
currents and other geophysical flows, a sink can flag an area of high
probability for search and rescue \cite{7}, and a spewing vortex can
point to a below-surface leak \cite{8}.
...
rotating-frame experiment of this paper, write
...
measured ocean field used in this paper. A five-robot estimator
...
determinant-derived quantity frame dependent on top of all of it.

\section{Adaptive Navigation Controllers}
\label{sec:controllers}

This section derives the two controllers. They share the estimation
step, the saturation idiom, and the three-layer architecture.
...
non-dimensional. Converting the ocean column through the unit block of
Section~\ref{sec:ocean_sim}, $c_{\max} = 0.04$ is $0.34$~m/s of
per-channel saturation and, with $k = 1.8$ and the two channels
orthogonal, the bound on commanded centroid speed is
$\sqrt{2}\,k\,c_{\max} = 0.87$~m/s, about $1.7$~knots. That cap is not
slack. The $D$ tracker's realized centroid speed touches it exactly
($0.87$~m/s peak against a $0.71$~m/s mean) and the $s_1$ tracker holds
$0.77$~m/s, $89\%$ of it, for the whole record
(Section~\ref{sec:disc_ocean}). The double-gyre
...
class for the $D$ law over that outer half. The fitted law's traversal
there is carried instead by the finite-radius
...
\end{equation}
which is the version a team can check without resolving the tangent
first.
\end{proposition}
...
of $\Omega \approx 0.238$. The gap between the two is geometric rather
than numerical. The sharp margin depends on where the rotation center
sits relative to the seed, growing as the seed's position vector aligns
with $\mathbf{t}_{\text{raw}}$, as it nearly does here, and collapsing
onto (\ref{eq:seed_cond_suff}) when the two are orthogonal.
\end{IEEEproof}
...
of travel or left free; the reported ring-phase-dependent quantities
are with respect to this fixed initial
...
\cite{17} was run, since initialization and advection differ. Frames are interpolated linearly in time
...
Section~\ref{sec:stability} is the tighter $\Delta t\,k\,a_\perp < 2$,
which at $\Delta t = 0.1$ and $k = 1.8$ admits transverse rates up to
$a_\perp = 11.1$ in these units. The FTLE fields used for evaluation are computed from
...
rate-of-strain eigenframe are therefore the physical ones. The tangent plane uses $111.32$~km per degree of
...
makes landfall on the middle island, at realized mean speeds of
$0.71$~m/s for the $D$ tracker and $0.77$~m/s for the $s_1$ tracker
against the $0.87$~m/s cap, so the cap is close to binding throughout
and the $D$ tracker reaches it. No noise level is estimated
...
$|x_c|$). Where the $s_1$ tracker fails, the sweeps supply noise-free inputs to individual channels to isolate the mechanism; the $D$ tracker's floor has a different, already-diagnosed cause (an unbiased random walk) that channel ablation does not target. The $s_1$ sweep widens the exit box to
...
$D$, where its effect is unbounded. The rate keeps the
...
run. It commits the whole ride to the opposite branch. Robustness is set
...
what the record reports. Its tangential channel holds the pinned
constant $k\,c_{\max}\tanh(1) = 0.470$~m/s identically, consistent with
$\beta = 1$ throughout, and the transverse channel runs saturated at
$k\,c_{\max} = 0.617$~m/s on 98.8\% of steps, the two being orthogonal
by construction in (\ref{eq:oecs_law}). The realized centroid speed is
their resultant, 0.772~m/s mean and 0.775~m/s peak, or 89\% of the
$\sqrt{2}\,k\,c_{\max}$ cap, so this run spends the record near the
edge of its authority rather than inside it. Over the 28~h trial both descend through
...
Four extensions remain. A time-derivative correction term would extend
...
$\boldsymbol{\Phi}$; rotating the ring within a run would average the
orientation-dependent truncation bias that sets the zero-noise tracking
error. Extension to three dimensions raises the estimation stakes to
ten coefficients per component, so ten robots or a reduced-order
estimator. Targets other than critical points, transient attracting
profiles and eddy cores among them, are a natural next object for the
same estimator.
...
This paper presented a cooperative estimator and two navigation
controllers built on it. A six-robot cluster recovers the local
quadratic model of a planar flow from a single synchronous sample.
Six robots is the minimum for the unconstrained quadratic model
(Proposition~\ref{prop:minimality}). The fit supplies both terms of the
splitting identity $D = \omega^2/4 - s_1^2$, with noise gains verified
to within $2.1\%$.

Two controllers ride the same trench network from that fit. The $D$
tracker uses a modified Newton step, backed by a Lyapunov certificate
and a traversal theorem. The $s_1$ tracker uses the strain term alone,
with a terminal test built only from the objective gradient. In a
rotating-frame experiment the $s_1$ tracker followed the same material
curve in both observer frames, with a final gap of 0.025; the $D$
tracker instead followed a frame artifact, with a gap of 1.219.

The estimator, not the control law, sets the $D$ tracker's noise
ceiling: its failure cliff falls within $5\%$ of the point where the
estimator's gradient stops being reliable. The $s_1$ tracker's ceiling
is set earlier, and by its law rather than by its estimator. It is set
by the eigenvector comparison that selects the tangent axis, which
weighs a coefficient whose true value is zero on the structure against
one that vanishes where the ride ends. The flow-seeded sign survives
that comparison intact and is laundered through it.

On real Santa Barbara Channel currents, both trackers followed the same
dominant ridge for 28 hours. The $D$ tracker reached a landfall
reproduced by 84 of 100 jittered starts; the $s_1$ tracker was still
closing on the same target when the record ended.

The choice between the two trackers depends on the observing platform
rather than on the flow. A team that can certify its orientation
against a trusted reference should use the $D$ tracker, which tolerates
a noisier sensor. A team that cannot should use the $s_1$ tracker,
which alone returns the same material curve regardless of frame. Both
controllers read the same fit, so a team that carries the estimator
carries both trackers at no added cost.
