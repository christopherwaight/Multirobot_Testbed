...
separates. Existing robotic trackers either commit to a presumed
structure or pay an integration latency. This paper presents a
real-time multirobot method using only instantaneous local flow
measurements. A cooperative estimator recovers the local quadratic
field model from one synchronous sample, with six robots proved
minimal. The fit supplies two surrogates: the Jacobian determinant,
which traces separatrices but shifts under a rotating observer, and
the smaller rate-of-strain eigenvalue, which is objective, recovering
the identical material curve in any frame. We develop navigation
controllers for both and demonstrate a tradeoff, in this
implementation, between objectivity and noise tolerance. In a
rotating-frame experiment the strain tracker maintains path invariance
while the determinant tracker diverges; Monte Carlo sweeps show the
determinant tracker withstands about four to five times more
measurement noise, failing where the estimator gradient stops being
informative. Deployed on Santa Barbara Channel radar currents, both
controllers follow the same dominant ridge for nearly the full record,
separating only near landfall. Four experiment families give a rule
for choosing between the two surrogates by field and mission.
\end{abstract}
...
curvature each policy consumes \cite{2,6}.
...
integration latency before an estimate exists. The AN approach of this paper requires neither.

\begin{table}[t]
\centering
\caption{Capability comparison against the closest prior AN separatrix
trackers. Neither prior family is objective; the straddle family reports
failure at a saddle rather than a handling strategy for it.}
\label{tab:capability}
\footnotesize
\begin{tabular}{lccccc}
\hline
 & robots & latency & prior & saddle & objective \\
 & & to estimate & commitment & handling & \\
\hline
Straddle \cite{17,18,19} & 3--$N$ & none & manifold id. & failure & no \\
Onboard FTLE \cite{21} & --- & integration horizon & none & --- & no \\
This paper, $D$ & 6 & none & none & traverses & no \\
This paper, $s_1$ & 6 & none & none & traverses, captures & yes \\
\hline
\end{tabular}
\end{table}
...
certify its own orientation against a trusted external reference, as
when a vehicle dead-reckons on a drifting compass heading between GPS
fixes or operates without GPS at all \cite{10,22}. Third, closed-loop
...
since the material rate is $s_2 = -s_1$ (Section~\ref{sec:strain_field}).
...
\begin{equation}
    ...
    \label{eq:splitting}
\end{equation}
the middle equality holding generally, and the last specializing to incompressible flow ($s_2 = -s_1$). Four
...
the same curve: the third consequence's coincidence is a fixed point of
the fourth's derivative condition, not an independent fact.
...
\cite{24}. This is the objective counterpart of the $D$ trench. Serra
et al. extracted these structures from instantaneous surface-current
measurements and confirmed in field trials at sea that floating objects
collect onto them within hours \cite{35}.
...
estimator fits the unconstrained model deliberately. Horizontal surface currents are not
divergence free. The horizontal divergence of the surface layer is the
signature of upwelling and downwelling, and it is nonzero over the
measured field of Section~\ref{sec:ocean_sim}. (Section~\ref{sec:disc_ocean}
measures it directly: the ratio $2|\mu|/r$ along the tracking corridor has
median $0.8$--$1.1$, comparable to the strain rather than small, so the
$s_2 = -s_1$ approximation in (\ref{eq:splitting}) is not tightly bounded
on this field either.) A five-robot estimator
that imposed $\operatorname{tr}(\mathbf{J}) = 0$ would bias every
recovered coefficient there, not only the three it eliminates, more
severely than a small-divergence field would. A divergence-constrained
five-robot variant remains available where incompressibility is
defensible; the sixth
robot buys exactness over every planar quadratic field rather than only
over an incompressible subclass, and neither pre-straddling an assumed
manifold nor accumulating data points is required
...
conditioning limit of Section~\ref{sec:limitations}.
\end{remark}
...
cluster's current position, not a stale model extrapolated forward, so
each step descends a locally valid quadratic even though no single fit
is a valid model of $s_1$ over any larger neighborhood.
\end{remark}
...
Truncation pulls against noise in the choice of radius. With $\nu =
...
Noise therefore sets precision and truncation sets accuracy. The $D$
family is unbiased under measurement noise, so repeated sampling
converges to the truncation-limited value, while $\hat{s}_1$ and its
eigenframe are biased by an amount repeated sampling cannot remove. The
radius governs $\hat{D}$ and $\nabla\hat{D}$, the floor $e_H$ governs
$\hat{\mathbf{H}}_D$ at any radius, proximity to degeneracy governs
$\hat{s}_1$, and (\ref{eq:D_not_objective}) makes every
determinant-derived quantity frame dependent on top of all of it. Three
constants introduced here are cashed out later: the gain ladder's
$\gamma_2$ sets the noise floor behind the $s_1$ tangent-selection
failure of Section~\ref{sec:disc_noise}, the $72^\circ$ orientation
period reappears there as the zero-noise tracking floor, and the floor
$e_H$ reappears in Section~\ref{sec:disc_clean} as what lets the $D$
tracker traverse a saddle its exact estimates would not.
...
cluster space control studies \cite{3,4,9,36}, shown in
Fig.~\ref{fig:control_architecture}. An adaptive navigation layer reads
the field and issues a desired centroid velocity $\dot{\mathbf{p}}_c$,
holding the feature estimator of Section~\ref{sec:estimation}, whichever
tracker's control law is active, and a formation controller. A cluster
controller layer maps that command to individual robot velocities through
the inverse kinematic Jacobian, and maps robot positions back into
cluster variables for feedback \cite{36,37}. A robot controller layer
turns those velocities into actuation. Each layer presents a fixed
interface to the one above, so replacing the $D$ tracker with the $s_1$
tracker changes only the adaptive navigation layer, which is also the
only layer that reads the field.


The cluster is the pentagon-plus-center formation of
Remark~\ref{rem:conic}, its centroid and heading following the
command from above and every other cluster variable regulated to its
nominal shape value. Each robot is an omnidirectional point mass with a
first-order response. Section~\ref{sec:robot_model} gives that response,
the identified $\tau$, and the speed limit and stiction floor.
...
floor. Parameters are fixed across every experiment unless noted:
double gyre, $c_{\max} = 0.04$ ($3.4$~m/s), $k = 3.0$,
...
and $s_{\mathrm{trim}} = 0.3$. The $3.4$~m/s figure for $c_{\max}$ is the
per-channel saturation at the ocean's world scale, not the commanded
cap: with $k = 1.8$, the actual bound on commanded centroid speed is
$\sqrt{2}\,k\,c_{\max} \approx 8.7$~m/s, about 17~knots, an artifact of
the gain choice that never binds at the realized speeds of
Section~\ref{sec:disc_ocean} (under 1~m/s). The double-gyre
...
positions and field readings down to six robot velocities tracked
...
\begin{equation}
    ...
    \label{eq:sat}
\end{equation}
chosen over a hard clip because it is a bounded transit at $c_{\max}$ for
$|c| \gg c_{\max}$ and transparent for $|c| \ll c_{\max}$. One parameter
therefore fixes both the transit speed and the width of the terminal
boundary layer, and no per-eigendirection gains are needed. The second
property is addressed by treating the two eigendirections separately,
keeping the Newton component across the trench and, along it, keeping
only its magnitude and taking the direction from the ambient flow.
...
how well $\lambda_2$ matches the field's transverse curvature. Since $v_\parallel \geq
...
$\hat{b}_5 = \hat{b}_6$ in the Taylor limit ($\rho \to 0$) and (\ref{eq:hess_det})
returns $\hat{\mathbf{H}}_{D,0}$ traceless with zero off-diagonal entry. At finite radius, truncation induces a trace that varies with ring phase
(Section~\ref{sec:sensitivity}: $-0.057$ to $-0.287$ at the nominal
radius), so the eigenvalues approximate $\pm|\hat{H}_{xx}|$ rather than
equal it, and the degenerate
...
channel contracts at gradient rate rather than at Newton rate.
...
The exact-estimate hypothesis and the $D$ law's own degenerate-frame guard
are in tension on this benchmark: the guard requires an indefinite
$\hat{\mathbf{H}}_D$, but the true $\mathbf{H}_D$ is positive definite over
the outer half of each separatrix segment ($|y| > 0.25$), so with exact
estimates the guard would fire there and carry no traversal claim. The
theorem's hypothesis holds for the class of fields on which the relevant
curvature stays indefinite along $\Gamma$; the double gyre is not in that
class for the $D$ law over that outer half. What Section~\ref{sec:disc_clean}
demonstrates there is the fitted law's traversal, carried by the finite-radius
truncation bias of Section~\ref{sec:sensitivity} making $\hat{\mathbf{H}}_D$
traceless and therefore artificially indefinite, not a case where the
theorem's exact-estimate hypothesis is met.
...
both laws. Section~\ref{sec:disc_noise} measures what replaces it for
the $s_1$ tracker once the estimates carry noise, where the argmax
rather than the seed decides the tangent and the traversal direction can
invert.
...
is active, but the floor $e_H$ of Section~\ref{sec:sensitivity} leaves the
...
discrete one, so the two cannot be read independently. Table~\ref{tab:bandwidth} evaluates both
bounds at the double-gyre operating point.
\begin{table}[!t]
\centering
\caption{Transverse rate of each law along a separatrix segment, and the
fraction of that segment past the discrete bounds of
Section~\ref{sec:stability}, at the double-gyre operating point
($\Delta t = 0.1$, $k = 3.0$,
...
(1-\alpha_{\text{mom}}) > 2(1+\alpha_{\text{mom}})$, which at
this operating point is $\Delta t\,k\,a_\perp > 11.3$. This column also
evaluates $a_{\perp,D}$ on the realized fit, not on exact estimates
(see Theorem~\ref{thm:separatrix}'s caveat in Section~\ref{sec:stability}).}
\label{tab:bandwidth}
\footnotesize
...
resolved sign to agree across frames,
\begin{equation}
    \Omega\,\lVert\mathbf{p}_{\text{seed}}\rVert \;<\; \lvert\hat{\mathbf{v}}_0^\top\mathbf{t}_{\text{raw}}\rvert,
    \label{eq:seed_cond}
\end{equation}
at the seed point $\mathbf{p}_{\text{seed}}$; if it fails the two paths
are not close, they commit to opposite branches.
\end{proposition}
...
(\ref{eq:seed_cond}) is tested: at the objectivity experiment's start
$(0.05, 0.40)$ with $\Omega = 0.2$, $\Omega\lVert\mathbf{p}_{\text{seed}}\rVert
= 0.081$ against $\lvert\hat{\mathbf{v}}_0^\top\mathbf{t}_{\text{raw}}\rvert
= 0.107$, a margin of $1.3\times$. The sign survives here, but not
comfortably; solving (\ref{eq:seed_cond}) for the critical rate at this
point gives $\Omega \approx 0.265$, so a $33\%$ faster carousel would
flip the seed at this exact start.
\end{IEEEproof}
...
(\ref{eq:robot_first_order}), and the cluster layer distributes the centroid
...
zero in the noise-free demonstration runs. The cluster's angular
velocity command is zero throughout every run, so heading is held fixed
at its draw for the run's duration rather than slaved to the direction
of travel or left free; the reported ring-phase-dependent quantities of
Section~\ref{sec:disc_noise} are with respect to this fixed initial
heading. Clean trials run 400 to 600
...
GMT to May 17, 2012 12:00 GMT, so the numbers below are interpretable
against prior work on this record; no controlled head-to-head against
\cite{17} was run, since initialization and advection differ (Table~\ref{tab:capability}, Section~\ref{sec:limitations}). Frames are interpolated linearly in time
inside the field evaluator. World coordinates on $[-0.65, 0.65]^2$ map
affinely onto a region centered on
$(34.2^{\circ}\mathrm{N}, -120.4^{\circ}\mathrm{W})$ spanning
$\pm 0.300^{\circ}$ of latitude and $\pm 0.363^{\circ}$ of longitude, so one world
unit is $51.4$~km on both axes, and one simulated
step represents $600$~s of field time. The $168$-step trial spans the
full record. The FTLE fields used for evaluation are computed from
subwindows of this same record, at two horizons. The 24-h field of
Fig.~\ref{fig:ocean_ftle} integrates the first 25 frames, 08:00 GMT
May 16 to 08:00 GMT May 17, and the four-anchor persistence check of
Section~\ref{sec:disc_ocean} uses a 12-h horizon.

The longitude half-extent is $0.3^{\circ}/\cos(34.2^{\circ})$, so both
axes carry the same physical scale and one world unit is $L_w = 51.4$~km in
each. The map is then a similarity, a uniform dilation of the local
tangent plane, and with $\mathbf{J}_{\mathrm{world}} = L_w
\mathbf{J}_{\mathrm{phys}}$ the determinant scales by $L_w^2$ and the
symmetric part by $L_w$, both positive constants. Sampled vector
directions, every zero set, trench, and extremum of $D$, and the
rate-of-strain eigenframe are therefore the physical ones. Over the
corridor the trackers use, at four times across a $7 \times 7$ grid of
placements and at $14$ points on the tracked path, the sampled direction
and the angle between the world $\mathbf{e}_2$ and the physical one agree
to within numerical precision, and $\det \mathbf{J}_{\mathrm{world}} /
\det \mathbf{J}_{\mathrm{phys}}$ holds its constant to $2 \times
10^{-13}$ relative. The tangent plane uses $111.32$~km per degree of
latitude and that value scaled by $\cos(34.2^{\circ})$ per degree of
longitude, the same constants as the FTLE reference, so the controller
frame and the evaluation frame agree by construction.

The ocean operating point follows. The pentagon is enlarged to $\rho = 0.105$,
...
problem left outside its scope. The noise model of Section~\ref{sec:test_plan} is
validated on the double gyre only.

\subsection{Test Plan}
\label{sec:test_plan}

Four experiment families and diagnostic ablations validate the trackers, alongside three
supporting checks.
...
straddle to stop, the failure count of \cite{17}) and tracking error (mean of
$|x_c|$). Where the $s_1$ tracker fails, the sweeps supply noise-free inputs to individual channels to isolate the mechanism; the $D$ tracker's floor has a different, already-diagnosed cause (an unbiased random walk, Section~\ref{sec:disc_noise}) that channel ablation does not target. The $s_1$ sweep widens the exit box to
...
\mathbf{p}') + \Omega\,(-y', x')^\top$. The strength of the rotation
relative to the field is set by the Rossby number, in the usual sense of
the field's own vorticity scale against the frame's,
\begin{equation}
    \mathrm{Ro} = \frac{\lvert\omega\rvert_{\max}}{2\Omega} ,
\end{equation}
so that small $\mathrm{Ro}$ is rotation dominated. The double gyre's peak
vorticity is $2\pi^2 A \approx 1.97$ against an added solid-body
vorticity $2\Omega = 0.4$, giving $\mathrm{Ro} \approx 4.9$. The frame is
therefore a perturbation on the vorticity scale and not on the scale of
$D$, where its effect is unbounded
(Section~\ref{sec:objectivity_tradeoff}). The rate keeps the
structure's apparent speed at the tracked radius, $\Omega r \leq 0.10$,
inside the command authority $k\,c_{\max} = 0.12$, the condition
...
Barbara Channel field at the ocean operating point, evaluated against the 24-h forward-time FTLE
field anchored at the record start, computed offline from the full
radar grid. A $10 \times 10$ grid
of jittered starts tests start sensitivity for the $D$ tracker.

\subsubsection{Supporting checks}
Four checks sit alongside the four families. The estimator-accuracy
...
the degenerate-frame guard is needed at the isotropic point.
...
rather than arrival at a saddle. Finally, a spot check evaluates the $D$ tracker 
on the standard time-varying double gyre \cite{17} to confirm trench tracking 
without a feedforward time-derivative term.
...
(\ref{eq:grad_s1}) at $(0.03, 0.25)$, averaged over ring phase, recovers a
restoring slope of $6.83$ against the analytic transverse curvature
$6.888$, a full-sweep mean of $0.9$\% truncation deficit. A single unstated
phase can read as low as $6.6$, an isolated $4.7$\% deficit that is not
representative on its own; the spread is the orientation dependence of
Section~\ref{sec:sensitivity} rather than scatter. Rotating the ring
...
a tuning margin. By the identity of Section~\ref{sec:sep_controller} the
...
(Section~\ref{sec:sensitivity}), a finite-radius truncation artifact. At
...
$[-0.72, 0.43]$, which is the floor $e_H$ at $|y| = 0.45$, evaluated in
closed form in Section~\ref{sec:disc_noise}. The same limit binds in
both directions, since $\hat{\mathbf{H}}_{s_1}$ is negative semidefinite
for every field and formation (Remark~\ref{rem:s1hess}), so neither
...
from the 20\,000 acquisition trials of Section~\ref{sec:disc_noise}) no
run settles at a
...
\subsection{Behavior on a Time-Varying Field}
\label{sec:time_varying}

A spot check on the time-varying double gyre \cite{17}, parameterized by $\varepsilon = 0.1$ and
...
Acquisition is not the noise-limited phase either, the team reaching the
trench network within 0.05 in every one of the 20\,000 acquisition trials
through $\sigma_{uv} = 0.02$.

The $s_1$ tracker fails earlier, for a reason that has nothing to do with
estimator accuracy.\footnote{The $s_1$ sweep's own widened exit box
($|y| \leq 0.60$, Section~\ref{sec:test_plan}) recovers 30 of 54 trials
at $\sigma_p = 0.005$ (0.5\% of the cell) that the $D$ tracker's
narrower box ($|y| \leq 0.52$) would instead have scored as domain
exits, all of which exit at $y \in [0.520, 0.529]$ while still
converging.} Its success
...
$\|\mathbf{J}\| \approx 1$ at this test point. Runs on either side of the
transition settle at a saddle with comparable reliability and differ in
which one. The team has not lost the structure, become erratic, or
collapsed. It rides the separatrix competently and settles, in the wrong
direction. This is a sign inversion rather than a decay toward chance,
which is what makes the far-saddle column the score that separates the
two outcomes, since a near-saddle settle is indistinguishable from a
success in every other recorded quantity.
...
in one step either, returning
$\mathbf{t}^\top\mathbf{t}_{\text{ref}} =
|\mathbf{t}_{\text{raw}}^\top\mathbf{t}_{\text{ref}}| \geq 0$
identically, and instrumented runs count no one-step reversal at any
...
$\sigma_{uv} = 0.002$ and $\sigma_{uv} = 0.005$, where the unaided tracker
is at zero. The sign is therefore
...
0.0016 for the $D$ tracker, have a separate cause. Both trackers park
...
averaged over ring phase at the nominal formation. The signed offset has
period $72^\circ$ and changes sign at each mirror-symmetric phase (both
trackers: it vanishes at the same two phases and peaks $18^\circ$ away),
so the magnitude $|x_c|$ that the closed loop actually reports carries an
approximate $36^\circ$ period, running from 0.0006 to 0.0024 for the $D$
tracker. Success is not anisotropic in the same way, staying within
75.9--77.1\% across initial-heading quartiles over the 40\,000 trials at
$\sigma_{uv} \leq 0.01$, because the bias displaces the ride without
threatening it. Neither tracker shows the signature of the discrete
bound. Trajectories at the worst orientation hold a smooth one-sided
offset with two sign changes over a 300-step run at $\Delta t\,k\,a_\perp$
above 12, where a limit cycle would change sign every step or two, which
is what the actuator-aware threshold of Section~\ref{sec:stability}
predicts.
...
tracker here. The asymmetry is algebraic rather than statistical. The $D$
...
consulting. The clean runs already bound the consequence, since $D$
never fails to converge there: closest approach to the saddle ranges
0.0003 to 0.0194 across the six starts, and the re-signing simply has
one noisier cycle in that window rather than a wrong branch, because a
single miscue is overwritten the very next cycle instead of being
carried forward. The $s_1$ tracker
...
reversed. A perturbation large enough to break it does not degrade the
...
This distance is to the nearest wall or domain-boundary line ($x \in
\{-1,0,1\}$ or $y \in \{-0.5,0.5\}$) along the single deterministic run
from $(0.05, 0.40)$ at zero initial heading, not the $|x_c|$ transverse
offset the noise sweeps of Section~\ref{sec:disc_noise} report as a
trial mean over random heading; the two are not directly comparable, and
the near-zero value here reflects this one run's near-exact symmetry
about $x=0$ rather than a general property of the estimator. The $D$
...
strain field rather than to the sensing. The rotating-frame gaps contain
no $\nu$, no $\tilde{\rho}$, and no formation geometry, so no estimator
improvement removes them. Objectivity is therefore paid twice over, once
at the controller in the Newton normalization
...
90th percentile $3.0$--$5.9$ across the two trackers' paths: comparable
...
splitting-identity reading of the two trackers' agreement is qualified
accordingly on this field. Both trackers were run unmodified from a single shared
start north of the channel entrance,
$(34.41^{\circ}\mathrm{N}, -120.39^{\circ}\mathrm{W})$, at the operating
point given above. The $s_1$ tracker's depth condition
...
what the record below reports; its realized speed stays within
0.772--0.775~m/s of the pinned constant $k\,c_{\max}\tanh(1)$, consistent
with $\beta = 1$ throughout. Over the 28~h trial both descend through
...
Read together, the four experiment families above give a selection rule.
On a clean field in an inertial frame the two trackers are
interchangeable, and (\ref{eq:trench_coincide}) says when that holds
exactly. With a noisy sensor the $D$ tracker holds a grid step longer and
fails by decay rather than by inversion, which also makes its failures
easier to detect from the recorded quantities. Under a rotating observer
it does not degrade but leaves the structure, and only the $s_1$ tracker
returns the same material path. On a generic measured field either
recovers the corridor. Both consume the same fit, so the switch costs one
function call rather than a new estimator or formation.

\subsection{Limitations}
\label{sec:limitations}

First, the fit is ill-conditioned whenever the six sampling
points drift toward a common conic (Remark~\ref{rem:conic}); on the
ocean corridor $\kappa(\boldsymbol{\Phi}) = 688$ throughout for both
trackers (the formation never approaches degeneracy there), but
monitoring $\kappa(\boldsymbol{\Phi})$ and adding a
shape-maintenance term is the direct mitigation for fields where it
might, not implemented here. The cluster-layer shape controller already
holds formation well below this risk in practice: RMS deviation from the
nominal pairwise distances stays under $10^{-9}$ throughout the ocean
runs.
...
The $s_1$ tracker's tangent rule is the sharpest of the design
limitations, and Section~\ref{sec:disc_noise} gives its mechanism: the
exposure is the eigenvector comparison, not the seed, so remedies aimed
at the seed alone do not reach it. Holding the tangent whenever the
...
the decision instead of taking it at chance, costs no objectivity since
it reads only fitted coefficients, and is now implemented and swept on
the single-far-saddle acquisition trial (10\,000 trials/cell): success
rises from 44.6\% to 84.1\% at $\sigma_{uv} = 0.002$ and 6.0\% to
50.0\% at 0.003, largest exactly where the unaided tracker's success
falls fastest, narrowing above $\sigma_{uv} = 0.005$ where holding the
...
flow whenever the selected axis changes, which re-anchors the sign
rather than propagating it (the same one-time flow read the law already
permits, taken again); and, for a team with a trusted heading, resolving
the sign against a world axis every step, removing the propagation
chain entirely. Each would need its own noise sweep to verify.
...
planned Decabot hardware follow-on, since the robots are not immersed
in anything and the field is encoded and read rather than physically
experienced \cite{39}, but on the ocean field it is an extrapolation.
The commanded speed cap is far above realized ocean currents at this
operating point, but the assumption itself is untested against real
advection and is a scope boundary for that experiment, not a validated
one.

Four further extensions: a time-derivative correction term, analyzed
against the analytic $\varepsilon > 0$ double gyre of
Section~\ref{sec:time_varying}, would extend the stability results to
time-varying fields with a quantified bound; online formation reshaping
would protect the conditioning of $\boldsymbol{\Phi}$, and rotating the
ring within a run would average the orientation-dependent truncation
bias that Section~\ref{sec:disc_noise} shows sets the zero-noise
tracking error, which no gain reaches; extension to three dimensions
raises the estimation stakes to ten coefficients per component, so ten
robots or a reduced-order estimator; and targets other than critical
points, transient attracting profiles and eddy cores among them, are a
natural next object for the same estimator.
...
with noise gains verified to $2.1\%$. The $D$ tracker rides the
trench network of $D$ on a modified Newton step, with a Lyapunov
certificate and a traversal theorem; the $s_1$ tracker rides the same
network from the strain term alone, with a terminal test built only
from the objective gradient. Its frame equivariance was demonstrated
in closed loop: the same material curve in either observer frame (gap
0.025), where the $D$ tracker follows a frame artifact (gap 1.219). The
estimator, not the control law, sets the $D$ tracker's noise ceiling,
its failure cliff falling within $5\%$ of the gradient-reliability
threshold; the $s_1$ tracker's ceiling is set one level earlier, by
the reliability of its one-time flow-seeded tangent sign. On real Santa
Barbara Channel currents both trackers followed the same dominant ridge
for 28 hours, the $D$ tracker reaching a landfall reproduced by 84 of
100 jittered starts and the $s_1$ tracker still closing on that target
when the record ended. The choice between the two trackers is set by
the observing platform rather than the flow: where a team can certify
its orientation against a trusted reference, the $D$ tracker tolerates
the noisier sensor; where it cannot, only the $s_1$ tracker returns the
same material curve. Both read the same fit, so a team that carries the
estimator carries both.
