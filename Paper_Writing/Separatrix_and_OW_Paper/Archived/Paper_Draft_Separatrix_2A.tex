...
% This is the last version of this document to be primarily written by AI
...
\title{One Estimator, Two Surrogates: Distributed Multirobot Tracking of
...
{Waight and Kitts: Distributed Multirobot Tracking of Instantaneous Coherent Structures}
...
along which drifting material collects within hours. This paper presents
a framework in which a team of six robots, each measuring only the flow
velocity at its own position, finds and follows these structures in real
time, with no map, no forecast, and no global information. One
distributed estimator recovers the local quadratic model of the field
and, from a single synchronous set of measurements, two scalar
surrogates of the same hidden skeleton, the two terms of one splitting
of the fitted velocity gradient into strain plus spin: the Jacobian
determinant, which keeps both terms, whose valley lines trace the
separatrices and strain skeleton of the flow but depend on the
observer's frame, and the smaller rate-of-strain eigenvalue, which
keeps only the strain term, whose valley lines are the objective,
frame-invariant attracting structures of short-term transport. Two
adaptive controllers, one per surrogate, span the tradeoff between them.
The separatrix tracker rides the determinant's globally connected trench
network on a modified Newton step, through the saddle points where
trench segments cross, with a network-traversal guarantee. The TRAP
tracker inherits the objectivity of its surrogate: in a rotating-frame
experiment it recovers the same material curve in either observer frame,
while the separatrix tracker follows a frame artifact. Monte Carlo noise
sweeps place both failure cliffs at the estimator's own reliability
threshold: the estimator, not the control law, sets the noise ceiling.
Both controllers run unmodified on ocean-radar currents from the Santa
Barbara Channel, one following a coherent ridge for 28 hours to a
consistent landfall, the other converging to and holding a transient
attracting profile within one grid cell of ground truth computed offline
from the full radar map.
\end{abstract}

\begin{IEEEkeywords}
Multirobot Systems, Adaptive Navigation, Coherent Structures,
Okubo-Weiss, Rate of Strain, Objectivity, Double Gyre, Cooperative
...
pays an integration latency before an estimate exists. This paper's
approach requires neither a seeded manifold nor an integration horizon:
from an arbitrary six-robot configuration, the team infers a structure's
location, its type, and, at a crossing, which branch to continue onto,
from the current instantaneous measurement alone. Our own prior work
estimated a single isolated critical point from a linear three-robot fit
\cite{14}; that is a point-estimation problem, and the work here is a
field-structure problem, requiring the curvature of derived scalar
fields and therefore a second-order estimate.

The paper is organized around one algebraic fact. The velocity
gradient of a flow splits uniquely into strain plus spin, $\mathbf{J} =
\mathbf{S} + \mathbf{W}$, and for incompressible planar flow its
determinant splits with it, $D = \det(\mathbf{J}) = \omega^2/4 -
s_1^2$, where $\omega$ is the vorticity carried by $\mathbf{W}$ and
$s_1$ the smaller eigenvalue of $\mathbf{S}$
(Section~\ref{sec:detj_field}). Six robots measuring the flow at their
own positions determine the local quadratic model of the field exactly
(Section~\ref{sec:estimation} proves six is the minimum), and the same
twelve fitted coefficients deliver both terms of the identity as
navigable scalar fields whose valley lines mark the flow's
instantaneous skeleton: $D$, classical, Galilean invariant, and
globally connected into a trench network; and $s_1$, the surrogate
underlying Serra and Haller's objective Eulerian coherent structures
(OECSs) \cite{17}, invariant under every time-dependent rotation of the
observer but confined to bounded attracting segments. A rotation of the
observer lands entirely in the spin term, so the choice between the
surrogates is exactly the choice of whether to trust it. Where the
vorticity vanishes the identity collapses to $s_1 = -\sqrt{-D}$ and the
two coincide; everywhere else, and in every rotating frame, they part
ways. The \emph{separatrix tracker} rides the $D$ network with flow
assistance and a traversal guarantee; the \emph{TRAP tracker} refuses
every frame-dependent input and in exchange tracks curves that exist
for every observer. Both are one function call on the same estimate, so
the choice between them is a mission decision, not an architectural
one.
...
\begin{enumerate}
    ...
    \item The separatrix tracker: a hybrid controller built on a
          ...
          special case.
    \item The TRAP tracker: an objective controller built from the
...
and its sensitivity. Section~\ref{sec:controllers} presents the two
adaptive controllers. Section~\ref{sec:stability} states the stability
results, with proofs in Appendix~\ref{app:proofs}.
Section~\ref{sec:results} describes the simulation program and presents
results, including the ocean experiments.
Section~\ref{sec:discussion} discusses implications, failure modes, and
...
\subsection{The Rate-of-Strain Tensor and the Splitting Identity}
...
separatrix tracker does to $D$, with one structural difference in what
...
Two design consequences. First, the gains are isotropic: estimator
quality does not degrade as the cluster turns. Second, the $1/\rho$ and
$1/\rho^2$ factors exhibit the fundamental scaling: second derivatives
are harder to sense than first derivatives by one power of the
formation radius. A numerical search over six-point formations of the
same footprint found at best a few percent improvement in worst-case
...
The favorable one: the tangent the OECS tracker rides,
$\mathbf{e}_2$, comes from the \emph{first-order} fitted coefficients,
whose noise gain is $(2/\sqrt{10})\,\sigma_{uv}/\rho$ by
Lemma~\ref{lem:gains}, one full derivative order better than the
$\hat{\mathbf{H}}_D$ eigenvectors the separatrix tracker uses for the
...
$6.9$ at the test point of Section~\ref{sec:results}). The OECS tracker
...
for the separatrix tracker's network reach, the strain term alone for
the TRAP tracker's frame invariance.
...
\begin{tabular}{lll}
\hline
 & \textbf{Separatrix tracker} & \textbf{TRAP tracker} \\
\hline
...
\subsection{OECS Tracker (Objective Controller)}
...
Newton snap, so the two channels are built differently from Logic C's.
...
\hrule
\vspace{4pt}
\noindent\textbf{OECS tracker: one control cycle}
...
plays in Logic C, and $s_{\text{capture}}$ is the exact analogue of
...
Logic C is equally explicit: it has no flow-assisted mode, so it does
...
noted. The OECS tracker shares $v_{\max}$ and $k$ and adds $g_\perp =
...
This section states the closed-loop guarantees; all proofs are
...
\subsection{The OECS Tracker: Objectivity and Local Convergence}
\label{sec:stab_oecs}

Two properties of the OECS tracker are stated at the same navigation
...
\subsection{Separatrix Tracker: Clean Runs and Noise Robustness}
\label{sec:test_plan}
...
\begin{figure}[!t]
    ...
    \caption{Separatrix tracker centroid trajectories from six
...
$\mathbf{p}^*_1$ for the separatrix tracker, core arrival at
$\mathbf{p}^*_2$ for the TRAP tracker. (c), (d): straddle retention
(fraction of trials, the Michini-comparable metric \cite{9}). The TRAP
...
\multicolumn{6}{l}{(a) Separatrix tracker: traverse success (\%)} \\
\hline
...
\hspace{1.5em}
\begin{tabular}{r|rrrrr}
\multicolumn{6}{l}{(b) TRAP tracker: core-arrival success (\%)} \\
\hline
$\sigma_{uv}$ \textbackslash\ $\sigma_p$ & 0.000 & 0.005 & 0.010 & 0.020 & 0.050 \\
\hline
...
\\[8pt]
\begin{tabular}{r|rrrrr}
\multicolumn{6}{l}{(c) Separatrix tracker: straddle retention} \\
\hline
$\sigma_{uv}$ \textbackslash\ $\sigma_p$ & 0.000 & 0.005 & 0.010 & 0.020 & 0.050 \\
\hline
...
\hspace{1.5em}
\begin{tabular}{r|rrrrr}
\multicolumn{6}{l}{(d) TRAP tracker: straddle retention} \\
\hline
$\sigma_{uv}$ \textbackslash\ $\sigma_p$ & 0.000 & 0.005 & 0.010 & 0.020 & 0.050 \\
\hline
...
Noise robustness is evaluated in Monte Carlo over a two-dimensional
noise grid, from one consistent initial condition, a formation
straddling the separatrix at $(0, 0.35)$, so that the success rate
measures noise robustness alone rather than basin geometry
...
Task success is reaching the terminal saddle (centroid within 0.06 of
$\mathbf{p}^*_1$) without formation collapse (an inter-robot distance
...
\subsection{TRAP Tracker: Double Gyre and Objectivity}
...
Fig.~\ref{fig:sep_trajectories} is the designed one: Logic C's flow
mode carries it through the crest and its trench continuation carries
it through the terminal saddle, while the OECS tracker treats both as
what they are objectively, the end of one attracting structure and the
core of another.

\subsubsection{Estimator diagnosis}
...
for the $\hat{\mathbf{H}}_D$ eigenvector that Logic C uses as its
...
level at which both gradient channels approach chance.
...
Both controllers were run on the same physical double gyre expressed
in two observer frames: the inertial frame, and a frame rotating at
...
near-zero gaps for the OECS tracker and offers no such protection to
Logic C, whose surrogate obeys (\ref{eq:D_not_objective}).

Fig.~\ref{fig:oecs_objectivity} shows the outcome. The
OECS tracker's two runs are the same material path: final gap 0.020,
...
trench network of 0.001 and 0.005 respectively. The separatrix
...
\begin{figure}[!t]
    ...
    ($\bullet$). Left: the OECS tracker's two trajectories coincide
    (final gap 0.020) and park at the TRAP core ($\times$). Right:
    Logic C's rotating-frame run leaves the separatrix and ends off
...
The OECS tracker was swept over the same noise grid, from the same
straddling start with the same random-heading draw, in core-seek mode.
The terminal target differs by necessity: the attracting segment
through that start ends at its TRAP core, the top saddle
$\mathbf{p}^*_2 = (0, 0.5)$, so task success is reaching
$\mathbf{p}^*_2$ within 0.06 without formation collapse, and the
...
Table~\ref{tab:mc_success}(b) reports core-arrival success for the TRAP
tracker over the same noise grid, with straddle retention in panel (d).
The zero-noise tracking error is 0.008, four times the separatrix
tracker's 0.002, the gradient-rate cost predicted by
Remark~\ref{rem:s1hess}. Everything else moves in the TRAP tracker's
...
separatrix tracker's cliff (panels (d) against (c)). Position noise is
where the first-order eigenframe shows most clearly: $\sigma_p = 0.05$
alone, which leaves the separatrix tracker at 18.8\%, leaves the TRAP
...
columns, which degrade smoothly for the TRAP tracker but jump for the
separatrix tracker. The
...
\subsection{Ocean HFR Experiments}
\label{sec:ocean_sim}
...
\subsubsection{Separatrix tracker}
The separatrix tracker was run on this field from a single start north
...
miniature: Logic C rides a ridge sixty kilometers to landfall, while
...
\begin{figure}[!t]
    ...
    \caption{OECS tracker (core-seek) on the 2~km Santa Barbara Channel
...
follows. The TRAP tracker keeps only the strain term, and what it
...
0.008 against Logic C's 0.002, is this cost made visible); and a
...
known, fixed frame is better served by Logic C, and one that needs
frame-proof material prediction, or operates from a rotating or
drifting platform, by the TRAP tracker. Both consume the same fit, so
...
Third, the OECS tracker's eigenframe is noise-fragile exactly where
...
separatrix tracker rides the connected trench network of $D$ on a
...
away. The TRAP tracker rides the bounded attracting segments of $s_1$;
...
Barbara Channel currents the separatrix tracker followed a coherent
ridge for 28 hours to a consistent landfall reproduced by 84 of 100
nearby starts, and the TRAP tracker converged to and held a transient
...
% ---------------------------------------------------------------------------
\section{Proofs of the Stability Results}
\label{app:proofs}
