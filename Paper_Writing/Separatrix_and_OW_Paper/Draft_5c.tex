...
\det(\mathbf{J}(\mathbf{p}))$ throughout. In time-dependent flows the
separatrix generalizes to a Lagrangian coherent structure (LCS), a ridge
of the finite-time Lyapunov exponent (FTLE) field \cite{6,7,8}; but
computing FTLE requires integrating dense trajectory grids over a
finite horizon, exactly the global, forward-in-time knowledge this
paper's sensing model excludes. The two Eulerian surrogates this paper
tracks instead, both delivered by the same fit, are computed pointwise
from the instantaneous field and its spatial derivatives alone.
...
\emph{transient attracting profile} (TRAP) in the sense of \cite{36}.
TRAPs are the terminal points a traverse settles at, not the object it
follows. One caution accompanies
...
minimum of $s_1$. The vanishing shear makes the double gyre nongeneric,
with degeneracy lines in place of the isolated singular points of a
generic strain field \cite{17}; the ocean field is generic.
...
order though Newton normalization is unavailable; and at $\sigma_{uv} =
...
(\ref{eq:splitting}); Fig.~\ref{fig:oecs_objectivity} and
Section~\mbox{VI-B-2} give the mechanism and the measurement. On the
double gyre objectivity costs no reach, since the two trenches coincide
exactly, but it costs rate and, under noise, an earlier failure
threshold set by the one-time tangent seed rather than by transverse
tracking. A mission whose separatrix carries vorticity, or whose
platform is fixed in a known frame, may prefer the $D$ tracker's
Newton-rate convergence and its independence from a seeded tangent
direction; one that needs frame-proof material prediction, or operates
from a rotating or drifting platform, gains that guarantee at the cost
just quantified. Both consume the same fit, so the choice is one
function call, not a new estimator or formation.
...
(Proposition~\ref{prop:conic}); monitoring the conditioning
$\kappa(\boldsymbol{\Phi})$ and adding a shape-maintenance term is the
direct mitigation, not implemented in the reported trials (the Monte
Carlo collapse flag, RMS inter-robot distance error above 0.30, never
triggers in any reported trial regardless). Second, noise can chatter the
