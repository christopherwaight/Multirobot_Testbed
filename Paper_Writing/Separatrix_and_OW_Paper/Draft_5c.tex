...
$\boldsymbol{\phi}(\mathbf{p}_i)^\top \mathbf{c} = 0$ for all
...
vanishes at the origin, so interpolation forces $\hat{a}_1$ to equal the
...
selection (\ref{eq:tangent_select}) uses only the
...
the transverse channel (\ref{eq:oecs_law}) apply the invariant gradient
...
on. ACQUIRE, PARK, and CAPTURE are the vector-saturated descent
$-\mathrm{sat}(g_\perp\nabla\hat{s}_1)$ of (\ref{eq:oecs_law}), which
preserves the direction of the co-rotating invariant gradient and is
therefore exactly covariant. Only the flow $\mathbf{f}$ enters
noncovariantly, and it does so at a single instant: CROSS's direct use
of it, reached only if $r < r_{\text{band}}$ on the first tracking step
with no $\mathbf{t}_{\text{prev}}$ established, and the sign reference on
