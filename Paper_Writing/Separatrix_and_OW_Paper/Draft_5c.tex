...
0.075$ throughout the double-gyre experiments, where $M_3$ is known
analytically and the tradeoff above is exact. No bias bound is claimed
on the ocean field, where $\rho$ is set instead by the sampling
requirement of Section~\mbox{V-C}.
...
ocean runs of Section~\ref{sec:results_ocean} and
Section~\ref{sec:results_ocean_oecs} follow the same convention: the
noise model of Section~\mbox{V-D} is validated on the double gyre only.
...
validated start shows the outcome is not knife-edge sensitive to the
initial condition: 84 of the 100 land on the same middle-island branch
as the validated trial, with the rest carried onto a western branch by
starts nearest the jitter-box edge furthest from the ridge.
...
tracker's own path also crosses before continuing past it. A genuine
two-dimensional minimum of $s_1$ was never
...
tracker does from the objective strain term alone. The method runs
unmodified on measured data for 28~h and produces a path that follows
the dominant FTLE ridge, the same demonstration the $D$ tracker gives
in Fig.~\ref{fig:ocean_ftle}.

Across twelve dispersed starts spanning the channel, the outcome is not
knife-edge sensitive to the start: the run of Fig.~\ref{fig:ocean_oecs}
is the one that travels farthest from the offline TRAP cores before the
28-h record ends and is shown for the traverse it produces, not as a
typical result. Which segment of the network a ride commits to, like the
...
\begin{figure}[!t]
    ...
    tracker's own path also crosses. White stars mark the 92 offline
    TRAP cores present at the final frame, for context; this start is
    the one of twelve tested that travels farthest from them and is
    shown for the traverse it produces, not as a typical result.}
