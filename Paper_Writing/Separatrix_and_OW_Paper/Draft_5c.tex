...
to twelve unknowns, and Section~\ref{sec:estimation} shows that six
robots, the same count as the ring-plus-center of \cite{15}, is the
exact minimum for the unconstrained quadratic model, against any
algorithm, and that six suffice for every placement outside a single
conic degeneracy. Where \cite{15} applies fixed symmetric weights valid
...
planar quadratic field rather than over a constrained
...
construction, not made small by a good fit;
...
Section~\ref{sec:strain_field}. Write $\mu$, $s_n$, and $s_s$ for the
...
Two degeneracies fall outside (\ref{eq:d_law}), both arising when
$\hat{\mathbf{H}}_{D,0}$ has eigenvalues of one sign and the trench frame
is undefined. Off the band, the command reverts to the signed Newton
step $\sum_k \mathrm{sat}(-\mathbf{w}_k^\top
\hat{\mathbf{g}}_0/\lambda_k)\mathbf{w}_k$, which drives the centroid to
the nearest stationary point of $\hat{D}$ and serves as the terminal
capture law of Theorem~\ref{thm:attract}. On the band, it reverts to the
...
Section~\ref{sec:strain_field} established that $s_1$ is objective where
$D$ is not. The second controller tests whether that guarantee is
...
Three situations clear the ride flag. Before the team has reached any
...
one, when $\hat{s}_1$ first falls below $-s_{\mathrm{trim}}$. If an
optional core threshold $s_{\mathrm{capture}}$ is supplied, $\beta$
returns to zero wherever $\hat{s}_1 < s_{\mathrm{capture}}$. The third
situation is the default terminal behavior, and it holds the team at a
...
$\hat{s}_1$ away from real structure. The condition latches once
...
We begin with the degenerate case of (\ref{eq:d_law}), the signed Newton
step, because it supplies the terminal behavior the traversal result
ends in.

\begin{theorem}[Terminal capture]
\label{thm:attract}
Let $V_A = \tfrac{1}{2}\|\mathbf{g}(\mathbf{p}_c)\|^2$. Along
(\ref{eq:kinematic_model}) with the signed Newton step,
\begin{equation}
    \dot{V}_A = -c_{\max} \sum_{k=1}^{2} \lambda_k^2\,
    z_k \tanh\!\left(\frac{z_k}{c_{\max}}\right) \;\leq\; 0,
    \label{eq:VA_dot}
\end{equation}
with equality exactly where $\mathbf{g} = \mathbf{0}$. Every bounded
trajectory on which $\mathbf{H}_D$ is nonsingular therefore converges to
the critical set of $D$, and isolated critical points with
$\mathbf{H}_D$ nonsingular are locally exponentially stable.
\end{theorem}

The step descends $\|\nabla D\|^2$ toward the stationary set of $D$, and
near a well, where $\mathbf{H}_D \succ 0$, convergence is exponential.
...
\end{equation}
with vanishing cross terms and eigenvalue gap $\lambda_2 - \lambda_1
\geq \gamma > 0$ wherever $\mathbf{H}_D$ has mixed signs. The curve
...
the trench length. At the saddle, $D$ attains a local minimum with
$\mathbf{H}_D \succ 0$, so the degenerate case of (\ref{eq:d_law}) is
active and (iii) Theorem~\ref{thm:attract} gives local exponential
convergence to the saddle.
\end{theorem}

Part (iii) is a statement about the true Hessian, and in practice the
tracker continues past the saddle instead. The saddle is a crossing of
two perpendicular trench segments (Section~\ref{sec:dg_example}), and
...
segment, repeating (i) and (ii) with $\Gamma$ relabeled, which is what
Section~\ref{sec:results} reports at every saddle reached. Both
...
$\bar{d}$ inherits the noise and truncation scalings of
Section~\ref{sec:sensitivity}, (\ref{eq:iss_bound}) degrades linearly in
...
let the tracker of Section~\ref{sec:oecs_controller} run on the
...
every frame. The single exception is the degenerate guard of
Section~\ref{sec:oecs_controller}, which reads the flow directly and is
...
$\varepsilon_{\text{raw}}$ & $10^{-3}$ & $10^{-3}$ & --- & on-structure raw thr. (s$^{-2}$) \\
$\varepsilon_{\text{dim}}$ & 0.025 & 0.025 & --- & on-structure curv.-norm. thr. (m$^2$) \\
...
$g_{\text{capture}}$ & 0.15 & 0.15 & --- & terminal gradient-mag. thr. (s$^{-1}$m$^{-1}$) \\
$r_{\text{band}}$ & 0.05 & 0.05 & --- & eigenframe-resolution thr. (s$^{-1}$) \\
\hline
\end{tabular}
\end{table}
...
entirely, checked against the analytic geometry of
Section~\ref{sec:dg_example}. Each run records the step at which the
...
structure. For the $D$ tracker, a park-versus-traverse pair
(Section~\ref{sec:park_vs_traverse}) runs both settings of
$D_{\text{capture}}$ from the same start; the $s_1$ tracker's own
terminal test (\ref{eq:capture_test}) is unconditional by design and
needs no such pair.
...
scored as domain exits while still converging. The step budget, start,
...
0.05 to 0 reported in Section~\ref{sec:limitations}, the degenerate-frame
guard of Section~\ref{sec:oecs_controller} never fires once, so the
...
0.322 across the six. It is largest where the two trackers commit to
different saddles, from a start equidistant enough that the seeded
tangent and the $D$ tracker's own flow-tested tangential term do not
agree; where they commit to the same saddle the two paths are visually
...
$\sigma_{uv} = 0.001$ through 43.6\% at 0.002 to 6.7\% at 0.003, and by
...
The mechanism is the one-time flow read
...
comparable to $\sigma_{uv}$ in the 0.001--0.003 range, so the sign it
resolves is decided by noise well before the transverse channel's own
tolerance is approached. Because the decision is made once and then
carried forward by continuity, a perturbation large enough to flip it
does not merely degrade the run, it commits the whole ride to the
opposite branch; and because the flow at $(0, 0.35)$ points down the
separatrix by a margin that noise of this size reverses systematically
rather than symmetrically, the flip is near-total above the threshold
...
of Table~\ref{tab:params}, $s_{\text{capture}}$ unset so the team relies
on the terminal test (\ref{eq:capture_test}) alone. Ground truth is
...
Remark~\ref{rem:eigvec} identifies the terminal-capture test as
...
case checked before the on-structure test (\ref{eq:flow_band}) in
(\ref{eq:d_law}),
\begin{equation}
    \dot{\mathbf{p}}_c =
...
Section~\ref{sec:strain_field}). Before the ride begins, such a region
...
a monomial in $\cos\theta_j, \sin\theta_j$ of degree at most four, hence
a combination of $\sum_j e^{in\theta_j} = e^{in\psi}\sum_j\omega^{jn}$
with $\omega = e^{2\pi i/5}$ and $|n| \leq 4$. Since $\sum_j\omega^{jn}
= 0$ unless $n \equiv 0 \pmod 5$, only $n = 0$ survives, and it carries
no $\psi$: every power sum through fourth order is phase-independent.
The odd sums vanish and the even ones are $\sum_j\cos^2\theta_j =
\sum_j\sin^2\theta_j = \tfrac{5}{2}$, $\sum_j\cos^4\theta_j =
\tfrac{15}{8}$, and $\sum_j\cos^2\theta_j\sin^2\theta_j = \tfrac{5}{8}$.
...
Write $\boldsymbol{\delta}$ for the signed Newton step, the degenerate
case of (\ref{eq:d_law}), whose $k$-th eigendirection component is
$\mathrm{sat}(-z_k) = -c_{\max}\tanh(z_k/c_{\max})$.

\textit{Gradient of the Lyapunov function.} Since $V_A =
\tfrac{1}{2}\mathbf{g}^\top\mathbf{g}$ with $\mathbf{g} = \nabla
D(\mathbf{p}_c)$, the chain rule gives $\nabla V_A = (\partial
\mathbf{g}/\partial\mathbf{p})^\top\mathbf{g} = \mathbf{H}_D\mathbf{g}$,
using that the Jacobian of the gradient is the Hessian and that
$\mathbf{H}_D$ is symmetric.

\textit{Derivative along trajectories.} Differentiating along
(\ref{eq:kinematic_model}), $\dot{V}_A = \nabla V_A^\top\dot{\mathbf{p}}_c
= \mathbf{g}^\top\mathbf{H}_D\boldsymbol{\delta}$. Expanding
$\boldsymbol{\delta}$ in the eigenbasis and using
$\mathbf{H}_D\mathbf{w}_k = \lambda_k\mathbf{w}_k$,
\begin{equation*}
    \mathbf{g}^\top\mathbf{H}_D\boldsymbol{\delta} =
    \sum_k \mathrm{sat}(-z_k)\,\lambda_k(\mathbf{w}_k^\top\mathbf{g}).
\end{equation*}
By the definition $z_k = (\mathbf{w}_k^\top\mathbf{g})/\lambda_k$ we have
$\mathbf{w}_k^\top\mathbf{g} = \lambda_k z_k$, so each summand carries a
factor $\lambda_k^2 z_k$, which is (\ref{eq:VA_dot}).

\textit{Sign and equality case.} Every summand has the form $\lambda_k^2
z_k\tanh(z_k/c_{\max})$ with $\lambda_k^2 \geq 0$ and $z\tanh z \geq 0$,
vanishing only at $z = 0$. Hence $\dot{V}_A \leq 0$, with equality
exactly when $z_1 = z_2 = 0$. Because $\mathbf{H}_D$ is nonsingular the
$\lambda_k$ are nonzero, so this forces $\mathbf{w}_1^\top\mathbf{g} =
\mathbf{w}_2^\top\mathbf{g} = 0$, and since $\{\mathbf{w}_1,
\mathbf{w}_2\}$ is a basis of the plane, $\mathbf{g} = \mathbf{0}$.

\textit{Convergence.} On a bounded trajectory along which $\mathbf{H}_D$
stays nonsingular, $V_A$ is continuous, nonincreasing, and bounded below,
so LaSalle's invariance principle gives convergence to the largest
invariant subset of $\{\dot{V}_A = 0\}$, which by the previous step is the
critical set $\{\mathbf{g} = \mathbf{0}\}$ of $D$.

\textit{Local rate.} Near an isolated critical point
$\mathbf{p}^\star_D$ with $\mathbf{H}_D$ nonsingular, $\|\mathbf{g}\|$ is
small, so $|z_k| \ll c_{\max}$ and the saturation is inactive to first
order: $\tanh(x) = x + O(x^3)$ gives $\boldsymbol{\delta} \approx
-\sum_k z_k\mathbf{w}_k = -\mathbf{H}_D^{-1}\mathbf{g}$. Linearizing
$\mathbf{g}$ about $\mathbf{p}^\star_D$ gives $\mathbf{g} \approx
\mathbf{H}_D(\mathbf{p}_c - \mathbf{p}^\star_D)$, so
$\boldsymbol{\delta} \approx -(\mathbf{p}_c - \mathbf{p}^\star_D)$: a
linear contraction at unit rate in the rescaled time of
(\ref{eq:kinematic_model}), that is at rate $k$ in real time. Local
...
\textit{Step 4: capture, giving (iii).} At the flow saddle $D$ attains a
local minimum, so the true $\mathbf{H}_D \succ 0$ and its eigenvalues
share a sign. The degenerate case of (\ref{eq:d_law}) is then active and
the command is exactly the signed Newton step certified by
Theorem~\ref{thm:attract}, which gives local exponential convergence to
that well; the well coincides with the flow saddle $\mathbf{p}^*$.

\textit{Step 5: continuation when the test does not fire.} Step 4
concerns the true Hessian. If the sign test fails on
$\hat{\mathbf{H}}_{D,0}$ despite $\mathbf{H}_D \succ 0$ holding for the
field, (\ref{eq:d_law}) stays in its band or off-band branch. The saddle
is a right-angle crossing of two trench segments
(Section~\ref{sec:dg_example}), so whichever segment the flow direction
and the sign convention on $\mathbf{w}_1$ select satisfies
Assumption~\ref{ass:trench} in its own right, and Steps 1 to 3 apply
unchanged with $\Gamma$ relabeled. Section~\ref{sec:results} reports this
at every saddle reached.
\end{IEEEproof}
...
contact on $\hat{s}_1 < -s_{\mathrm{trim}}$, the optional park on
$\hat{s}_1 < s_{\mathrm{capture}}$, and (\ref{eq:capture_test}) on $r$,
$\lVert\nabla\hat{s}_1\rVert$, and $\hat{s}_1$. So $\beta$ takes the same
value at corresponding states.
...
\textit{Step 5: the single exception.} The degenerate guard of
Section~\ref{sec:oecs_controller} reads the measured flow, which is not
