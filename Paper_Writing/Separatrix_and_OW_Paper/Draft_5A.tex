...
two for the second-order coefficients gives
\begin{equation}
    \rho^* \propto \left(\sigma_{uv}/M_3\right)^{1/3},
    \label{eq:rho_star}
\end{equation}
which is best read as a scaling law rather than a tuning rule. It
explains the shape of Fig.~\ref{fig:est_accuracy}(b), a shallow bowl in
$\rho$ for each derivative order, and its cube-root form makes the
optimum flat: a factor-of-two error in the unknown $M_3$ moves $\rho^*$
by only $26\%$. The optimum itself is not computable in the field, since
$M_3$ is a property of an unknown flow; with six robots and six
coefficients the fit is exactly determined, leaving no residual from
which to estimate $M_3$ online. A seventh robot, or a temporal
consistency check across cycles, would supply one, and the empirical
minima for the benchmark field are reported below.
...
Fig.~\ref{fig:est_accuracy}(b) shows the shallow bowl the scaling law
(\ref{eq:rho_star}) predicts, with empirical minima at $\rho \approx
0.11$ for $\hat{D}$ and $\rho \approx 0.23$ for $\nabla\hat{D}$ and a
radius-independent floor for $\hat{\mathbf{H}}_D$.
