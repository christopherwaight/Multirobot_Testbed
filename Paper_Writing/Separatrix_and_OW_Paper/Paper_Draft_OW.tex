...
% This is the last paper that mentions OW det(J) = 0. That feature is ommited in future revisions. July 09, 2026
...
{Waight and Kitts: Adaptive Navigation along the Separatrix and Okubo-Weiss Boundary of the Double Gyre}
...
distributed sensing [1]. Deployed in geophysical flows such as ocean
...
advecting patches [2] to heterogeneous ASV/AUV teams tracking salinity
fronts [3] and glider fleets adaptively sampling upwelling events during
the AOSN-II experiment in Monterey Bay [4]. Two boundaries appear
prominently in steady two-dimensional flows: the separatrix, which
contains both the stable and unstable manifold connecting to saddle
points, which partitions the domain into two distinct basins; and the
Okubo-Weiss boundary, the locus of points where the local flow transitions
from rotation-dominated to strain-dominated behavior, closely related to
the physical parameter used to detect and classify oceanic mesoscale
eddies [5]. Both structures are relevant to dispersion modeling,
pollutant containment, and the deployment of sensor networks in coastal
and atmospheric environments [6, 7, 8], and both can, in principle,
be inferred from instantaneous field measurements alone.
...
manifold and the other two on opposite sides of it [9]; the same strategy was
extended to an $N$-robot team for finer spatial sampling of the same manifold
[10] and validated on physical micro-ASVs in a flow tank [11]. Knizhnik et al.
couple a related manifold-tracking law to vehicle-level flow control in
gyre-like marine environments [12]. Kularatne and Hsieh take a different
...
structure's type explicitly rather than assuming it [13]. Every member of this
family, however, fixes the structure's identity before tracking begins: [9],
[10], and [11] require a saddle-straddling line segment seeded around an
...
estimator and controllers require neither a seeded manifold nor an
...
formation and a proportional or orbital control law [14]. That is a
...
\begin{enumerate}
    \item A distributed quadratic estimator that uses simultaneous measurements
          ...
          field [15], and derive $\det(\mathbf{J})$, its gradient, and its
    ...
    \item Two adaptive controllers built on a modified Newton step: the
          ordinary step $-\mathbf{H}_D^{-1}\nabla D$ is decomposed in the
          eigenbasis of $\mathbf{H}_D$, each eigen-direction is independently
          saturated, and, in the separatrix tracker, each signed eigenvalue is
          replaced by its magnitude so the team slides along a trench even
          where the local curvature is negative. (a) A hybrid three-mode
          separatrix tracker (Logic C) that uses this step to slide along the
          stable manifold of the double-gyre saddle; (b) an Okubo-Weiss
          boundary tracker that applies a Newton-step contour-following law to
          drive $\det(\mathbf{J}) \to 0$ while drifting tangentially along the
          boundary.
    \item A stability and network-traversal proof for the separatrix tracker:
          ...
          [9, 10], this controller commits to no manifold identity in
...
organizes tracer transport over a finite time horizon [6]. Haller's later
...
Euclidean frame changes that a material response cannot depend on [16].
...
[8]. Both the definition and its objectivity come at a fixed cost: an FTLE
...
[16]. Serra and Haller later placed a version of this instantaneous idea on
...
made precise for the FTLE field itself [17, 18]. The velocity gradient
...
gradients from sparse data [19]. The rate-of-strain and spin decomposition
...
deformation gradient [20]. A robot team sampling the flow once per
...
$Q < 0$ [21, 22]. Hunt, Wray, and Moin generalized the same second-invariant
idea to three dimensions as the Q-criterion for identifying eddies, streams,
and convergence zones in turbulent flow fields [23]. Hua and Klein revisited
...
regions of a nearly two-dimensional turbulent flow [24]. This paper works
...
vanishes at every critical point regardless of type [25]. Rather than
...
two on either side of it, with no FTLE field or global map required [9].
...
of the boundary [10], and Michini, Hsieh, Forgoston, and Schwartz separately
validated the three-robot strategy experimentally on a team of micro
autonomous surface vehicles in a flow tank [11]. Knizhnik, Li, Yu, and Hsieh
...
control [12]. In each of these, the team's controller commits to a manifold
...
revisited [9]. The accompanying stability proof holds only under the
...
formation-keeping guarantees later validated on real ocean flow data [13];
...
[26]. Briñón-Arranz, Renzaglia, and Schenato extended this to the Hessian,
...
radius is properly scaled to the noise floor [15]. McDonald, Kitts, and
...
alone, without an explicit Hessian estimate [27, 28], later verified
experimentally on physical hardware [29]. Hessian estimation of a
...
command to every robot [30]. Adamek, Kitts, and Mas applied the same
...
hardware [31]. The side-angle-side (SAS) triangle parameterization used for
three- and four-robot vector field navigation in [14] is one instance of
...
reflectivity-based scalar field mats [32] to the current Decabot fleet
with HSV floor-color vector field encoding and OptiTrack motion capture
[33]; the simulations and stability results of this paper adopt the same
...
finite-time tracking and containment control of Sun et al. [34], point
...
controllers prioritize the geometric traversal guarantee over a
finite-time convergence rate. Where [14] used a proportional or orbital law
with no curvature, [15]'s Newton-Raphson source seeking uses the raw
...
tracking a spatial field [35], the modified Newton step introduced here is
...
In prior work [14], a three-robot cluster fit a linear model to each
...
Lyapunov exponent (FTLE) field [6, 7, 8]. Tracking an LCS directly is not
...
integration horizon shrinks [18]. This paper adopts the classical
...
parameter [21, 22] is $Q = \tfrac{1}{4}(s_n^2 + s_s^2 - \omega^2)$, and
...
member of the Shadden family [8], shifted from the canonical domain
...
the $3 \times 3$ matrix $\mathbf{A}$ of [14]. The coefficient vectors
...
center [15]. Their estimator combines the ring measurements with fixed
...
\begin{figure}[!t]
    ...
    (Corollary~\ref{cor:pentagon}, [15]). For formation control the six
...
symmetric estimator in [15]: smaller formations track curvature better.
...
Both controllers run inside the three-layer architecture of [14, 30]:
...
\begin{enumerate}
    ...
    \item \textbf{Robot dynamics.} Each robot obeys the first-order
          momentum model identified on the Decabot hardware [14],
          \begin{equation}
...
saddle-free Newton methods in optimization [35], replaces the signed
...
controller of [14], with the estimated boundary distance playing the
...
the robot-level limits of (\ref{eq:momentum}) are unchanged from [14].
...
the same separation was validated in hardware in [14]. Time is rescaled
...
is the sensor model of [9]. Position noise perturbs the location at
...
\begin{enumerate}
    ...
    \item \textbf{Straddle retention:} whether robots remain on both
          ...
          the stop, the failure count of [9].
...
of 1000 trials, Michini-comparable metric [9])}
...
offline over the same window in the style of [17]; the cluster's path
...
adaptive-navigation primitives [27], [28] likewise report local
