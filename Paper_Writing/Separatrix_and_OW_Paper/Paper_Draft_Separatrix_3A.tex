...
certificates, and Monte Carlo noise sweeps place both failure cliffs at
the estimator's own reliability threshold. Running unmodified on
...
\begin{enumerate}
    \item A distributed second-order estimator: six robots are proved
          ...
          a coupled two-component field, one more than suffices for a
          scalar field \cite{15}, and one synchronous sample yields
    ...
    \item Validation of both trackers under a fully specified noise
          model: Monte Carlo sweeps placing both failure cliffs at the
          estimator's own reliability threshold, a rotating-frame
...
field doubles the estimation problem, and Section~\ref{sec:estimation}
proves six robots, not five plus a center, is the exact minimum for the
coupled case. Deriving $D$ and $s_1$ by differentiating through the
...
eigenframe co-rotating), so the commanded velocity co-rotates as well
and the closed-loop trajectory is the same material path in any frame
(Section~\ref{sec:stab_oecs}). What the controller gives up relative to
...
exact estimates. Then its commanded velocity transforms as
$\boldsymbol{\delta}' = \mathbf{Q}\boldsymbol{\delta}$, and the
closed-loop navigation trajectory (\ref{eq:kinematic_model}) satisfies
$\mathbf{p}'_c(t) = \mathbf{Q}(t)\mathbf{p}_c(t)$ up to the
frame-transport term: the tracker follows the same material structure
in every frame.
\end{proposition}
...
mode the PARK step is the attract certificate of
Theorem~\ref{thm:attract} applied to the scalar $s_1$ in its gradient
form: componentwise saturated descent of a function whose minimum on
the segment is the TRAP core.
...
model~(\ref{eq:momentum}) with $\tau = 0.3$~s. The cluster layer uses
...
runs. Clean trials run several hundred to 1500 steps and stop at domain
exit or a fixed number of steps past first saddle contact; noise-sweep
...
\begin{equation}
    ...
    \label{eq:momentum}
\end{equation}
with $\alpha_{\text{mom}} = \exp(-\Delta t/\tau)$, actuator time
constant $\tau = 0.3$ s, control period $\Delta t = 0.1$ s (so
$\alpha_{\text{mom}} \approx 0.717$), speed limit
...
$\tau$ & 0.3 & -- & actuator time constant (s) \\
$\Delta t$ & 0.1 & 0.1 (= 600 s field) & control period (s) \\
$\alpha_{\text{mom}}$ & 0.717 & 0 & momentum coefficient $e^{-\Delta t/\tau}$ \\
$A$ & 0.1 & -- & double-gyre amplitude \\
$\rho$ & 0.075 & 0.0525 ($0.7\times$) & formation ring radius (m) \\
...
level doubled and re-swept until the worst-cell success rate falls
below 10\% so the failure cliff is always bracketed, and $\sigma_p \in
...
within 0.06) without formation collapse (an inter-robot distance
excursion beyond twice the nominal pair length); each trial also
...
(fraction of trials, the Michini-comparable metric \cite{9}). The $s_1$
sweep auto-extended to $\sigma_{uv} = 0.8$; rows beyond 0.2 sit on the
34--38\% random floor and are omitted.}
...
the parked position averages $-1.4$ over the $\sim$2.7~km footprint
...
SEEK mode throughout, a smoothing effect to fold into threshold
...
trials flag the weaker symptom, an inter-robot distance excursion
beyond twice the nominal pair length, as a formation collapse, and it
does not occur in any reported trial.
...
closed-form, heading-isotropic noise gains verified to $0.2\%$. The
...
follows a frame artifact (gap 1.219). Monte Carlo sweeps placed both
failure cliffs at the estimator's own reliability threshold, so the
estimator, not the control law, sets the noise ceiling. On real Santa
...
is $(x_c, y_c, \theta_c, p_1, \beta_1, q_1, L_2, \psi_2, L_3, \psi_3,
L_4, \psi_4)$, with pair 1's length fixed and $\psi_1 = \theta_c$ by
convention. The inverse kinematics reconstructs the midpoint triangle
...
Every branch of the selector is built from these quantities through
inner products, which are invariant
($\nabla' s_1'^\top \mathbf{e}_k' = \nabla s_1^\top \mathbf{e}_k$), and
scalar thresholds on invariants, so each branch fires identically in
both frames and outputs the rotated command
$\mathbf{Q}\boldsymbol{\delta}$. The one non-covariant input, the flow
