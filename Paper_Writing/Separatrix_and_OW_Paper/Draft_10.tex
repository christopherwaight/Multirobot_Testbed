...
horizon. We present a six-robot cluster that locates these
separating boundaries and navigates to and along them from the
instantaneous local flow measured simultaneously across the formation without requiring an integration horizon or pre-straddling.
...
when the robots lie on a common conic; a conic through all six nodes is
a nonzero quadratic vanishing at every sample, placing it in the null
space of $\boldsymbol{\Phi}$. For example, a standard ring formation is
degenerate at any size, being a conic by construction.
...
$564$. The inflation is not spread evenly across the derivative orders.
The pure second-order gains rise by about $56$ and the constant term by
$70$, while the first-order and mixed second-order gains are untouched
to three digits. Conic drift is confined to the curvature channels the
$D$ tracker reads and leaves the first-order channels the $s_1$ tracker
uses.

The fit is linear in the measurements, so the noise gains are the row
norms of $\boldsymbol{\Phi}^{-1}$. A coefficient of
derivative order $q$ has standard deviation $\gamma_q
\sigma_{\mathrm{eff}} / \rho^{\,q}$, so curvature scales as $\rho^{-2}$
where the gradient scales as $\rho^{-1}$. Table~\ref{tab:gain_ladder}
gives the ladder for the formation used here. The constants
$\gamma_q$ are formation specific and the exponent $-q$ is not. Within
second order the pure and mixed coefficients carry different constants,
a factor of two apart, so one scaling rule holds per derivative order,
not one gain per order.
...
\begin{table}[!t]
\caption{Noise gains, tabulated for the formation used here. The
constants $\gamma$ depend on the formation and are exact and independent
of ring radius; the $\rho^{-q}$ scaling with derivative order does not
depend on either.}
...
the previous paragraph relies on, leaving $E[\hat{a}_2\hat{b}_3] = a_2
b_3 + \operatorname{cov}(\hat{a}_2, \hat{b}_3)$ and a determinant bias
that scales with the local Jacobian. Unbiasedness belongs to the
measurement channel alone.
...
standard deviation from the ladder. So $\hat{s}_1$ reads a trench deeper
than it is, and the error grows without bound as the strain degenerates.
The tracker is drawn toward the one place its own eigenvector comparison
is least determined. 

\subsection{Polynomial Surrogates}
\label{sec:surrogates}

Both surrogates the trackers read are recovered from $\mathbf{J}$, the
first from its determinant $D = \det \mathbf{J}$ and the second from the
eigenvalues of its symmetric part. The Okubo-Weiss
...
One identity relates the two surrogates, and it follows from the
decomposition. For any two $2 \times 2$ matrices,
...
positive, so the surface is convex across the curve \cite{35}. The
...
the $s_1$ tracker, against the $\mathcal{O}(\rho)$ expected of a
quadratic fit to a field with nonzero third derivatives. The noise level at 50\% success grows as $\rho^{2.02}$ and
...
channels carry constants. Everything derived from $\hat{D}$ carries the additive observer term, and $\hat{\mathbf{H}}_D$ the truncation floor as well. The strain eigenvalue carries neither, and is exposed instead at $1/r$, a property of the strain field rather than of the sensing. Objectivity
