...
$\boldsymbol{\Phi}$, each column of order $q$ divided by $\rho^{\,q}$, since the raw quadratic columns shrink as $\rho^{2}$ and would report a healthy formation as a degenerate one. The nominal shape gives $\kappa(\boldsymbol{\Phi}) = 8.26$ at every
...
standard deviation. So $\hat{s}_1$ reads a trench deeper than it is.
...
$Q \propto D$ either way for incompressible flow, which covers the double gyre but not the ocean record of Section~\ref{sec:disc_ocean}. Its sign
...
Where the vorticity vanishes, trenches of $D$ in strain regions mark the strongest instantaneous attraction and repulsion.
...
derivatives vanish together and the two trenches coincide.
...
$x = \pm 1$, $y = \pm 0.5$.
So $s_1$ is a transverse trench on \emph{both} halves of the separatrix. What alternates by segment is the
...
\begin{figure}[!tb]
    ...
    Fig.~\ref{fig:trench_cross}. The separatrix at $x = 0$ is a trench on both halves, deepest at the saddles. The two halves differ in the material
...
test carries no such latch (Section~\ref{sec:disc_limits}). Each layer presents a fixed interface upward, so swapping
...
floor. That gain has units of inverse time, the step being $600$~s at the ocean operating point. Table~\ref{tab:params}
...
The state layer moves $\beta$ through two transitions. It starts at zero, so before the cluster reaches any structure the descent term homes onto the trench. First contact, when $\hat{s}_1$ first falls below $-s_{\mathrm{trim}}$, latches $\beta$ to one and the ride begins. Terminal capture returns it to zero, holding the cluster at a two-dimensional
...
$\lambda_1\lambda_2 \geq 0$ cannot fire. By
...
45.8\%, a clean gradient and eigenframe to the argmax to 100\%. The cluster settles in the wrong direction, a sign inversion.
...
for 28 hours, within 1.8 and 2.4~km of an FTLE ridge neither could see. A cluster that can certify its orientation should carry the $D$ tracker,
