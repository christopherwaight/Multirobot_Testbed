...
\begin{abstract}
Adaptive navigation is the process of modifying a robot's or a system of robot's motion in
...
of such structures either commit to the structure's identity and pre-straddles them before
tracking begins or accumulate an estimate over a finite integration
horizon. We present a six-robot cluster locates these
separating boundaries and navigates to and along them from the
instantaneous local flow measured simultaneously across the formation without requiring an integration horizon nor pre-straddling.
...
adaptive navigation primitive for both surrogates and characterize the tradeoff in terms of objectivity and noise tolerance. To our knowledge this is the
...
and they now operate across land, sea, air, and space \cite{1}.
 In adaptive
navigation (AN), a vehicle or group of vehicles modifies the direction and path travelled in real time
...
\cite{2,3}. An MRS, on the other hand, can collect spatially distributed readings at one instant, fit a
local model of the landscape, and steer on its features. \cite{4}
...
Neumann developed and experimentally verified extrema seeking, ridge and trench following, 
saddle seeking and front tracking using an indoor testbed \cite{2,3,6}.

Some environments are better modelled as vector fields, which contain features without scalar field equivalents. Critical points, such as sinks, sources, and spewing vortices can be used to model convergence zones and
...
Beyond critical points, vector fields have curves that are useful for navigation. For example, dynamic ocean flows contain naturally occurring boundaries known as coherent structures that influence where floating materials like debris and larvae will accumulate or separate \cite{12}. Mapping these exact curves allows teams to strategically deploy resources in highly targeted search areas \cite{13,14,15}. Without adaptive navigation, a large scale team must sample a much larger basin to determine where these structures lie to compute their location. \cite{9,10,11}.


Coherent structures are typically identified through two primary methods. The first relies on Lagrangian coherent structures (LCS) \cite{9}, typically computed as ridges in the finite-time Lyapunov exponent field \cite{11}, with ongoing research addressing the Lagrangian/Eulerian tradeoff and sparse-trajectory gradient recovery \cite{16,17,18,19,20}. The second method uses instantaneous Eulerian criteria, such as the Okubo-Weiss parameter, to partition flows based solely on the local velocity gradient \cite{21,22}. This instantaneous approach has been generalized to three dimensions \cite{23} and successfully applied to large-scale oceanographic eddy censuses \cite{24,25}. However, both of these methods characterize these structures offline using a reconstructed field, which are not well suited for real-time adaptive navigation of a field which evolves in time.  
...
on micro surface vehicles using a PIM triple inspired approach \cite{28,29}. The robots begin on the
...
\omega^2/4 - s_1^2$, off the same estimated model and navigate to the structure and rise along it without the need for pre-straddling and using only current instantaneous measurement. 

For the estimator, six robots are proved minimal. Its noise gains
...
prior, and this estimator does not impose the obvious one: horizontal surface
...
gains are untouched to three digits. Conic drift degrades the curvature
...
\sigma_{\mathrm{eff}} / \rho^{\,q}$, so curvature scales as $\rho^{-2}$
where the gradient scales as $\rho^{-1}$. Table~\ref{tab:gain_ladder} gives
...
read, robot count improves precision at the orders the geometry already
constrains, and radial diversity resolves the higher orders outright.
Robots added to a single ring drive the first-order gain down as $\sqrt{2/N}$ but leave the second-order gain near a floor of $2$, since $x^2 + y^2 = \rho^2$ identically on a ring and the quadratic basis is rank deficient there for any $N$. The ring-plus-center configurations that avoid this are design guidance, not exercised here.
...
during acquisition, but triggering the condition there is benign, since the
...
frame is undefined, which a general field will present and the law must
therefore cover. Off the band it reverts to the signed Newton step
...
scalar trench throughout. Neither needs a dedicated rule at the
...
(Fig.~\ref{fig:traverse_vs_logic_c}), and the split follows from the
model class. The $s_1$ tracker
...
and unavailable where the $D$ tracker would need one.
...
direction, a sign inversion rather than a decay. Holding the tangent whenever the argmax margin falls below the $\gamma_2\sigma_{\mathrm{eff}}/\rho^2$ the gain ladder predicts defers the decision rather than taking it at chance. That derived threshold raises far-saddle success from 44.6\% to 84.1\% at that level.
...
law's own prior output. The sign recursion therefore follows from the
objectivity requirement rather than from this particular law.
...
cross. The $s_1$ tracker is covered instead by
...
guarantee that carries to arbitrary flows. The record is also time-varying,
...
not binding: a $6 \times 6$ solve per cycle against a $600$~s ocean step.
...
capture test latch. The deeper exposure is the eigenvector comparison
itself, which remedies aimed at the seed do not reach; the margin-hold
...
leave (\ref{eq:hess_det}) traceless, so the fitted frame stays
indefinite by a symmetry of the field.
...
both frames, final gap 0.025. On Santa Barbara Channel currents both followed the same transport corridor for 28 hours, within 1.8 and 2.4~km of an FTLE ridge neither could see. Instantaneous criteria therefore resolve the corridor reliably and the trajectory within it unreliably. The operator's choice
...
tracker, which alone is objective.

The primitives are admittedly simple, reactive, and minimal in robot
count; even so, they hold a transport corridor for 28 hours on a measured
record.
