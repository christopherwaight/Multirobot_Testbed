...
terminal boundary layer. The second is met by treating the two eigendirections separately. The Newton
component is kept across the trench. Along it, only the magnitude is kept, and
the direction comes from the ambient flow.
...
\begin{equation}
    ...
    \frac{|\hat{D}_0|}{\|\hat{\mathbf{H}}_{D,0}\|_F}
        < \varepsilon_{\dim},
\end{equation}
a raw threshold on absolute well depth, or a curvature-normalized one measuring depth against the local curvature. The latter carries units of length squared, so the band scales with the landscape rather than the absolute size of $D$. Note that $\mathcal{B}$ is a band around $\{D = 0\}$, not around the
separatrix. On the benchmark the separatrix carries $D = -\pi^4 A^2$ along
almost all of its length. It enters the band only at the crest, the case the
branch is written for. The set
also contains the Okubo-Weiss boundary, which the cluster can cross
during acquisition. Triggering the condition there costs nothing, since the
...
frame is undefined, a case a general field will present and the law must
cover. Off the band it reverts to the signed Newton step
...
automatically through the origin. Continuity fixes the residual sign,
while the argmax supplies the direction.
...
law's own prior output. The sign recursion is a consequence of the
objectivity requirement, not of this particular law.
...
The added solid-body vorticity moves the whole $D'$ landscape, and the same rotation contaminates the flow measurement the on-band branch of the along-trench speed reads. Both terms of the $D$ law are steered by a frame artifact rather than by the field. No gain recovers this. The artifact is in the surrogate, and against $D$
...
The gain ladder places both surrogates on the same rung. The choice between
...
set by the estimator rather than by its own law. Under a rotating observer the
