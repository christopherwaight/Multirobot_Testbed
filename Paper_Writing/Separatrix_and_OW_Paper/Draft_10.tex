...
being informative (Section~\ref{sec:results_est}). The gradient is not the only input exhausted by then. Fig.~\ref{fig:est_accuracy}(b) puts the curvature eigenframe $39^\circ$ from truth at the cliff, against $45^\circ$ for a random axis, and both terms of the $D$ tracker are built on that frame. Neither threshold belongs to the control law, so the estimator sets the
...
The primitives allow a cluster of robots to hold a transport corridor for
28 hours on a measured record.
