...
Part D is housekeeping. Nothing here has been applied to the .tex. Numbers quoted here
still need author sign-off before entering any .tex.
...
- The convergence arguments use a_perp = kappa_perp / kappa_perp_hat without its size
  (see A2.1).
- The rho-sweep sentence in IV-E has no result.
...
1. The argument that an objective tracker must carry its tangent sign in memory, and
   ...
   demonstrates the mechanism. It is the most interesting idea in the paper and it sits
   inside a paragraph about the rho sweep.
...
- Give the rho-sweep sentence a number, or cut it.
...
- "Held there by truncation" (IV-C) and "the truncation floor" (IV-F) will be read as
  finite-radius truncation. It is model-order truncation.
- II-D says s1's transverse curvature "lives in third derivatives, outside a quadratic
  ...
  paper implies D has a usable Hessian and s1 does not; neither does. This undercuts
  IV-F's "Objectivity costs the Newton normalization": D's normalization is itself
  biased, bounded only by saturation.
...
- On the benchmark a_perp = 1/sin^2(pi y_f), about 45 at (0, 0.45). The Lyapunov
  ...
  Fig. 7 (S4, S6), which the text does not mention. Inference, not simulated. "Held to
  the trench at Newton rate throughout" does not hold near saddles.
...
- "The cliff and an estimator threshold nearly coincide" (0.0079 vs 0.0075) compares a
  ...
  likely coincidence. The Conclusion's "ceiling set by the estimator" rests on it.
...
### A2.3 The s1 bias does not "grow without bound" [verified]

Eq. (s1_bias) is a second-order expansion. The true bias saturates at
-sigma_r sqrt(pi/2), about -1.25 sigma_r, as r goes to 0. Monte Carlo: r/sigma = 1
gives -0.55 sigma (formula -0.50); r/sigma = 0.1 gives -1.16 sigma (formula -5.0). The
approximation blows up, not the bias.

### A2.4 The pentagon aliases cubic terms into the second-order coefficients [verified]
...
### A2.5 Table I units contradict 0.87 m/s [verified]
...
multiplies grad s1_hat, which is not a displacement. III-B's "The Newton and gradient
terms are displacements" is false for the s1 tracker.

### A2.6 The s1 latch arms almost everywhere [verified]
...
step one. The beta = 0 phase that "homes onto the trench from anywhere in the basin"
runs in about 11% of starts. Elsewhere acquisition is the ride plus projected descent,
with the tangent seeded off the structure. It works (six of six acquire), but the text
describes a different mechanism.

### A2.7 "No ridge to cross" is false [verified]

s1 is zero along the whole line y = 0. The s1 trench crosses that ridge at the origin,
rising to s1 = 0 exactly as D rises to its crest; Fig. 5(a) shows it as the yellow band.
At the origin s1's transverse curvature vanishes while D's is constant, so at the
crossing D is the better-defined trench. "Since s1 <= 0 everywhere" does not support the
claim, because D <= 0 on x = 0 too. Cut the ridge clause in II-E and the Fig. 2 caption.

### A2.8 "Objectivity forces memory" is scoped too broadly
...
### A2.9 Contradiction on tangent noise gain

IV-F: the s1 tangent "comes from the first-order coefficients, one order better in noise
gain" (4.5 against 41 degrees). IV-E shows the failure is the choice between
eigenvectors, decided by |a5_hat| > |a4_hat|, second-order coefficients, with
grad s1_hat at 44 degrees error. The axis set is first order; which axis is chosen, and
its sign, are second order. The sentence sits right before the result that s1 tolerates
four to five times less noise.

### A2.10 The objectivity headline rests on one trial, and the text contradicts the figure
...
- Fig. 8(b) does not match "leaves the structure ... ending in the gyre interior." The
  dashed run drifts slightly west in the upper half, reaches p1*, then takes the
  opposite branch along the bottom wall trench, ending near (0.4, -0.6). The branch
  choice is consistent with the transport term Omega(-y, x) = (+0.1, 0) at p1* flipping
  the flow sign that orients w1 every cycle. That is the same failure the appendix
  analyzes for the s1 seed, applied every cycle, and it is reopened item E7 (Part B)
  showing up in the paper's own figure. "Does not degrade. It leaves the structure"
...
### A2.11 Only rotation is tested

J is Galilean invariant, but both trackers read the raw velocity for a sign (D's w1 every
cycle, s1's seed). A velocity-reference bias makes the cluster a translating observer
and perturbs both. One sentence in Limitations.

### A2.12 The ocean validation has no null baseline
...
rho-sweep metric and primitive).
...
- Straddle retention is plotted, never quantified, and contradicts the text. At
  sigma_uv = 0.002 retention is 23% against 44% success, so about half the successful
  runs lost the straddle at least once. The text says "The cluster holds the
  separatrix."
- Success falls toward 0%, not 50%. A single seed flip would plateau near 50%. Repeated
  ...
  "a sign inversion rather than a decay" undersells it.
- Tracking error versus noise is recorded, not reported (old R1 item 26). The rho sweep
  reports "monotonic" with no quantity.
...
| Final gaps 1.219 / 0.025 | Conclusion | Numbers match; "departs," "gyre interior" contradicted by Fig. 8(b) |
| "within 1.8 and 2.4 km" | Conclusion | These are means; "within" reads as a bound |
| "acquire, ride, and hold station" | Conclusion | D capture never fires; s1 never captures on the ocean |
| "ceiling set by the estimator" | Conclusion | Rests on A2.2 |
| "slightly different trajectories" / "separate only near landfall" | Abstract, IV-G | Fig. 9 s1 path runs about 5 km west early and ends near the western island, not "closing on" the middle one |
| 0.87 m/s | IV-B | Only true with per-time-unit c_max (A2.5) |
...
- Dangling: Limitations cites "the margin-hold rule of Section IV-E." The rule was in
  Draft 8a's IV-E and has been cut; the reference survived.
- "The cluster stays within the tube" (III-C): no tube is defined.
- III-A says Section IV-H "reports what that costs" for band chatter; IV-H asserts it.
- "Two situations clear the ride flag" (III-D) lists a non-clearing and a latch that
  sets it.
- "Tangent, not merely coincident": coincident is the stronger claim; the phrase reads
  inverted.
- "Trenches of D in strain regions mark the strongest instantaneous attraction" (II-D)
  holds only where omega = 0.
- "The Jacobian determinant traces separatrices" (title, abstract) holds only for
  vorticity-free trench networks.
- "Q proportional to D ... for the incompressible flows here" does not cover the ocean.
- "Have a separate cause" (separate from what?), and "which channels carry constants"
  (unparseable).
- kappa is normalized by dividing by rho^q, not scaling by it (scaling gives 2.4e5 at
  rho = 0.075). Raw quadratic columns carry rho^+2, not rho^-2.
...
- "-120.39 deg W" is a double negative (twice). "Core wells" reads as gyre cores.
  "Robots' orientation" conflicts with omnidirectional point masses.
...
| II-E | B- | Concrete except "signs its segments" and the ridge claim |
| III-A | B | 75-word list sentence; latch used before defined |
| III-B | C | Units; "implicit per-step time unit absorbed into k" |
| III-C | C | "Height-ridge sense," "tube," "Newton rate" unglossed; convergence compressed to assertion |
| III-D | C+ | Figure helps but is unreferenced; flag logic inverted |
| IV-A/B | B | Checkable setup |
| IV-C | C+ | Number-dense, fragments ("An individual formation orientation is.") |
| IV-D | C | "Neither a noise floor nor a tuning margin," "watershed ... composited with the flow direction" |
| IV-E | C- | Grammar slip ("with no noise, which then separates them"); abrupt jump from rho sweep to a design principle |
| IV-F | C | "Does not degrade. It leaves"; "carry constants" |
| IV-G | B- | Clear setup, over-compressed close |
| IV-H | C+ | "Exposed twice over," dangling reference |
| Conclusion | B- | Direct; overclaims; ends on a flourish |
...
contributes a novel approach to the problem." "Six robots are proved minimal." "No gain
recovers this." (false if Omega is known: D' - Omega omega - Omega^2 recovers D). "The
sign recursion is a consequence of the objectivity requirement, not of this particular
design." (A2.8). "Switching surrogates costs one function call." "It alone is
objective." (the seed is not). "Instantaneous criteria therefore resolve the corridor
reliably." (one record; IV-G says so). Fig. 4 caption: "what is abrupt is the outcome it
selects between."
...
compressed and aphoristic: restating closers ("No gain recovers this," "The trial
reverses the ranking," "It alone is objective"), reversals ("forced rather than chosen,"
"not one of noise gain, but of which channels carry constants"), and math objects given
agency ("carry," "read," "exposed," "commit to" branches). If told the whole paper came
from a strong model, I would believe it for II-C through the Appendix, not for I through
II-B. The house rules remove the surface tells, so what remains is structural. The fix
is to unpack each aphorism into a number or mechanism, or delete it. Check IEEE's
current wording on disclosing AI-generated text in the acknowledgments.
...
**Missing limitations.** No sensor model (A2.13). H_D_hat biased at any radius (A2.1).
Only rotation tested; flow-sign steps are not Galilean invariant (A2.11). s1 not fully
objective (the seed), omitted from abstract and conclusion. A 5.4 km radius means links
of about 11 km, so "ample bandwidth" is an assumption at that scale. Temporal filtering
across cycles is never named or excluded; filtering coefficients across cycles is still
instantaneous in the field sense.
...
point (A2.2); the objectivity headline rests on one trial whose text contradicts its
figure, and the Introduction never motivates it (A2.10); one false math statement
(A2.3), one self-contradicted citation claim (A2.4), a units error (A2.5), a dangling
reference, and two unreferenced figures. None needs new theory; most is rewriting.
...
|---|---|---|---|
| E1 / C7 | kappa on mixed normalizations (8.26 and 564 normalized, 688 raw) | II-C states radius-normalized 8.26 and 564; IV-G reports 8.26 | **Closed.** Residual: the normalization sentence has the direction backwards (A3 clarity list). |
...
| D terminal test | Section III gave the D tracker no terminal condition | Eq. (d_capture), lambda_1 lambda_2 >= 0, in III-C | **Closed, with a new defect.** III-C: "A saddle of D where two trench segments cross is such a point." A flow saddle where two trench segments cross is a local minimum of D (true H_D = 2 pi^6 A^2 I there), not a saddle of D. The test fires on a minimum. |
| "straddle retention" | Undefined | Defined in IV-E | **Closed.** Residual: never quantified, and the plotted values contradict "the cluster holds the separatrix" (A2 test plan). |
...
mismatch moved). C6 is still live in the abstract (A2.10). Figure hygiene is still live
(Part D).
...
- **The rho sweep** (`rho_sweep_findings.md`, kept in this folder). 25,600 trials; no
  ...
  0.94 against a predicted 1, 2.02 and 1.95 against a predicted 2. Draft 10 carries one
  sentence of it in IV-E. The full result is a gain-ladder result and most likely the
  seed of the next paper.
