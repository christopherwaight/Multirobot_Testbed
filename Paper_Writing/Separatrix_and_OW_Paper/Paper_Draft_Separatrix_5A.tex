...
and on real Santa Barbara Channel radar currents; hardware validation
is future work (Section~\ref{sec:future}).
...
attracting and repelling segments meet, to the nearest TRAP core, using
only objective quantities throughout.
...
(\ref{eq:tangent_select}): $\hat{d}_1, \hat{d}_2$ are built from
$\nabla\hat{s}_1$ and the co-rotating eigenframe alone, so the
selected eigenvector and the sign it inherits from continuity both
co-rotate with $\mathbf{Q}$, unlike a rule that instead resolved the
same choice by projection onto the flow at every step, which would
inherit $\dot{\mathbf{Q}}\mathbf{Q}^\top\mathbf{p}'$, a term with no
counterpart in the inertial frame, at every step rather than only at
the seed. The ACQUIRE branch saturates $\nabla\hat{s}_1$ componentwise
in observer axes and is covariant only where the saturation is
inactive; it descends the invariant $\hat{s}_1$ in every frame, so its
contribution is a frame-dependent transient that the covariant
contraction of (\ref{eq:oecs_n_dynamics}) absorbs. CROSS reads the
flow directly and so is covariant only under a fixed observer
rotation, not a time-dependent one; it is reached only on the first
tracking step under an already-degenerate eigenframe with no prior
tangent, essentially never in practice
...
\end{equation}
with equality only at a critical point of $\hat{s}_1$. This is the same
descent ACQUIRE and PARK apply, so all three share
(\ref{eq:s1_lyapunov}); what distinguishes CAPTURE is only its trigger,
$\lVert\nabla\hat{s}_1\rVert < g_{\text{capture}}$, which fires at any
network minimum the ride reaches rather than a single field-specific
one. Unlike the $D$ tracker's own capture test, whose reliance on
$\hat{\mathbf{H}}_D$ Remark~\ref{rem:capture_gap} shows to be unreliable
exactly at a saddle, (\ref{eq:s1_lyapunov}) needs no curvature estimate
at all: the saddle is recoverable as a two-dimensional minimum of the
objective field $\hat{s}_1$ using only the gradient the estimator
already certifies, and Section~\ref{sec:results_oecs} confirms the test
fires reliably where the $D$ tracker's does not.
...
The $D$ tracker's terminal target is fixed by design, the bottom saddle
$\mathbf{p}^*_1$ that its selector's FLOW step always continues toward
from this start. The $s_1$ tracker's is not fixed the same way: which
saddle its flow-seeded tangent commits to
(Section~\ref{sec:oecs_controller}) depends on the local flow direction
at $(0, 0.35)$, so success is scored as contact with either saddle. The
on-segment path to the nearer saddle, $\mathbf{p}^*_2$, is much shorter
than the traverse to $\mathbf{p}^*_1$ (0.15 against 0.85), so a trial
that commits to the near saddle completes in a fraction of the steps a
trial committing to the far one needs; success rates compare cell by
cell only qualitatively for this reason as well as the noise model's own
differences. The prediction under test is the location of the failure
cliff, which the estimator analysis places at $\sigma_{uv} \approx 0.01$
for both controllers, while straddle retention and tracking error test
whether the first-order eigenframe buys measurable robustness on the
structure.
...
(a) $D$ tracker traverse success. (b) $s_1$ tracker success; the
transition falls entirely between $\sigma_{uv} = 0.001$ and $0.005$ at
every $\sigma_p$ shown, resolved at 0.0005 spacing in
...
\hspace{1.5em}
\begin{tabular}{r|rrrrr}
\multicolumn{6}{l}{(b) $s_1$ tracker: $\mathbf{p}^*_1$ success (\%)} \\
\hline
$\sigma_{uv}$ \textbackslash\ $\sigma_p$ & 0.000 & 0.005 & 0.010 & 0.020 & 0.050 \\
\hline
...
Table~\ref{tab:mc_success}(b) reports capture success for the $s_1$
