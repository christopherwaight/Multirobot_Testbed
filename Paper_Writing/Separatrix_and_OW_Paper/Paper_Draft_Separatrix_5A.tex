...
{Anonymous Authors: Multirobot Tracking of Instantaneous Coherent Structures}
...
point. In \cite{14}, a three-robot cluster estimated the location
and type of an isolated critical point from a linear fit of
simultaneous local velocity measurements. Coordinated
...
paper; objectivity and the Lagrangian/Eulerian tradeoff that motivate
this paper's two surrogates are developed in
Section~\ref{sec:detj_field}, with the underlying theory in
\cite{16,17,18}; velocity-gradient recovery from sparse trajectories
\cite{19} and the strain-spin decomposition itself \cite{20} remain
active adjacent directions. The instantaneous criteria remain the
...
All quantities of interest have closed forms. Writing $s_x = \sin\pi
x_f$, $c_x = \cos\pi x_f$, $s_y = \sin\pi y_f$, $c_y = \cos\pi y_f$,
the Jacobian entries are $u_x = -\pi^2 A c_x c_y = -v_y$ and $u_y =
\pi^2 A s_x s_y = -v_x$, so the flow is divergence free and direct
differentiation of (\ref{eq:dg_u}) with the half-angle identities
gives
\begin{equation}
...
\end{equation}
with vorticity $\omega = -2\pi^2 A s_x s_y$. Three facts follow
directly. First, the Okubo-Weiss boundary is the diamond
...
coordinate axes away from the degeneracy lines $c_x c_y = 0$, giving
\begin{equation}
    s_1 = -\pi^2 A\,\lvert\cos \pi x_f \cos \pi y_f\rvert,
    \label{eq:s1_analytic_main}
\end{equation}
with TRAP cores, the minima $s_1 = -\pi^2 A$, at the six flow saddles.
The attracting/repelling identity alternates by segment (the upper
separatrix is an attracting OECS, the lower half repelling, its
attracting partner the bottom wall), meeting at the degeneracy lines
$x = \pm 0.5$, $y = 0$ where $\mathbf{S} = \mathbf{0}$: the $D$ crest
at the origin is also an isotropic point of $\mathbf{S}$, where the
attracting segment terminates in the Serra-Haller sense, so an
objective tracker riding it should stop there. The vanishing shear
makes the double gyre nongeneric, with degeneracy lines in place of
the isolated singular points of a generic strain field \cite{17}; the
ocean field of Section~\ref{sec:ocean_sim} is generic.
...
$\det(\boldsymbol{\Phi})$. The contrast is measurable: under per-robot
position scatter of $0.1\rho$ at zero measurement noise, the
fixed-weight recovery's deviatoric $\hat{\mathbf{H}}_D$ scatter is 250
times that of the measured-position fit and its strain eigenframe
error averages $44^\circ$ against $2.8^\circ$, while at
$\sigma_{uv} = 0.01$ both recoveries are noise-limited and
indistinguishable; the measured-position fit costs one $6 \times 6$
factorization per cycle and buys deformation robustness.
...
two effects for the second-order coefficients gives an optimal radius
\begin{equation}
...
with the effective-noise argument of Section~\ref{sec:noise_model}
...
is 0.008, four times the $D$ tracker's 0.002, the gradient-rate cost
predicted by Remark~\ref{rem:s1hess}. Everything else moves in the
$s_1$ tracker's favor: the 50\% level is not reached until
$\sigma_{uv} \approx 0.1$, roughly an order of magnitude past the $D$
tracker's cliff, and beyond it success settles onto a floor rather
than collapsing, since the short on-segment path lets a noise-driven
walk reach the 0.06 contact disk by chance. Straddle retention is free
of that floor and places the sharp degradation two to four times
higher in $\sigma_{uv}$ than the $D$ tracker's cliff. Position noise is
where the first-order eigenframe shows most clearly, leaving the $s_1$
tracker's success an order of magnitude above the $D$ tracker's at the
same $\sigma_p$. Part of the margin is the shorter task, inherent to
...
\bibitem{14} Anonymous, "Adaptive navigation of multirobot systems to
critical points in 2D vector fields," submitted for publication.
