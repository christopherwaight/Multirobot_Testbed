...
field, and down the opposing branch. Because this surrogate, like every
velocity-gradient invariant, is not objective, we additionally derive a
second controller from the symmetric rate-of-strain tensor, whose
eigenstructure is frame-invariant: it acquires and tracks the attracting
OECSs directly, and a rotating-frame experiment demonstrates that it
recovers the same material curve in any observer frame while the
surrogate-based tracker follows a frame artifact. The strategy is
...
\begin{enumerate}
    ...
    \item An objective second controller that tracks attracting OECSs
          directly from the rate-of-strain tensor
          $\mathbf{S} = (\mathbf{J} + \mathbf{J}^\top)/2$, whose
          eigenstructure the same six-robot fit delivers at
          first-order noise gains. Two structural results accompany it:
          the Hessian of the smaller strain eigenvalue under any
          quadratic velocity fit is negative semidefinite, so a
          Newton-type trench snap is unavailable in principle and the
          tracker reads its frame from the strain eigenvectors instead;
          and a rotating-frame experiment demonstrating that the tracker
          recovers the same material structure in any observer frame,
          while the $\det(\mathbf{J})$ tracker follows a frame artifact.
...
adaptive controllers, the separatrix tracker and the objective OECS
tracker.
...
computed offline from ocean radar data in Section~\ref{sec:results}. The
objectivity gap itself is then closed rather than merely conceded: a
second controller (Section~\ref{sec:oecs_controller}) is built entirely
from the rate-of-strain tensor of Section~\ref{sec:strain_field}, tracks
the attracting OECSs of \cite{17} directly, and is demonstrated to be
frame-invariant in Section~\ref{sec:results}; the price it pays relative
to the $D$-based tracker is quantified there as well.
...
\subsection{The Rate-of-Strain Tensor and Attracting OECSs}
...
$\mathbf{W} = (\mathbf{J} - \mathbf{J}^\top)/2$. Under a time-dependent
Euclidean change of observer $\mathbf{p}' = \mathbf{Q}(t)\mathbf{p} +
\mathbf{b}(t)$, the velocity gradient transforms as $\mathbf{J}' =
\mathbf{Q}\mathbf{J}\mathbf{Q}^\top + \dot{\mathbf{Q}}\mathbf{Q}^\top$:
the frame's own rotation contaminates the antisymmetric part, and with
it the vorticity, $D$, and every Okubo-Weiss-type invariant. For a
trace-free $\mathbf{J}$ and a frame spinning at rate $\Omega$ the
contamination is explicit,
\begin{equation}
    D' = D + \Omega\,\omega + \Omega^2,
    \label{eq:D_not_objective}
\end{equation}
so two observers rotating relative to one another disagree about where
the $D$ structures are. The symmetric part is different:
$\mathbf{S}' = \mathbf{Q}\mathbf{S}\mathbf{Q}^\top$ exactly, so the
eigenvalues $s_1 \leq s_2$ of $\mathbf{S}$ are frame-invariant and its
eigenvectors $\mathbf{e}_1, \mathbf{e}_2$ co-rotate with the frame.
Quantities built from the eigenstructure of $\mathbf{S}$ are objective;
material behavior, which cannot depend on the observer, can therefore be
...
(incompressibility makes $\operatorname{tr}\mathbf{S} = 0$). Serra and
Haller's attracting OECSs, operationalized as transient attracting
profiles (TRAPs) for search-and-rescue prediction \cite{17,36}, are
built from exactly this structure: a TRAP core is a local minimum of
$s_1$ with $s_1 < 0$, the strongest instantaneous attraction in its
neighborhood, and the attracting curve through the core is tangent to
the stretching eigenvector $\mathbf{e}_2$ with normal $\mathbf{e}_1$,
terminated where attraction stops dominating ($s_1$ no longer negative
or no longer a transverse minimum). In the language already introduced,
attracting OECSs are trenches of the signed scalar field $s_1$, ridden
along $\mathbf{e}_2$; the objective tracker of
Section~\ref{sec:oecs_controller} does to $s_1$ what the separatrix
tracker does to $D$, with one structural difference in what the
estimator can supply (Section~\ref{sec:s1_estimation}). On the
separatrix of the steady double gyre the two surrogates agree exactly:
the vorticity vanishes there, $\mathbf{J} = \mathbf{S}$, and
$s_1 = -\sqrt{-D}$; off such symmetry lines, and in any rotating frame,
they part ways, and only $s_1$ keeps reporting the material truth. One
caution accompanies the eigenstructure: $\mathbf{e}_1, \mathbf{e}_2$
form direction fields with no globally consistent sign, undefined at
...
\subsubsection{The strain field shares the grid but signs its segments}
The double gyre's shear strain vanishes identically,
$(\partial u/\partial y + \partial v/\partial x)/2 = 0$, so $\mathbf{S}$
is diagonal everywhere and (Appendix~\ref{app:analytic})
\begin{equation}
...
The trenches of $s_1$ form the same grid as the trenches of $D$, and the
TRAP cores, the minima $s_1 = -\pi^2 A$, sit exactly at the six flow
saddles on the walls. But the attracting/repelling identity of the
segments alternates: on the upper half of the separatrix ($0 < y <
0.5$) the compression eigenvector $\mathbf{e}_1$ is transverse to the
line, so that segment is an attracting OECS (the unstable manifold of
the top saddle, onto which material collapses); on the lower half the
eigenvectors swap identity, the segment is repelling, and the attracting
structure of the lower region is the bottom wall. The segments meet at
the degeneracy lines $x = \pm 0.5$ and $y = 0$, where $\mathbf{S} =
\mathbf{0}$ and $s_1 = 0$: the crest of the $D$ trench at the origin is
also an isotropic point of $\mathbf{S}$, where the attracting segment
terminates in the Serra-Haller sense. An objective tracker riding the
upper separatrix therefore should not sail through the crest; the
structure it is tracking ends there. Because the shear strain vanishes,
the double gyre is nongeneric in one respect: its degeneracies form
lines rather than the isolated wedge and trisector points of a generic
strain field \cite{17}, which the ocean field of
Section~\ref{sec:ocean_sim} does exhibit.
...
$g_\perp$ & 1.0 & m$^{2}$ & OECS $s_1$-descent gain \\
$s_{\text{trim}}$ & 0.05 & s$^{-1}$ & OECS attraction-dominance threshold \\
$s_{\text{capture}}$ & $-0.9$ & s$^{-1}$ & OECS core-park threshold (core-seek) \\
$\varepsilon_{\text{degen}}$ & $10^{-3}$ & s$^{-1}$ & OECS eigenframe degeneracy threshold \\
\hline
\end{tabular}
\end{table}
...
\subsubsection{OECS tracker sweep}
% Sweep implementation: experiments/mc_sweep_oecs.py (VF_Robot);
% per-trial rows in experiments/outputs/mc_oecs/trials_fixed.csv.
The OECS tracker is swept over the same noise grid, from the same
straddling start $(0, 0.35)$ with the same random-heading draw, in
core-seek mode. The terminal target differs by necessity: the
attracting segment through that start ends at its TRAP core, the top
saddle $\mathbf{p}^*_2 = (0, 0.5)$, so task success is reaching
$\mathbf{p}^*_2$ within 0.06 without formation collapse. The on-segment
path is shorter than the Logic C traverse (0.15 against 0.85), so
success rates are compared cell by cell only qualitatively; the
...
both controllers, since $\nabla\hat{s}_1$ and $\nabla\hat{D}$ share the
second-order coefficient noise gain, while the straddle-retention and
tracking-error columns test whether the first-order eigenframe
(Section~\ref{sec:s1_estimation}) buys measurable robustness on the
structure.

\subsubsection{Objectivity experiment}
% Implementation: experiments/oecs_objectivity_demo.py;
% figure figures/oecs_objectivity.png, CSV outputs/oecs/.
Both controllers are run on the same physical double gyre expressed in
two observer frames: the inertial frame, and a frame rotating at
$\Omega = 0.2$ rad/s, in which the field is
$\mathbf{v}'(\mathbf{p}', t) = \mathbf{Q}\mathbf{v}(\mathbf{Q}^\top
\mathbf{p}') + \Omega\,(-y', x')^\top$. The added solid-body vorticity
$2\Omega = 0.4$ is 20\% of the gyre-core vorticity $2\pi^2 A$ and
dominates the near-zero local vorticity at the separatrix; the rate was
chosen so the structure's apparent speed at the tracked radius,
$\Omega r \leq 0.10$, stays inside the command authority $k\,v_{\max} =
0.12$. Rotating-frame trajectories are pulled back to inertial
coordinates through $\mathbf{Q}^\top(t)$ and compared with the
inertial-frame run of the same controller: reported metrics are the
final and mean pointwise gap between the two runs over their common
steps, and each run's mean distance to the true trench network over the
post-acquisition phase. Proposition~\ref{prop:equivariance} predicts
near-zero gaps for the OECS tracker and offers no such protection to
Logic C, whose surrogate obeys (\ref{eq:D_not_objective}).

\subsubsection{Ocean TRAP protocol}
The OECS tracker is to be run on the 2~km Santa Barbara Channel field
of Section~\ref{sec:ocean_sim} at the same operating point as the
separatrix tracker (Table~\ref{tab:ocean_params}), in core-seek mode
from the same validated start. Ground truth is computed offline from
the full radar grid in the style of \cite{17,36}: $s_1$ and its
eigenframe on the grid at each hourly frame, TRAP cores as local minima
of $s_1$, and TRAP curves integrated as $\mathbf{e}_2$ tensor lines
from each core until $s_1$ loses transverse minimality. The comparison
metrics are the distance from the tracked path to the nearest offline
TRAP curve over the trial, and the distance from the parked position to
the nearest TRAP core, against the same 1.4~km start-to-ridge scale
used in the FTLE comparison of Section~\ref{sec:results_ocean}. Unlike
the double gyre, the ocean strain field is generic: its degeneracies
are isolated points and its TRAP cores move between frames, so this
experiment additionally exercises the continuity-orientation rule and
the TRIM test on a live field.
...
\subsection{OECS Tracker: Double Gyre and Objectivity}
...
Fig.~\ref{fig:oecs_objectivity} shows the experiment of
Section~\ref{sec:test_plan}: both controllers, both frames, one start,
rotating-frame trajectories pulled back to inertial coordinates. The
...
trench network of 0.001 and 0.005 respectively. Logic C's two runs are
not: the rotating observer's $D'$ landscape (\ref{eq:D_not_objective})
...
Table~\ref{tab:oecs_success} reports core-arrival success for the OECS
tracker over the noise grid of Section~\ref{sec:test_plan}. The
zero-noise tracking error is 0.008, four times Logic C's 0.002, the
gradient-rate cost predicted by Remark~\ref{rem:s1hess}. Everything
else moves in the tracker's favor. At $\sigma_{uv} = 0.01$, the level
that halves the Logic C traverse, core arrival still succeeds in 94.3\%
of trials; the 50\% level is not reached until $\sigma_{uv} \approx
0.1$, and beyond it success settles onto a floor of 34--38\% (measured
out to $\sigma_{uv} = 0.8$) rather than collapsing, since the short
on-segment path lets a noise-driven walk reach the 0.06 contact disk by
chance. The straddle-retention column is free of that floor and places
the sharp degradation between $\sigma_{uv} = 0.01$ and $0.02$, still
two to four times the Logic C cliff at fixed start
(Table~\ref{tab:sep_straddle}). Position noise is where the
first-order eigenframe shows most clearly: $\sigma_p = 0.05$ alone,
which leaves Logic C at 18.8\%, leaves the OECS tracker at 79.6\%. Part
of the margin is the shorter task (0.15 against 0.85 of on-structure
path), which the test plan accepts as inherent to the objective
geometry; the part that is not task length is visible per cell in the
tracking-error columns, which degrade smoothly where Logic C's jump.
The mechanism is the one Section~\ref{sec:s1_estimation} predicts: the
tangent direction survives noise an order of magnitude past the point
where every gradient-based quantity approaches chance, so a noisy OECS
team keeps riding the right line even while its transverse regulation
coarsens.

\begin{table}[!t]
\centering
\caption{OECS tracker (core-seek): success rate (core arrival) by noise
cell, 1000 trials/cell, fixed straddling start $(0, 0.35)$, target
$\mathbf{p}^*_2$}
\label{tab:oecs_success}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{r|rrrrr}
\hline
...
point of Table~\ref{tab:ocean_params}, from the same start as the
separatrix run, with the strain thresholds set from the field's own
$s_1$ scale ($s_{\text{trim}} = 0.3$, $s_{\text{capture}} = -3.0$
against a domain median of $-0.73$). Fig.~\ref{fig:ocean_oecs} shows
the outcome of the first execution of the ocean protocol of
Section~\ref{sec:test_plan}: the team converges to a local minimum of
...
work under the protocol of Section~\ref{sec:test_plan}.
...
The two controllers of this paper are not competitors so much as the
two sides of a tradeoff that Section~\ref{sec:intro} inherited from the
coherent-structure literature. The $D$ tracker exploits everything the
instantaneous velocity field offers, including the flow direction
itself: its flow-assisted mode is what carries it through the crest,
...
by Fig.~\ref{fig:oecs_objectivity}, is that its structure is an
artifact of the frame the velocities are reported in: a rotating
observer, a drifting reference, or any platform whose navigation frame
turns relative to the Earth sees a different $D$ landscape
(\ref{eq:D_not_objective}) and tracks a curve no material particle
follows. The OECS tracker refuses every frame-dependent input, and what
it recovers is the guarantee that matters for material prediction: the
...
Its structures are bounded segments that end at degeneracies, so there
is no crest crossing and no network traversal; its transverse
convergence is gradient-rate, since Remark~\ref{rem:s1hess} shows no
quadratic fit can supply the curvature a Newton snap needs (the
measured zero-noise tracking error, 0.008 against Logic C's 0.002,
is this cost made visible); and its tangent must be oriented by
continuity, since a direction field admits no global sign. On the
double gyre the two trackers agree exactly where their surrogates
coincide, on the separatrix, where $\omega = 0$ forces $s_1 =
-\sqrt{-D}$; a mission that needs long-range structure traversal in a
...
drifting platform, by the OECS tracker. Both consume the same fit,
so the selection is one function call, not a new estimator or
formation.
...
$\mathbf{S}$ approaches isotropy: the angular error of
$\mathbf{e}_1, \mathbf{e}_2$ scales as the coefficient noise divided by
the eigenvalue gap $2r$, so near a degeneracy ($r \to 0$) the frame
direction approaches chance even though $s_1$ itself remains
well-estimated. This is a property of the physics, not of the
estimator: the tensor lines themselves are singular there
...
tangent-sign flip when a trial passes close to a degeneracy while
tracking, which the continuity rule damps but cannot exclude. On the
double gyre this region is confined to the known degeneracy lines; on a
generic field the isolated degenerate points move between frames of the
data, which is one reason the ocean protocol of
Section~\ref{sec:test_plan} logs tangent-sign flips explicitly.
...
A second, objective controller was derived from the rate-of-strain
tensor recovered by the same fit, tracking attracting OECSs from the
strain eigenframe and the $s_1$ gradient after a structural result
showed that no quadratic velocity fit can supply a usable $s_1$
Hessian. In a rotating-frame experiment the OECS tracker recovered the
same material curve in both observer frames (final gap 0.020) while the
$\det(\mathbf{J})$ tracker followed a frame artifact (final gap 1.219),
demonstrating the objectivity claim in closed loop rather than
asserting it.
...
The rate-of-strain tensor is diagonal everywhere: the shear strain is
$\tfrac{1}{2}(\partial u/\partial y + \partial v/\partial x) =
\tfrac{1}{2}(\pi^2 A s_x s_y - \pi^2 A s_x s_y) = 0$, so $\mathbf{S} =
\mathrm{diag}(\partial u/\partial x,\; \partial v/\partial y) =
\mathrm{diag}(u_x, -u_x)$ with $u_x = -\pi^2 A c_x c_y$. Its
eigenvalues are $\pm|u_x|$, giving
(\ref{eq:s1_analytic_main}), $s_1 = -\pi^2 A |c_x c_y|$, with
eigenvectors on the coordinate axes: the compression direction is
$\hat{\mathbf{x}}$ where $u_x < 0$ and $\hat{\mathbf{y}}$ where $u_x >
0$, which is the segment-identity alternation of
Section~\ref{sec:dg_example}. $\mathbf{S}$ is degenerate exactly where
$c_x c_y = 0$, the lines $x_f \in \tfrac{1}{2} + \mathbb{Z}$ and $y_f
