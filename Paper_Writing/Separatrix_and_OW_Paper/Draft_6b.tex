...
The remainder of the paper reviews related work, formulates the problem
(Section~\ref{sec:detj_field}), develops the estimator
(Section~\ref{sec:estimation}) and the two controllers
(Section~\ref{sec:controllers}), and reports the simulation program
(Section~\ref{sec:methods}) and its results
...
estimator fits the unconstrained model deliberately. Horizontal surface
currents are not divergence free, and the divergence is nonzero over the
...
defensible; the sixth robot buys exactness over every planar quadratic
field rather than only over an incompressible subclass.
...
nominal shape value. Each robot is an omnidirectional point mass.
...
at its draw, and the reported ring-phase-dependent quantities are with
respect to it. Clean trials run 400 to 600
...
$|x_c|$). Where the $s_1$ tracker fails, the sweeps supply noise-free
inputs to individual channels to isolate the mechanism. The $s_1$ sweep
widens the exit box to
...
across phases is the orientation dependence of
...
a tuning margin. By Section~\ref{sec:sep_controller} the
...
$\|\mathbf{J}\| \approx 1$ at this test point. The team has not lost the
structure, become erratic, or
...
period $72^\circ$ and changes sign at each mirror-symmetric phase, so the
magnitude $|x_c|$ that the closed loop actually reports carries an
...
edge of its authority rather than inside it; the $D$ tracker reaches
that cap exactly, 0.87~m/s peak against a 0.71~m/s mean. The formation
stays far from the conic degeneracy of Remark~\ref{rem:conic} for both
trackers, at $\kappa(\boldsymbol{\Phi}) = 688$ throughout the corridor
...
unmodified controller behaves as a traverser. Which of the two behaviors
...
fails by decay rather than by inversion. Under a rotating observer
...
is the defense once the ride is underway, and the residual risk of a
wrong branch at a crossing under heavy noise is damped but not excluded
by the latch on (\ref{eq:capture_test}).

The $s_1$ tracker's tangent rule is the sharpest of the design
limitations. Its exposure is the eigenvector comparison rather than the
seed (Section~\ref{sec:disc_noise}), so remedies aimed at the seed do
not reach it. Holding the tangent whenever the argmax margin
...
flow whenever the selected axis changes, and, for a team with a trusted
heading, resolving the sign against a world axis every step.
...
Four extensions remain: a time-derivative correction term to extend the
stability results to time-varying fields with a quantified bound; online
formation reshaping to protect the conditioning of
$\boldsymbol{\Phi}$, with ring rotation within a run to average the
orientation-dependent truncation bias that sets the zero-noise tracking
error; extension to three dimensions, which raises the estimation stakes
to ten coefficients per component; and targets other than critical
points, transient attracting profiles and eddy cores among them.
...
A six-robot cluster recovers the local
...
one that vanishes where the ride ends.
