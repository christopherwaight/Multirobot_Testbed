...
\subsubsection{The trench profile has two wells and a crest}
...
\emph{Problem 1 (separatrix tracking).} Drive the cluster centroid onto
the trench of $D$ and traverse it, in the direction of the ambient flow,
to the saddle point at its end.

\emph{Problem 2 (Okubo-Weiss boundary tracking).} Drive $D(\mathbf{p}_c)
\to 0$ and circulate along the boundary $\mathcal{C}$.
...
measurements. The estimated flow at the centroid,
...
Combining the two effects, the total Hessian error is bounded by
...
third-derivative scale. This rule is consistent with the formation-radius
sweeps reported in Section~\ref{sec:results}.
...
first-order coefficient errors. The gradient (\ref{eq:grad_det}) and
Hessian (\ref{eq:hess_det}) are bilinear in (first-order, second-order)
and (second-order, second-order) coefficient pairs respectively, so their
errors obey bounds of the same form, linear in the coefficient errors
with slopes set by the true derivative magnitudes. Two conclusions carry
into the stability analysis: the estimate errors entering the controllers
are bounded whenever the coefficient errors are bounded, with explicit
...
% TODO: parameter table (symbol, value, units, description) mirroring the
% previous paper: v_max, k, v_stiction, v_max_robot, alpha_mom, tau, dt,
% A, eps_raw, eps_dim, eps_grad, rho (formation radius).
...
\begin{theorem}[Separatrix traversal and terminal capture]
...
tube $T_w$
(i) remains in $T_w$ and converges exponentially to the trench, (ii)
...
$L_\Gamma/m$ with $L_\Gamma$ the trench length, and (iii) converges
exponentially to the saddle, where $D$ attains its local minimum and
$\mathbf{H}_D \succ 0$.
\end{theorem}
...
reached in time at most $L_\Gamma/m$. (iii): inside the ball,
$\mathbf{H}_D \succ 0$ (the well is nondegenerate:
$\mathbf{H}_D = 2\pi^6 A^2\,\mathbf{I}$ at the double-gyre saddles), the
selector is in its attract fallback, and Theorem~\ref{thm:attract} gives
local exponential convergence to the well, which coincides with the flow
saddle $\mathbf{p}^*$.
\end{IEEEproof}
...
the crest, and doubles as the terminal capture law at the wells. The
...
\begin{figure}[!t]
    ...
    shaded interquartile range in (a)) at a strain-region point of the
    double gyre. (a) Versus measurement noise $\sigma_{uv}$ at the nominal
