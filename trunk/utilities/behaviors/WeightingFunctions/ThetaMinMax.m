function [SV] = ThetaMinMax(NRobot,RobotParams,DL,SysParam)
...
SV(1,1:2:2*N) = ones(1,N);
%theta method 
%find theta based on the angle created by the minimum and amximum robot
[~,minR] = min(ss); 
[~,maxR] = max(ss); 
theta = atan2d(ys(maxR) - ys(minR), xs(maxR) - xs(minR)); 
