...
% Add sinking vortex quiver plot
...
% Fixed vortex component
U_vortex = r .* sin(theta);
V_vortex = -r .* cos(theta);

% Sink component (pointing inward toward center)
r_epsilon = r.^2 + 1e-6; % Add small epsilon to avoid division by zero
U_sink = -0.2 * (X - center_x) ./ r_epsilon;
V_sink = -0.2 * (Y - center_y) ./ r_epsilon;

% Combine components for sinking vortex
U = U_vortex + U_sink;
V = V_vortex + V_sink;
