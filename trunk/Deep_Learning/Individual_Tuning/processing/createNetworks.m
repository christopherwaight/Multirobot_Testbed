function [everything_hue_net, everything_sat_net] = createNetworks()
    ...
    hiddenLayerSizes1 = [randi([4, 10]) randi([4, 10]) ]; % 2 Neuron Output for Hue
    hiddenLayerSizes2 = [randi([4, 10]) randi([4, 10]) ]; % 1 Neuron Output for Saturation

    % Occasionally try a 3-layer network (20% of the time)
    if rand < 0.05
        hiddenLayerSizes1 = [randi([4, 15]) randi([4, 10]) randi([2, 10]) randi([2, 10])];
        hiddenLayerSizes2 = [randi([4, 15]) randi([4, 10]) randi([2, 10]) randi([2, 10])];
    end
    ...
    everything_hue_net.trainParam.max_fail = randi([20, 30]);     % Increase patience
    ...
    everything_sat_net.trainParam.max_fail = randi([20, 30]);     % Increase patience
