# main4.py - Unified 4-robot simulation runner
# Based on main3.py configuration system, adapted for 4-robot cluster
...
# Available 4-robot control primitives (from control_primitives.py):
#   "dual_jacobian_center_finder"  - Uses both top and bottom triangle Jacobians
#   "center_orbiter"                - Orbit around estimated center

# --- Visualization Options ---
FIGURE_SIZE = (12, 12)  # Figure size for compare mode
SAVE_FIGURE = False     # Set to True to save the plot
OUTPUT_FILENAME = "simulation_output_4robot.png"
...
# Control primitive mapping (4-robot specific)
control_primitive_map = {
    "dual_jacobian_center_finder": cp.dual_jacobian_center_finder,
    "center_orbiter": cp.center_orbiter,
}
...
PREDICTOR_DIR_MAP = {
    ...
    "spewing_vortex1": "sinking_vortex_predictors",  # Use sinking_vortex models as fallback
    "spewing_vortex2": "sinking_vortex_predictors",
    "spewing_vortex3": "sinking_vortex_predictors",
    "sink1": "sinking_vortex_predictors",  # Use sinking_vortex models as fallback
    "sink2": "sinking_vortex_predictors",
    "sink3": "sinking_vortex_predictors",
    "source1": "sinking_vortex_predictors",  # Use sinking_vortex models as fallback
    "source2": "sinking_vortex_predictors",
    "source3": "sinking_vortex_predictors",
}
...
def create_cluster(env_func, env_name):
    """Create a 4-robot cluster based on the selected mode and environment"""
...
def get_control_primitive():
    ...
    if CONTROL_PRIMITIVE not in control_primitive_map:
        print(f"WARNING: Unknown control primitive '{CONTROL_PRIMITIVE}', using 'dual_jacobian_center_finder'")
        return cp.dual_jacobian_center_finder
    return control_primitive_map[CONTROL_PRIMITIVE]

def run_blending_test(env_func):
    """Run blending verification test"""
    print("\n" + "=" * 60)
    print("TESTING BLENDED PREDICTIONS AT SAMPLE POINTS (4-ROBOT)")
    print("=" * 60)
    ...
    for point in test_points:
        ...
        for cluster in [test_cluster_blend, test_cluster_nn, test_cluster_rbf]:
            ...
            cluster.robot_offsets = np.array([[0, 0], [0, 0], [0, 0], [0, 0]])

        # Get readings from each mode (first robot only for comparison)
...
def run_single_simulation():
    ...
    print("=" * 60)
    print("4-ROBOT CLUSTER SIMULATION - SINGLE ENVIRONMENT")
    print("=" * 60)
...
def run_comparison_simulation():
    ...
    print("=" * 60)
    print("4-ROBOT CLUSTER SIMULATION - MODE COMPARISON")
    print("=" * 60)
...
def run_multi_environment_simulation():
    ...
    print("=" * 60)
    print("4-ROBOT CLUSTER SIMULATION - 18 ENVIRONMENTS")
    print("=" * 60)
    ...
    fig.suptitle(f'4-Robot Cluster Simulations - All 18 Environments ({mode_str})', fontsize=16)
