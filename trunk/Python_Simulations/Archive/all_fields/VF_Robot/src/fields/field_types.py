"""
Field abstractions for vector field environments.

All field types provide a unified interface: get_value(x, y) -> (u, v)
"""
import numpy as np
import torch
...
class AnalyticalField(VectorField):
    ...
    def __init__(self, field_function):
        """
        Args:
            field_function: Function with signature (x, y) -> (u, v)
        """
...
class NNField(VectorField):
    ...
    def __init__(self, predictor_dir='sinking_vortex_predictors', noise_std=0.00):
        """
        Args:
    ...
    def get_value(self, x, y):
        ...
        u = magnitude * np.cos(angle) + np.random.randn() * self.noise_std
        v = magnitude * np.sin(angle) + np.random.randn() * self.noise_std
...
class BlendedField(VectorField):
    ...
    def __init__(self, predictor_dir='sinking_vortex_predictors',
        ...
        self.nn_field = NNField(predictor_dir, noise_std=0.0)  # No noise in components
        self.rbf_field = RBFField(predictor_dir, noise_std=0.0)
