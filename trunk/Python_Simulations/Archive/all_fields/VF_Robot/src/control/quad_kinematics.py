...
def forward_kinematics_quad(x1, y1, x2, y2, x3, y3, x4, y4, prev_theta_c=None):
    ...
    Returns:
        dict with keys: {x_c, y_c, th_c, d1, d2, r1, r2, phi}
    """
    # Centroid
    x_c = (x1 + x2 + x3 + x4) / 4.0
    y_c = (y1 + y2 + y3 + y4) / 4.0
    ...
    phi = -np.arctan2(y4 - y2, x4 - x2) + np.arctan2(y3 - y1, x3 - x1)
...
def inverse_kinematics_quad(x_c, y_c, th_c, d1, d2, r1, r2, phi):
    """
...
def compute_inverse_jacobian_quad(d1, d2, r1, r2, phi, th_c, epsilon=1e-6):
    """
    ...
    [vx1, vy1, vx2, vy2, vx3, vy3, vx4, vy4]^T = J_inv * [vx_c, vy_c, omega_c, vd1, vd2, vr1, vr2, vphi]^T
    ...
    x1_0, y1_0, x2_0, y2_0, x3_0, y3_0, x4_0, y4_0 = inverse_kinematics_quad(
        x_c_nom, y_c_nom, th_c, d1, d2, r1, r2, phi
    )
    ...
    shape_vars = [
        (x_c_nom, y_c_nom, th_c, d1, d2, r1, r2, phi),  # Nominal
    ]

    # Index: 0=x_c, 1=y_c, 2=th_c, 3=d1, 4=d2, 5=r1, 6=r2, 7=phi
    for i in range(8):
        # Create perturbed configuration
        x_c_p, y_c_p, th_c_p, d1_p, d2_p, r1_p, r2_p, phi_p = shape_vars[0]
...
def shape_velocities_to_robot_velocities_quad(J_inv, vx_c, vy_c, omega_c=0.0,
                                               vd1=0.0, vd2=0.0, vr1=0.0, vr2=0.0, vphi=0.0):
    """
    ...
    Args:
        ...
        vphi: Angle between diagonals velocity (default 0)

    Returns:
        vx1, vy1, vx2, vy2, vx3, vy3, vx4, vy4 (robot velocities)
    """
    # Shape velocity vector
    shape_vel = np.array([vx_c, vy_c, omega_c, vd1, vd2, vr1, vr2, vphi])
