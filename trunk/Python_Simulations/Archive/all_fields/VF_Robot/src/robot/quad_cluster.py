...
class QuadCluster:
    ...
    def sample_scalar_field_at_robots(self):
        """
        Get scalar field readings at each robot position.

        For use with ScalarField environments.

        Returns:
            List of z values (scalars) for each robot (4 values)
        """
        return [robot.sample_scalar_field(self.field) for robot in self.robots]

    def estimate_gradient_and_hessian(self):
        """
        Estimate gradient and Hessian from 4-robot scalar field measurements.

        Uses all C(4,3)=4 combinations of 3 robots to estimate gradients,
        then fits planes to gradient components to estimate the Hessian.

        This is the core estimation method for Newton's method saddle finding.

        Returns:
            avg_gradient: numpy array [grad_x, grad_y] - average gradient
            hessian: 2x2 numpy array - Hessian matrix
        """
        ...
        z_values = np.array([robot.sample_scalar_field(self.field)
                            for robot in self.robots])
        ...
        for combo in combinations(range(4), 3):
            ...
            # Fit plane to get gradient
            A = np.column_stack([selected_points[:, 0],
                                selected_points[:, 1],
                                np.ones(3)])
            b = selected_points[:, 2]
        ...
        A_hess = np.column_stack([centroids[:, 0], centroids[:, 1], np.ones(4)])

        try:
            # Solve for ∂z/∂x plane coefficients
            coeffs_dx = np.linalg.lstsq(A_hess, dz_dx, rcond=None)[0]
            d2z_dx2 = coeffs_dx[0]          # ∂²z/∂x²
            d2z_dxdy_from_dx = coeffs_dx[1]  # ∂²z/∂x∂y
        except np.linalg.LinAlgError:
            d2z_dx2 = 0.0
        ...
        try:
            ...
            coeffs_dy = np.linalg.lstsq(A_hess, dz_dy, rcond=None)[0]
            d2z_dxdy_from_dy = coeffs_dy[0]  # ∂²z/∂y∂x
