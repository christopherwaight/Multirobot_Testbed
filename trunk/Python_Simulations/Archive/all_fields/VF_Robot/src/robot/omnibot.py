...
class Omnibot:
    ...
    def sample_field(self, field):
        ...
        Args:
            field: VectorField object with get_value(x, y) method
    ...
    def sample_scalar_field(self, scalar_field):
        """
        Sample a scalar field at the robot's current position.

        Args:
            scalar_field: ScalarField object with get_value(x, y) method

        Returns:
            z: Scalar field value at robot position
        """
        return scalar_field.get_value(self.position[0], self.position[1])
