#!/usr/bin/env python3
"""
generate_simulation_orbital_table_8.py - Generate Simulation Orbital Control Data

Runs analytical (no noise) simulations for orbital control at various radii
across 8 field types (6 analytical + 2 RBF) to create comparison table for the paper.

Tests:
- 8 field types: sink, source, vortex, saddle, sinking_vortex, spewing_vortex, vortex_rbf, saddle_rbf
- 1 robot config: 3-robot only
- 8 radii: 0.01, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70 m

Output: simulation_orbital_results_8fields.csv with columns:
  - field_type: sink, source, vortex, saddle, sinking_vortex, spewing_vortex, vortex_rbf, saddle_rbf
  - robot_type: 3-robot
...
# Field configurations (8 fields: 6 analytical + 2 RBF)
FIELD_CONFIGS = {
    'sink': sink1,
    'source': source1,
    'vortex': vortex1,
    ...
    'vortex_rbf': vortex_predictor_dir,  # RBF approximation
    'saddle_rbf': saddle_predictor_dir,  # RBF approximation
}
...
# Output file
OUTPUT_FILE = os.path.join(script_dir, "simulation_orbital_results_8fields.csv")
