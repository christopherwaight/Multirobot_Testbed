"""
main_ocean_hfr_2km_oecs.py

OECS/TRAP tracker (Primitive 10, core-seek) on the real 2km Santa Barbara
Channel HFR field, at the same validated operating point as the Logic C
run in main_ocean_hfr_2km_ftle_overlay.py (alpha_mom=0, stiction 0.002,
V_MAX 0.04, GAIN 1.8, TIME_WARP 6000, pentagon_small_2km 0.7x formation,
start 34.40N -120.39W, 168 steps = 28 h of field time).

Differences from the Logic C run:
  - Primitive: oecs_trap_step in core-seek mode. Strain thresholds are
    set from the field's own s1 scale, probed 2026-07-09 over the world
    domain at t=0 (finite differences, h=0.01): median s1 -0.73, 5th
    percentile -3.0, start value -1.06. Chosen: S_TRIM = 0.3 (attraction
    dominance), S_CAPTURE = -3.0 (park inside a deep TRAP core well).
  - Ground-truth overlay: offline TRAP cores extracted from the full
    radar grid at the final field time (s1 by finite differences on a
    120x120 world grid; a core is a 3x3 local minimum with s1 below the
    5th percentile), the lightweight version of the Serra-style
    extraction in the paper's ocean TRAP protocol (tensor-line curves
    are future work). The distance from the parked centroid to the
    nearest offline core is the headline metric.

Paper traceability: produces the candidate figure
  Paper_Writing/Separatrix_and_OW_Paper/figures/ocean_oecs_overlay_2km.png
(review copy ocean_data/det_jacobian_plots/oecs_trajectory_overlay_2km.png)
and the summary CSV experiments/outputs/oecs/ocean_oecs_summary.csv.
"""
import sys
import os
...
# ============================================================================
# CONFIGURATION (operating point identical to main_ocean_hfr_2km_ftle_overlay)
# ============================================================================
...
# OECS thresholds from the probed s1 scale of this field (docstring).
G_PERP    = 1.0
S_TRIM    = 0.3
S_CAPTURE = -3.0

START = (34.4, -120.39)     # lat, lon; same as the Logic C run

LON_MIN, LON_MAX = -120.7, -119.7
LAT_MIN, LAT_MAX =   33.8,   34.7

# FTLE background settings, matched to the Logic C overlay figure.
FTLE_HOURS    = 24
SUBSTEPS_HR   = 6
SEED_UPSAMPLE = 4
LAND_RGB      = (0.30, 0.55, 0.25)

# Offline TRAP-core extraction. Percentile 15 keeps cores deeper than
# 85% of the domain (s1 < ~-1.9 at the final frame); the first run used
# 5 and missed the tracked core, whose depth (-3.06) hovers exactly at
# the p5 boundary (-3.38), see the 2026-07-09 diagnosis in plan.md.
S1_GRID_N       = 120
S1_FD_H         = 0.01
...
# ============================================================================
# OFFLINE s1 FIELD AND TRAP CORES
# ============================================================================

def s1_grid(field, n=S1_GRID_N, h=S1_FD_H, lim=0.62):
    """s1 by central differences on an n x n world grid at the field's
    CURRENT internal time. Returns (xs, ys, S1) with S1[j, i] at
    (xs[i], ys[j]); NaN where the field is invalid."""
...
def trap_cores(xs, ys, S1, percentile=CORE_PERCENTILE):
    """3x3 local minima of S1 below the given percentile."""
...
def main():
    ...
    field.reset_clock()
    print(f"Running OECS core-seek from ({START[0]:.3f}N, {START[1]:.3f}W)...")
    ...
    # Offline cores at the FINAL field time (field clock is already there).
    ...
    print(f"  {len(cores)} offline cores; parked {d_core_world:.4f} world "
          f"units ({d_core_km:.1f} km) from the nearest, s1 tail mean "
          f"{np.mean(s1_tail):.2f}")

    # -- FTLE background (same style as the Logic C overlay) ----------------
    ...
    fig.suptitle(
        "OECS/TRAP Tracker (core-seek) on Forward-Time FTLE -- 2km SBC",
        fontsize=13, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.94])

    out_path = os.path.join(OUT_DIR, "oecs_trajectory_overlay_2km.png")
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    print(f"Saved: {out_path}")

    csv_path = os.path.join(CSV_DIR, "ocean_oecs_summary.csv")
