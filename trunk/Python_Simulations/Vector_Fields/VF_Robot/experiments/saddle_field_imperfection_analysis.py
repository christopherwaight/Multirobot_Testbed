"""
Saddle Field Imperfection Analysis

Compares NN and RBF field approximations against analytical ground truth for saddle1 field,
...
# Create non-rotated saddle for matching the printed field
def saddle_no_rotation(x, y):
    """Saddle field without rotation to match printed field"""
    center_x = 0
    center_y = 0
    strength = 1
    ...
    # Standard saddle without rotation
...
# Output directory
OUTPUT_DIR = 'saddle_field_analysis_plots'
...
def load_raw_training_data(filename='saddle_data.csv', max_points=200):
    """
    Load and process raw training data for comparison.
    Uses SWAP_NEGATE_ROTATED transformation: (-y, -x) then rotate 90° CW.

    This transformation was identified through systematic testing to best match
    the analytical saddle field visualization.

    Transformation steps:
    1. Swap and negate: (x_raw, y_raw) → (-y_raw, -x_raw)
    2. Rotate 90° CW: (x, y) → (y, -x)
    3. Combined effect: (x_raw, y_raw) → (-x_raw, y_raw)
    4. Rotate hue by -90° (subtract 0.25 in hue space)
    ...
    for i in range(len(df)):
        # Apply SWAP_NEGATE_ROTATED transformation
        # Step 1: Swap and negate (-y, -x)
        # Step 2: Rotate 90° CW: (x,y) → (y, -x)
        # Combined: (x_raw, y_raw) → (-x_raw, y_raw)

        # First measurement set
        x_temp = -df.iloc[i, 1]  # -y_raw
        y_temp = -df.iloc[i, 0]  # -x_raw
        # Rotate 90° CW
        x_new = y_temp  # -x_raw
        y_new = -x_temp  # y_raw
        hue_new = (df.iloc[i, 3] - 0.25) % 1.0  # Rotate hue -90°

        stacked_data.append([x_new, y_new, hue_new, df.iloc[i, 4]])

        # Second measurement set
        x_temp = -df.iloc[i, 6]  # -y_raw
        y_temp = -df.iloc[i, 5]  # -x_raw
        x_new = y_temp
        y_new = -x_temp
        hue_new = (df.iloc[i, 8] - 0.25) % 1.0

        stacked_data.append([x_new, y_new, hue_new, df.iloc[i, 9]])

        # Third measurement set
        x_temp = -df.iloc[i, 11]  # -y_raw
        y_temp = -df.iloc[i, 10]  # -x_raw
        x_new = y_temp
        y_new = -x_temp
        hue_new = (df.iloc[i, 13] - 0.25) % 1.0

        stacked_data.append([x_new, y_new, hue_new, df.iloc[i, 14]])
...
def plot_error_statistics(metrics_nn, metrics_rbf):
    ...
    fig.suptitle('Statistical Comparison: NN vs RBF Approximation Errors (Saddle1)',
...
def plot_error_histograms(metrics_nn, metrics_rbf):
    ...
    fig.suptitle('Error Distribution Histograms (Saddle1)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def generate_text_report(metrics_nn, metrics_rbf, scale_factor):
    ...
    with open(report_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("SADDLE FIELD IMPERFECTION ANALYSIS REPORT\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("\nNote: This analysis uses saddle field (saddle1) as reference.\n")
        f.write("Results may vary for different field types (sink, vortex, etc.).\n")
...
def main():
    print("=" * 80)
    print("SADDLE FIELD IMPERFECTION ANALYSIS")
    print("=" * 80)
    ...
    field_analytical = AnalyticalField(saddle1)  # Use saddle1 with 45° rotation
    ...
    raw_df = load_raw_training_data('saddle_data.csv', max_points=200)
