...
class QuadCluster:
    ...
    psi (cross-diagonal angle between diagonals).
    ...
    def _load_formation_config(self, config_path):
        ...
        # Accept both psi_degrees (new) and phi_degrees (legacy) for backwards compatibility
        psi_deg = formation.get('psi_degrees', formation.get('phi_degrees'))
        if psi_deg is None:
            raise KeyError("Formation config missing 'psi_degrees' (or legacy 'phi_degrees')")
        self.desired_psi = np.radians(psi_deg)
        ...
        print(f"Loaded quad formation config: d1={self.desired_d1}, d2={self.desired_d2}, "
              f"r1={self.desired_r1}, r2={self.desired_r2}, psi={np.degrees(self.desired_psi):.1f}°")
    ...
    def move(self, control_primitive):
        ...
        error_psi = self.desired_psi - current_formation['psi']

        # Wrap psi error to [-pi, pi]
        error_psi = np.mod(error_psi + np.pi, 2*np.pi) - np.pi
        ...
        vpsi = self.position_gain * error_psi
        ...
        vx1, vy1, vx2, vy2, vx3, vy3, vx4, vy4 = shape_velocities_to_robot_velocities_quad(
            J_inv, vx_c_desired, vy_c_desired, omega_c, vd1, vd2, vr1, vr2, vpsi
        )
    ...
    def __repr__(self):
        ...
        return (f"QuadCluster(centroid=({formation['x_c']:.3f}, {formation['y_c']:.3f}), "
                ...
                f"psi={np.degrees(formation['psi']):.1f}°)")
