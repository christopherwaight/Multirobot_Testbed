...
% MODIFIED: Cluster center is now calculated from 4 robot positions
...
robot4_color = [0.8 0.5 0.2];   % Orange (added for robot 4)
...
group_avg_robot4 = cell(1, num_groups);
group_std_robot4 = cell(1, num_groups);
...
for group_idx = 1:num_groups
    ...
    for run_idx = 1:length(existing_files)
        ...
        % Store robot trajectories if they exist
        ...
        if robot4_exists
            all_robot4_traj{end+1} = temp_data.robot4_pose(:,1:2);
        end
        
        % Calculate cluster center from robot positions
        if robot1_exists && robot2_exists && robot3_exists && robot4_exists
            % Get the number of time steps (assuming all robots have same length)
            num_steps = size(temp_data.robot1_pose, 1);
            
            % Initialize cluster center trajectory
            cluster_center = zeros(num_steps, 2);
            
            % Calculate cluster center for each time step
            for t = 1:num_steps
            ...
            all_cluster_traj{end+1} = cluster_center;
            fprintf('  Calculated cluster center from 4 robot positions\n');
        else
            % If not all 4 robots exist, check if cluster_pose exists in the data
            if isfield(temp_data, 'cluster_pose')
                all_cluster_traj{end+1} = temp_data.cluster_pose(:,1:2);
                fprintf('  Using existing cluster_pose from file\n');
            else
                fprintf('  Warning: Could not calculate cluster center (missing robot data)\n');
            end
        end
    end
    ...
    if ~isempty(all_robot4_traj)
        [group_avg_robot4{group_idx}, group_std_robot4{group_idx}] = averageTrajectories(all_robot4_traj);
        fprintf('Processed %d robot4 trajectories for group %d\n', length(all_robot4_traj), group_idx);
    end
end
...
% Add vortex field (matching Python vortex1)
...
for group_idx = 1:num_groups
    if ~isempty(group_avg_robot1{group_idx}) || ~isempty(group_avg_robot2{group_idx}) || ...
       ~isempty(group_avg_robot3{group_idx}) || ~isempty(group_avg_robot4{group_idx})
        ...
        if ~isempty(group_avg_robot4{group_idx})
            plotTrajectoryWithConfidence(group_avg_robot4{group_idx}, group_std_robot4{group_idx}, ...
                robot4_color, 2, 0.2, 'Robot 4');
        end
