"""
analytical_vs_real_comparison.py - Analytical Field vs Real Robot Comparison

Compares robot behavior between:
1. Analytical (ground truth mathematical field)
2. Real Robot (actual hardware data from MATLAB)
...
THIS FILE CREATES FIGURE 11 for my paper!!!!
"""
import sys
import os
...
OUTPUT_DIR = os.path.join(script_dir, "analytical_vs_real_results")
SAVE_PLOTS = True
...
# Field modes to compare (only analytical for this script)
FIELD_MODES = ['analytical']
...
def plot_multimode_comparison(trajectories: Dict[str, np.ndarray],
    ...
    """
    Create comparison plot showing analytical field vs real robot.
    """
    ...
    plot_configs = [
        ('analytical', 'Simulation', axes[0]),
        ('real', 'Hardware', axes[1])
    ]
...
def main():
    ...
    print("=" * 70)
    print("ANALYTICAL VS REAL ROBOT COMPARISON")
    print("=" * 70)
    print(f"Configuration:")
    print(f"  Environment: {ENVIRONMENT}")
    print(f"  Field Modes: Analytical only")
    ...
    for file_idx, trajectory_file in enumerate(trajectory_files):
        ...
        try:
            ...
            print(f"    Difference: {stats.get('analytical_vs_real_error', np.nan):+.4f}m")

            # Create plot for selected configurations
            if SAVE_PLOTS and orbit_name in PLOTS_TO_SAVE:
                plot_path = os.path.join(OUTPUT_DIR, f"{orbit_name}_analytical_vs_real.png")
    ...
    if len(results_df) > 0:
        ...
        print("\nAnalytical vs Real Robot Error:")
        col = 'analytical_vs_real_error'
        if col in results_df.columns:
            mean_err = results_df[col].mean()
            std_err = results_df[col].std()
            print(f"  Mean difference: {mean_err:+.4f} ± {std_err:.4f} m")
    ...
    print("• Analytical vs Real error shows the sim-to-real gap")
    print("• Real robot behavior includes unmodeled dynamics (friction, sensor noise, etc.)")
