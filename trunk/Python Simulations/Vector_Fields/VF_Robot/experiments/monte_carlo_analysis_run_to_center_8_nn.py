...
Tests each primitive on 8 field types (6 analytical + 2 NN) with 1000 random starting positions,
calculating average final position and RMSE.

NN VERSION: Uses neural network approximations instead of RBF.
"""
...
ENVIRONMENTS = {
    ...
    'saddle1': saddle1,
    'vortex1_nn': vortex_predictor_dir,  # NN approximation of vortex
    'saddle1_nn': saddle_predictor_dir   # NN approximation of saddle
}
...
def run_single_trial(field_func, primitive_config, start_x, start_y, save_trajectory=False):
    ...
    """
    try:
        # Create field (analytical or NN)
        if isinstance(field_func, str):
            # NN field - field_func is a directory path
...
def run_monte_carlo_analysis():
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "results_8fields_1000runs_nn.csv")
