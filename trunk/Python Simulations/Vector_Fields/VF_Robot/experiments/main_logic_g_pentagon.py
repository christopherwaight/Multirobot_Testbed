"""
main_logic_g_pentagon.py

6-robot pentagon cluster tracking the det(J)=0 Okubo-Weiss contour of a static
double-gyre vector field using logic_g_zero_flow_pentagon (heuristic Logic G).

Same control law as the 4-robot version but using the pentagon's existing
6-parameter quadratic-fit infrastructure.  Fixed orientation (no omega).
...
from src.control.pentagon_primitives import logic_g_zero_flow_pentagon
...
# Same 5 starting points as the quad experiment for direct comparison.
START_POINTS = [
...
def run_single(ax, start_x, start_y, title):
    ...
    def primitive(c):
        vx, vy = logic_g_zero_flow_pentagon(
...
def main():
    ...
    plt.suptitle(
        "Pentagon Cluster -- Logic G Heuristic, Okubo-Weiss Contour\n"
