...
% Calculate bin edges
eps = 0.025; % This circumvents the 0-1 wrapping issue
%bin_edges = linspace(0+eps, 1, num_bins + 1);
bin_edges_hue = [0.021, 0.3, 0.5,0.7,0.85,1];
% Initialize an array to store coefficients for each bin
num_bins_hue = length(bin_edges_hue)-1;
all_coefficients_hue = zeros(num_bins_hue, 2);


% Loop through the bins
for i = 1:num_bins_hue
    
    % Logical indexing to filter data
    %filter_idx = X(:,5) > bin_edges(i) & X(:,5) < bin_edges(i+1);
    filter_idx = Ytrue(:,1) > bin_edges_hue(i) & Ytrue(:,1) < bin_edges_hue(i+1);
    Y_filtered = Y(filter_idx, 1);
    X_filtered = X(filter_idx, 5);
    ...
    all_coefficients_hue(i, :) = coefficients;
    ...
    xlabel('hue (True)');
    ylabel('hue (Measured)');
    title(['Hue between ' num2str(bin_edges_hue(i)) ' and ' num2str(bin_edges_hue(i+1))]);
...
hue_cal1_rmse = rmse(Ytrue(:,1), X(:,5),1);
hue_cal1_rmse = rmse(Ytrue(Ytrue(:,1) >= 0.021, 1), X(Ytrue(:,1) >= 0.021, 5), 1)
sat_cal1_rmse = rmse(Ytrue(:,2), X(:,6),1)
