%% Training a NN to recognize Hue %%
...
output_size = 1;
epochs = 1e6;
learning_rate = 0.0001; % Adam typically uses a smaller learning rate
batch_size = 64; % Example batch size 
beta1 = 0.9; % Adam parameters
beta2 = 0.999;
epsilon = 1e-8; 
threshold = 0.01; % Loss threshold for early stopping
...
for i = 1:epochs
    for j = 1:batch_size:size(X_train, 1) % Batching 
        X_batch = X_train(j:min(j+batch_size-1, size(X_train, 1)), :);
        Y_batch = Y_train(j:min(j+batch_size-1, size(X_train, 1)));
